(function() {
    ...
    angular
        .module('app.pages')
        .controller('TreatmentPlanUpdateController', TreatmentPlanUpdateController);

    function TreatmentPlanUpdateController($scope, $interval, $stateParams, $state, $translate, utl, $filter) {
        var vm = this;
        ...
        $scope.getDetailsCallback = function(scope, data, options, hasError) {
            ...
            if ($scope.TreatmentPlanDetails.length > 0) {
                ...
                $scope.item.ServiceName = $scope.TreatmentPlanDetails[0].ServiceName;
        ...
        $scope.updateplan = function(detailInfo) {
            utl.Modal.open('patientemr.treatmentplanupdateform', {
                params: {
                    id: detailInfo.Id,
                },
        ...
        $scope.backToList = function() {
            $state.go('patientemr.treatmentplan', { pid: $scope.currentcontext.pid });
        }
        ...
        $scope.computedays = function(item) {
            if ($scope.item.NoOfDays > 0) {
                ...
                for (var i = 1; i <= $scope.item.NoOfDays; i++) {
                    ...
                    item.ServiceItemId = $scope.item.ServiceItemId;
                    item.ServiceCode = $scope.item.ServiceCode;
                    item.ServiceName = $scope.item.ServiceName;
                    item.ServiceCategoryId = $scope.item.ServiceCategoryId;
                    item.ServicePrice = $scope.item.ServicePrice;
    ...
    TreatmentPlanUpdateController.$inject = ['$scope', '$interval', '$stateParams', '$state', '$translate', 'utl', '$filter'];
