(function () {
    ...
    opbilling.controller('updateConsBillingController', updateConsBillingController);

    function updateConsBillingController($rootScope, $scope, $stateParams, $state, $translate, utl, $filter, $timeout, $uibModalInstance, modalConfig) {
        var vm = this;
        ...
        $scope.getPatientInfo = function (scope, data, options, hasError) {
            ...
            if ($scope.selectedPatient.Encounters && $scope.selectedPatient.Encounters.length > 0) {
                ...
                $scope.currentcontext.DoctorName = '';
                if (encounter.Doctor) {
                    if (encounter.Doctor.Title) {
                        $scope.currentcontext.DoctorName = encounter.Doctor.Title.Description;
                    }
                    if (encounter.Doctor.FirstName) {
                        $scope.currentcontext.DoctorName += ' ' + encounter.Doctor.FirstName;
                    }
                    if (encounter.Doctor.LastName) {
                        $scope.currentcontext.DoctorName += ' ' + encounter.Doctor.LastName;
                    }
                }
        ...
        $scope.updateenc = function () {
            $scope.items.Id = $scope.currentcontext.eid;
            $scope.items.BillingStatusId = 2;
            ...
            var options = {
                ...
                type: 'post',
                onComplete: $scope.updateencCallback
            };
            utl.Http.doAction(options);
        }
    ...
    updateConsBillingController.$inject = ['$rootScope', '$scope', '$stateParams', '$state', '$translate', 'utl', '$filter', '$timeout', '$uibModalInstance', 'modalConfig'];
