(function () {
    ...
    angular
        .module('app.pages')
        .controller('PharmacyDiscountReportController', PharmacyDiscountReportController);

    function PharmacyDiscountReportController($scope, $stateParams, $state, $translate, $filter, utl) {
        var vm = this;
        ...
        $scope.excelDownloadCallbackExcel = function (scope, data, options, hasError) {
                ...
                if (rowArray.DiscountApprovedUser) {
                    if (rowArray.DiscountApprovedUser.Title) {
                        if (rowArray.DiscountApprovedUser.Title.Description) {
                            discount = rowArray.DiscountApprovedUser.Title.Description;
                        }
                    }
                    if (rowArray.DiscountApprovedUser.FirstName) {
                        discount += ' ' + rowArray.DiscountApprovedUser.FirstName;
                    }
                    if (rowArray.DiscountApprovedUser.LastName) {
                        discount += ' ' + rowArray.DiscountApprovedUser.LastName;
                    }
                }
            ...
            hiddenElement.download = 'pharmacydiscount-report.csv';
            hiddenElement.click();
        ...
        $scope.print = function () {
            ...
            var options = {
                action: 'billing/patientbills/PrintPharmacyDiscountReport',
                data: inputData,
                type: 'post'
            };
        ...
        vm.gridConfig = {
            ...
            },
            {
                ...
                <span ng-if='entity.DiscountApprovedUser.Title && entity.DiscountApprovedUser.Title.Description'>{{entity.DiscountApprovedUser.Title.Description}}&nbsp;</span>\
                <span>{{entity.DiscountApprovedUser.FirstName}}</span>&nbsp;<span>{{entity.DiscountApprovedUser.LastName}}</span>\
                 </div>"
            },
    ...
    PharmacyDiscountReportController.$inject = ['$scope', '$stateParams', '$state', '$translate', '$filter', 'utl'];
