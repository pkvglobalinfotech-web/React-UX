(function () {
    ...
    angular
        .module('app.pages')
        .controller('ipbillcancelController', ipbillcancelController);

    function ipbillcancelController($scope, $stateParams, $state, $translate, utl, $uibModalInstance, modalConfig, $filter, $timeout) {
        var vm = this;
        ...
        $scope.cancelItem = function () {
            ...
            $scope.item.CancelReason = $scope.currentfilter.CancelReason;
            console.log($scope.item);
            var lines = cancelLinesForSave();
            ...
            var options = {
                // action: 'billing/patientbills/UpdatePatientBillsCancel',
                action: 'billing/patientbills/UpdatePatientBillsFromCancel',
                data: {
                    Data: inputData
                },
                type: 'post',
        ...
        function cancelLinesForSave() {
            var result = [];
            for (var idx in $scope.item.PatientBillDetails) {
                var item = $scope.item.PatientBillDetails[idx];
        ...
        $scope.getItemCallback = function (scope, res, options, hasError) {
            $scope.item = res.Data;
            $scope.currentfilter.CancelReason = $scope.item[0].CancelReason;
        };
        ...
        $scope.getItem = function () {
            if ($scope.currentcontext.bid && $scope.currentcontext.bid > 0) {
                var inputData = {
                    Params: [{
                        Key: 0,
                        Value: $scope.currentcontext.bid
                    }]
                };
                var options = {
                    action: 'billing/PatientBills/GetPatientBills',
    ...
    ipbillcancelController.$inject = ['$scope', '$stateParams', '$state', '$translate', 'utl', '$uibModalInstance', 'modalConfig', '$filter', '$timeout'];
