(function () {
    ...
    angular
        .module('app.pages')
        .controller('insurancedisallowancereportController', insurancedisallowancereportController);

    function insurancedisallowancereportController($scope, $stateParams, $state, $translate, $filter, utl) {
        var vm = this;
        ...
        $scope.excelDownloadCallbackExcel = function (scope, data, options, hasError) {
            const JsonFields = ["Date and Time", "Receipt", "Payer Name", "Patient Name", "MRN", "Bill Number", "Bill Date","Disallowed", "Remark"]
            ...
            data.Data.forEach(function (rowArray) {
                ...
                if (rowArray.BillDateTime) {
                    disallow = rowArray.BillDateTime;
                }
                if (rowArray.Disallowed) {
                    billDate = rowArray.Disallowed;
                }
                if (rowArray.Comments) {
                    remark = rowArray.Comments;
                }
                csvContent += date + ',' + receipt + ',' + insurance + ',' + patName + ',' + mrn + ',' + billNum + ',' + disallow + ',' + billDate + ',' + remark + "\n";
            });
            var encodedUri = encodeURI(csvContent);
            var hiddenElement = document.createElement('a');
            hiddenElement.href = 'data:text/csv;charset=utf-8,' + encodedUri;
            hiddenElement.target = '_blank';
            hiddenElement.download = 'insurancedisallowancereport.csv';
            hiddenElement.click();
  ...
  if (!$scope.currentfilter.FromDate || $scope.currentfilter.FromDate == '' ||
        ...
        $scope.print = function () {
            ...
            var options = {
                action: 'billing/insurancepaymentdetails/PrintInsurancedisallowancereport',
                data: inputData,
                type: 'post'
            };
    ...
    insurancedisallowancereportController.$inject = ['$scope', '$stateParams', '$state', '$translate', '$filter', 'utl'];
