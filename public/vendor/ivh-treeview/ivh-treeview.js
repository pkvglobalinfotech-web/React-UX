...
angular.module('ivh.treeview').directive('ivhTreeviewCheckboxHelper', [function() {
  ...
  return {
    ...
    template: [
      '<label class="radio-container m-r-45"><input type="checkbox" checked="checked"',
        'class="ivh-treeview-checkbox"',
        'ng-model="isSelected"',
        'ng-click="resolveIndeterminateClick()"',
        'ng-change="trvw.select(node, isSelected)" /><span class="checkmark"></span></label>  '
    ].join('\n')
  };
}]);
...
angular.module('ivh.treeview').directive('ivhTreeview', ['ivhTreeviewMgr', function(ivhTreeviewMgr) {
  ...
  return {
    ...
    template: [
      '<ul class="ivh-treeview col-sm-12">',
        '<li ng-repeat="child in root | ivhTreeviewAsArray"',
            'ng-hide="trvw.hasFilter() && !trvw.isVisible(child)"',
            'class="ivh-treeview-node col-sm-6 "',
            'ng-class="{\'ivh-treeview-node-collapsed\': !trvw.isExpanded(child) && !trvw.isLeaf(child)}"',
            'ivh-treeview-node="child"',
...
angular.module('ivh.treeview').provider('ivhTreeviewOptions', [
  ...
  var options = {
     ...
     */
    nodeTpl: [
      '<div class="ivh-treeview-node-content {{trvw.label(node)}}" title="{{trvw.label(node)}}">',
        '<span ivh-treeview-toggle>',
          '<span class="ivh-treeview-twistie-wrapper" ivh-treeview-twistie></span>',
        '</span>',
        '<span class="ivh-treeview-checkbox-wrapper main" ng-if="trvw.useCheckboxes()"',
            'ivh-treeview-checkbox>',
        '</span>',
        '<span class="ivh-treeview-node-label " ivh-treeview-toggle>',
          '<i class="fa {{trvw.icon(node)}}" aria-hidden="true"></i> ',
          '<span class="parentnode" >{{trvw.label(node)}}</span>',
        '</span>',
        '<div class="children-node" ivh-treeview-children></div>',
      '</div>'
    ].join('\n')
