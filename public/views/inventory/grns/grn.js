(function () {
    ...
    function GRNFormController($rootScope, $scope, $stateParams, $state, $translate, utl, $filter, $timeout) {
        ...
        $scope.beforeGSTCalculation = true;
        $scope.afterGSTCalculation = false;
        ...
        $scope.isPoExcelUpload = 0;
        $scope.isPoExcelUpload =
            utl.FacilitySetting.getFacilitySettingValue('billing', 'isPoExcelUpload');
            ...
            for (var idx in $scope.grnDetails) {
                ...
                if ($scope.item.GrnStatusId == 1) {
                    if ($scope.ShowmultipleDiscount == true) {
                        if ($scope.afterGSTCalculation == true) {
                            $scope.ComputeGstafterCalculation(grnitem);
                        } else {
                            $scope.ComputeMultipleDiscount(grnitem);
                        }
        ...
        //afterGSTCalculation
        $scope.ComputeGstafterCalculation = function (item) {
            console.log(item);
            // $scope.ComputeSchemeDiscount(item);
            item.GstAmount = 0;
            item.UnitGstAmount = 0;
            ...
            $scope.TotalNetAmount = 0;
            if ($scope.afterGSTCalculation == true) {
                calculatetotalAmountwithGroupGst();
            }
        ...
        $scope.computeAmount = function (item) {
            if (checkpoqty(item)) {
                if (parseFloat(item.GrnQuantity) > 0) {
                    ...
                    if ($scope.ShowmultipleDiscount == true) {
                        if ($scope.afterGSTCalculation == true) {
                            $scope.ComputeGstafterCalculation(item);
                        } else {
        ...
        // function groupAndCalculateGST(items) {
        //     let gstGroups = {};

        //     // Group items by GST rate
        //     items.forEach(item => {
        //         if (!gstGroups[item.gstRate]) {
        //             gstGroups[item.gstRate] = { totalPrice: 0, totalGST: 0, items: [] };
        //         }

        //         let gstAmount = (item.price * item.gstRate) / 100;
        //         let totalPriceWithGST = item.price + gstAmount;

        //         gstGroups[item.gstRate].totalPrice += item.price;
        //         gstGroups[item.gstRate].totalGST += gstAmount;
        //         gstGroups[item.gstRate].items.push({
        //             name: item.name,
        //             price: item.price,
        //             gstAmount,
        //             totalPriceWithGST
        ...
        // Call function
        function calculatetotalAmountwithGroupGst() {
            ...
            for (var idx in $scope.grnDetails) {
                ...
                if (activeitem.ItemMasterId > 0 && parseInt(activeitem.GrnQuantity) > 0 && activeitem.Status == 1) {
                    ...
                    if ($scope.ShowmultipleDiscount == true) {
                        // $scope.TotalDiscount1Amount = parseFloat(($scope.TotalDiscount1Amount + (activeitem.UomDiscount1Amount * parseInt(activeitem.GrnQuantityAfterConversion))).toFixed(4));
                        // $scope.TotalDiscount2Amount = parseFloat(($scope.TotalDiscount2Amount + (activeitem.UomDiscount2Amount * parseInt(activeitem.GrnQuantityAfterConversion))).toFixed(4));
                        if (activeitem.DiscountMode1Id == 2) {
                        ...
                        if (activeitem.DiscountMode2Id == 2) {
                        ...
                        if (activeitem.DiscountMode1Id == 1) {
                            $scope.TotalDiscount1Amount = parseFloat(($scope.TotalDiscount1Amount + (activeitem.UomDiscount1Amount * parseInt(activeitem.GrnQuantityAfterConversion))).toFixed(4));
                            // $scope.TotalDiscount1Amount = parseFloat(($scope.TotalDiscount1Amount + (activeitem.UomDiscount1Amount)).toFixed(4));
                        }
                        if (activeitem.DiscountMode2Id == 1) {
                            $scope.TotalDiscount2Amount = parseFloat(($scope.TotalDiscount2Amount + (activeitem.UomDiscount2Amount * parseInt(activeitem.GrnQuantityAfterConversion))).toFixed(4));
                        }
                        ...
                        $scope.TotalDiscountAmount = parseFloat(($scope.TotalDiscountAmount + (activeitem.UomDiscountAmount * parseInt(activeitem.GrnQuantityAfterConversion))).toFixed(4));
                    }
            ...
            //Gst Calculation
            if ($scope.ShowmultipleDiscount == true && $scope.afterGSTCalculation == false) {
            ...
            } else if ($scope.ShowmultipleDiscount == true && $scope.afterGSTCalculation == true) {
                let groupbills = _.groupBy($scope.grnDetails, 'GstPercentage');

                console.log(groupbills);
                let gstGroups = {};
                for (let idx in groupbills) {
                    let groupedBills = groupbills[idx];
                    console.log(idx);
                    if (groupedBills.length > 0 && groupedBills[0].ItemMasterId > 0) {
                        $scope.groupTaxableAmount = 0;
                        $scope.groupGstAmount = 0;
                        $scope.groupCGstAmount = 0;
                        $scope.groupSGstAmount = 0;
                        $scope.DiscountAmount = 0;
                        var gstPercentage = groupedBills[0].GstPercentage;
                        var cgstPercentage = groupedBills[0].CGstPercentage;
                        var sgstPercentage = groupedBills[0].SGstPercentage;
                        for (let i = 0; i < groupedBills.length; i++) {
                            let bills = groupedBills[i];
                            if (bills.ItemMasterId > 0 && parseInt(bills.GrnQuantity) > 0 && bills.Status == 1) {
                                $scope.groupTaxableAmount = parseFloat(($scope.groupTaxableAmount + bills.TaxableAmount).toFixed(4));
                            }
                        }
                        $scope.groupGstAmount = (($scope.groupTaxableAmount / 100) * parseInt(gstPercentage));
                        $scope.groupCGstAmount = (($scope.groupTaxableAmount / 100) * parseInt(cgstPercentage));
                        $scope.groupSGstAmount = (($scope.groupTaxableAmount / 100) * parseInt(sgstPercentage));
                        gstGroups[gstPercentage] = {
                            GstAmount: $scope.groupGstAmount,
                            CGstAmount: $scope.groupCGstAmount,
                            SGstAmount: $scope.groupSGstAmount,
                        }
                        $scope.TotalGstAmount = $scope.TotalGstAmount + $scope.groupGstAmount;
                        $scope.TotalCGstAmount = $scope.TotalCGstAmount + $scope.groupCGstAmount;
                        $scope.TotalSGstAmount = $scope.TotalSGstAmount + $scope.groupSGstAmount;
                    }

                }
                console.log(gstGroups);
                $scope.TotalNetAmount = parseFloat(parseFloat($scope.TotalGrossAmount) + parseFloat($scope.TotalGstAmount) - (parseFloat($scope.TotalDiscount1Amount) + parseFloat($scope.TotalDiscount2Amount))).toFixed(4);
            }
            else {
        ...
        function calculatetotalAmount() {
            ...
            for (var idx in $scope.grnDetails) {
                ...
                if (activeitem.ItemMasterId > 0 && parseInt(activeitem.GrnQuantity) > 0 && activeitem.Status == 1) {
                    $scope.TotalGrossAmount = parseFloat(($scope.TotalGrossAmount + activeitem.GrossAmount).toFixed(4));
                    if ($scope.ShowmultipleDiscount == true) {
                        // $scope.TotalDiscount1Amount = parseFloat(($scope.TotalDiscount1Amount + (activeitem.UomDiscount1Amount * parseInt(activeitem.GrnQuantityaf))).toFixed(4));
                        // $scope.TotalDiscount2Amount = parseFloat(($scope.TotalDiscount2Amount + (activeitem.UomDiscount2Amount * parseInt(activeitem.GrnQuantity))).toFixed(4));
                        if (activeitem.DiscountMode1Id == 2) {
                            $scope.TotalDiscount1Amount = parseFloat(($scope.TotalDiscount1Amount + (activeitem.UomDiscount1Amount * parseInt(activeitem.GrnQuantity))).toFixed(4));
                        }
        ...
        $scope.changeGstCalc = function (iVal) {
            ...
            if (iVal == 'afterGST') {
                $scope.beforeGSTCalculation = false;
            }
            for (var idx in $scope.grnDetails) {
                var grnitem = $scope.grnDetails[idx];
                if (grnitem.ItemMasterId > 0) {
        ...
        $scope.initLookup();
        // Grn Excel Upload
        $scope.grnUpload = function () {
            utl.Modal.open('app.importgrnexcel', {
                params: {
                    id: 0
                },
                confirmCallback: grnExcelData
            });
        };
        function grnExcelData(data) {
            console.log('Grn excel data', data);
            let poData = data.result;
            $scope.item.StoreMasterId = poData.StoreId;
            $scope.item.VendorFacilityMapId = poData.SupplierId;
            $scope.item.InvoiceNumber = poData.InvoiceNo;
            $scope.item.DcNumber = poData.DCNo;
            $scope.item.GpNumber = poData.GatePassNo;
            $scope.ExcelItems = poData.Items;
            $scope.getStoreDetails(poData.StoreId);
            $scope.getItemByExcel(poData);
        }
        $scope.getItemByExcelcallback = function (scope, res, options, hasError) {
            $scope.grnDetails = res.Data || [];
            $scope.grnDetails.forEach((grnitem, idx) => {
                grnitem.Id = 0;
                // grnitem.GstId = grnitem.ItemMaster.GstMaster.Id;
                // grnitem.InGstId = grnitem.ItemMaster.InGstMaster.Id;
                // grnitem.CGstId = grnitem.ItemMaster.CGstMaster.Id;
                // grnitem.SGstId = grnitem.ItemMaster.SGstMaster.Id;
                // grnitem.PurchaseUomId = grnitem.ItemMaster.PurchaseUomId;
                grnitem.BaseUomId = grnitem.ItemMaster.BaseUomId;
                // grnitem.SaleUomId = grnitem.ItemMaster.SaleUomId;
                if (grnitem.ItemMaster.ItemFacilityMaps && grnitem.ItemMaster.ItemFacilityMaps.length > 0) {
                    grnitem.ItemFacilityMapId = grnitem.ItemMaster.ItemFacilityMaps[0].Id;
                }
                grnitem.ManufacturerId = grnitem.ItemMaster.ManufacturerId;
                grnitem.IsMultiUse = grnitem.ItemMaster.IsMultiUse;
                grnitem.IsReusable = grnitem.ItemMaster.IsReusable;
                grnitem.Min = grnitem.ItemMaster.Min;
                grnitem.Max = grnitem.ItemMaster.Max;
                grnitem.NoOfTransactions = grnitem.ItemMaster.NoOfTransactions;
                grnitem.GrnQuantity = 0;
                grnitem.GrnQuantityAfterConversion = 0;
                grnitem.BatchId = '';
                grnitem.ExpiryDate = null;
                grnitem.ManufacturedDate = null;

                grnitem.UomCode = grnitem.ItemMaster.UomConversions[0].UomMaster.UomCode;
                grnitem.MasterItem = grnitem.ItemMaster;
                grnitem.HSNCode = grnitem.ItemMaster.HSNCode;

                grnitem.PurchaseUom = grnitem.PurchaseUom || {};
                // grnitem.PurchaseUom.UomCode = grnitem.ItemMaster.PurchaseUom.UomCode;
                if (grnitem.ItemMaster.PurchaseUom) {
                    grnitem.PurchaseUomId = grnitem.ItemMaster.PurchaseUom.Id || 0;
                    grnitem.PurchaseUom.Id = grnitem.ItemMaster.PurchaseUom.Id || 0;
                    grnitem.PurchaseUom.UomCode = grnitem.ItemMaster.PurchaseUom.UomCode;
                }
                grnitem.SaleUom = grnitem.SaleUom || {};
                if (grnitem.ItemMaster.SaleUom) {
                    grnitem.SaleUomId = grnitem.ItemMaster.SaleUom.Id || 0;
                    grnitem.SaleUom.Id = grnitem.ItemMaster.SaleUom.Id || 0;
                    grnitem.SaleUom.UomCode = grnitem.ItemMaster.SaleUom.UomCode;
                }
                if (grnitem.ItemMaster.UomConversions) {
                    if (grnitem.ItemMaster.UomConversions.length > 0) {
                        for (var idxUOM in grnitem.ItemMaster.UomConversions) {
                            var uom = grnitem.ItemMaster.UomConversions[idxUOM];
                            if (uom.UomTypeId == 2) {
                                grnitem.ConversionQuantity = uom.ConversionQuantity || 1;
                            }
                            if ($scope.item.IsOpenGRN && uom.UomTypeId == 2) {
                                grnitem.UomCode = uom.UomMaster.UomName;
                                if (grnitem.UomCode != '' && grnitem.UomCode != null) {
                                    grnitem.PurchaseUomId = uom.UomMaster.UomId || 0;
                                    grnitem.PurchaseUom.Id = uom.UomMaster.UomId || 0;
                                    grnitem.PurchaseUom.UomCode = uom.UomMaster.UomName;
                                }
                            }
                            if ($scope.item.IsOpenGRN && uom.UomTypeId == 3) {
                                grnitem.UomCode = uom.UomMaster.UomName;
                                if (grnitem.UomCode != '' && grnitem.UomCode != null) {
                                    grnitem.SaleUomId = uom.UomMaster.UomId || 0;
                                    grnitem.SaleUom.Id = uom.UomMaster.UomId || 0;
                                    grnitem.SaleUom.UomCode = uom.UomMaster.UomName;
                                }
                            }
                            if ($scope.item.IsOpenGRN && uom.UomTypeId == 1) {
                                grnitem.UomCode = uom.UomMaster.UomName;
                                if (grnitem.UomCode != '' && grnitem.UomCode != null) {
                                    grnitem.BaseUomId = uom.UomMaster.UomId || 0;
                                    grnitem.BaseUom.Id = uom.UomMaster.UomId || 0;
                                    grnitem.BaseUom.UomCode = uom.UomMaster.UomName;
                                }
                            }
                        }
                    }
                }
                grnitem.GstMaster = grnitem.GstMaster || {};
                if (grnitem.ItemMaster.GstMaster) {
                    grnitem.GstMaster.Id = grnitem.ItemMaster.GstMaster.Id || 1;
                    grnitem.GstId = grnitem.ItemMaster.GstMaster.Id || 1;
                    grnitem.GstMaster.GstCode = grnitem.ItemMaster.GstMaster.GstCode;
                    grnitem.GstCode = grnitem.ItemMaster.GstMaster.GstCode;
                    grnitem.GstMaster.GstPercentage = parseFloat(grnitem.ItemMaster.GstMaster.GstPercentage.toFixed(4));
                    grnitem.GstPercentage = parseFloat(grnitem.ItemMaster.GstMaster.GstPercentage.toFixed(4));
                }
                grnitem.CGstMaster = grnitem.CGstMaster || {};
                if (grnitem.ItemMaster.CGstMaster) {
                    grnitem.CGstMaster.Id = grnitem.ItemMaster.CGstMaster.Id || 1;
                    grnitem.CGstId = grnitem.ItemMaster.CGstMaster.Id || 1;
                    grnitem.CGstMaster.GstCode = grnitem.ItemMaster.CGstMaster.GstCode;
                    grnitem.CGstCode = grnitem.ItemMaster.CGstMaster.GstCode;
                    grnitem.CGstMaster.GstPercentage = parseFloat(grnitem.ItemMaster.CGstMaster.GstPercentage.toFixed(4));
                    grnitem.CGstPercentage = parseFloat(grnitem.ItemMaster.CGstMaster.GstPercentage.toFixed(4));
                }
                grnitem.SGstMaster = grnitem.SGstMaster || {};
                if (grnitem.ItemMaster.SGstMaster) {
                    grnitem.SGstMaster.Id = grnitem.ItemMaster.SGstMaster.Id || 1;
                    grnitem.SGstId = grnitem.ItemMaster.SGstMaster.Id || 1;
                    grnitem.SGstMaster.GstCode = grnitem.ItemMaster.SGstMaster.GstCode;
                    grnitem.CGstCode = grnitem.ItemMaster.SGstMaster.GstCode;
                    grnitem.SGstMaster.GstPercentage = parseFloat(grnitem.ItemMaster.SGstMaster.GstPercentage.toFixed(4));
                    grnitem.SGstPercentage = parseFloat(grnitem.ItemMaster.SGstMaster.GstPercentage.toFixed(4));
                }
                grnitem.InGstMaster = grnitem.InGstMaster || {};
                if (grnitem.ItemMaster.InGstMaster) {
                    grnitem.InGstMaster.Id = grnitem.ItemMaster.InGstMaster.Id || 1;
                    grnitem.InGstId = grnitem.ItemMaster.InGstMaster.Id || 1;
                    grnitem.InGstMaster.GstCode = grnitem.ItemMaster.InGstMaster.GstCode;
                    grnitem.InGstCode = grnitem.ItemMaster.InGstMaster.GstCode;
                    grnitem.InGstMaster.GstPercentage = parseFloat(grnitem.ItemMaster.InGstMaster.GstPercentage.toFixed(4));
                    grnitem.InGstPercentage = parseFloat(grnitem.ItemMaster.InGstMaster.GstPercentage.toFixed(4));
                }
                if (grnitem.ItemMaster.StockItem) {
                    grnitem.StockItemId = grnitem.ItemMaster.StockItem.Id;
                } else {
                ...
                if (grnitem.ItemMaster.ItemVendorMaps) {
                    var VendorMappedItem = grnitem.ItemMaster.ItemVendorMaps[0];
                    grnitem.UomPrice = VendorMappedItem.UomPrice;
                    grnitem.PurchasePrice = VendorMappedItem.Price;
                    grnitem.UomMrPrice = VendorMappedItem.UomMrPrice;
                    grnitem.MrPrice = VendorMappedItem.MrPrice;
                    grnitem.DiscountModeId = VendorMappedItem.DiscountModeId;
                    if (VendorMappedItem.DiscountModeId === 1) {
                        grnitem.DiscountMode = 'SR';
                        grnitem.Discount = VendorMappedItem.Discount;
                        grnitem.UomDiscountAmount = parseFloat(VendorMappedItem.Discount.toFixed(4));
                        grnitem.UomDiscount1Amount = parseFloat(VendorMappedItem.Discount1.toFixed(4));
                        grnitem.UomDiscount2Amount = parseFloat(VendorMappedItem.Discount2.toFixed(4));
                        grnitem.DiscountAmount = parseFloat((VendorMappedItem.Discount / VendorMappedItem.ConversionQuantity).toFixed(4));
                    } else if (grnitem.ItemMaster.DiscountModeId == 2) {
                        grnitem.DiscountMode = '%';
                        grnitem.Discount = VendorMappedItem.Discount;
                        grnitem.UomDiscountAmount = parseFloat(((grnitem.UomPrice / 100) * VendorMappedItem.Discount).toFixed(4));
                        grnitem.UomDiscount1Amount = parseFloat(((grnitem.UomPrice / 100) * VendorMappedItem.Discount1).toFixed(4));
                        grnitem.UomDiscount2Amount = parseFloat(((grnitem.UomPrice / 100) * VendorMappedItem.Discount2).toFixed(4));
                        grnitem.DiscountAmount = parseFloat(((grnitem.PurchasePrice / 100) * VendorMappedItem.Discount).toFixed(4));
                    } else {
                        grnitem.DiscountMode = '';
                        grnitem.Discount = 0;
                        grnitem.UomDiscountAmount = 0;
                        grnitem.UomDiscount1Amount = 0;
                        grnitem.UomDiscount2Amount = 0;
                        grnitem.DiscountAmount = 0;
                    }
    
                    grnitem.UomPriceAfterDiscount = parseFloat(grnitem.UomPrice - grnitem.UomDiscountAmount.toFixed(4));
                    grnitem.PurchasePriceAfterDiscount = parseFloat(grnitem.PurchasePrice - grnitem.DiscountAmount.toFixed(4));
                    ...
                    grnitem.GrossAmount = 0;
                    grnitem.NetAmount = 0;
                } else {
                    grnitem.PurchasePrice = 0;
                    grnitem.UomDiscountAmount = 0;
                    grnitem.UomDiscount1Amount = 0;
                    grnitem.UomDiscount2Amount = 0;
                    grnitem.DiscountAmount = 0;
                    grnitem.UomPriceAfterDiscount = 0;
                    grnitem.PurchasePriceAfterDiscount = 0;
                    grnitem.GstAmount = 0;
                    grnitem.UnitGstAmount = 0;
                    grnitem.CGstAmount = 0;
                    grnitem.UnitCGstAmount = 0;
                    grnitem.SGstAmount = 0;
                    grnitem.UnitSGstAmount = 0;
                    grnitem.UomCostPrice = 0;
                    grnitem.UnitCostPrice = 0;
                    grnitem.GrossAmount = 0;
                    grnitem.NetAmount = 0;
                }
        
                grnitem.IsBatchRequired = grnitem.ItemMaster.IsBatchMandatory || false;
                grnitem.IsExpiryRequired = grnitem.ItemMaster.IsExpiryMandatory || false;
                grnitem.IsManfRequired = grnitem.ItemMaster.IsManufacture || false;
                grnitem.IsMRPRequired = grnitem.ItemMaster.IsMRPRequired || false;
            ...
            if (Array.isArray($scope.ExcelItems) && $scope.ExcelItems.length > 0) {
                $scope.grnDetails.forEach((grnitem) => {
                    let matchedExcelItem = $scope.ExcelItems.find(excelItem => excelItem.ItemCode === grnitem.ItemCode);
        
                    if (matchedExcelItem) {
                        grnitem.GrnQuantity = matchedExcelItem.InvoiceQty || 0;
                        grnitem.FreeQty = matchedExcelItem.FreeQty || 0;
                        grnitem.ConversionQuantity = matchedExcelItem.CoversionQty || 0;
                        grnitem.TotalQuantityAfterConversion = matchedExcelItem.TotalQty || 0;
                        grnitem.BatchId = matchedExcelItem.BatchNo || 0;
                        // grnitem.dispExpiryDate =  utl.Formatter.formatDate(matchedExcelItem.ExpiryDate_MM_YY, 'MM/YY') || '';
                        grnitem.ExpiryDate =  moment(matchedExcelItem.ExpiryDate_MM_YY).format("MMYY");
                        grnitem.dispExpiryDate =  moment(matchedExcelItem.ExpiryDate_MM_YY).format("MMYY");
                        grnitem.UomPrice = matchedExcelItem.PrPriceUom || 0;
                        grnitem.UomMrPrice = matchedExcelItem.SalePrUom || 0;

                        grnitem.DiscountMode1Id = matchedExcelItem.DiscountMode1 || 0;
                        grnitem.Discount1 = matchedExcelItem.Discount1 || 0;
                        grnitem.DiscountMode2Id = matchedExcelItem.DiscountMode2 || 0;
                        grnitem.Discount2 = matchedExcelItem.Discount2 || 0;
                        grnitem.Discount = (Number(matchedExcelItem.Discount1) || 0) + (Number(matchedExcelItem.Discount2) || 0);
                        grnitem.DiscountModeId = matchedExcelItem.DiscountMode || 0;
                    }
                    $scope.computeAmount(grnitem);
                    $scope.validateFreeQty(grnitem);
                    $scope.CheckExpiryDate(grnitem);
                    $scope.computeforgivenPrice(grnitem);
                    $scope.ComputeMultipleDiscount(grnitem);
                    $scope.ComputeGstafterCalculation(grnitem);
                    // $scope.onDiscountModeChange(grnitem);
                    // $scope.OnSelectGst(grnitem);
                    calculatetotalAmount();
                    calculatetotalAmountwithGroupGst();
                });
            }
            $scope.addNewLineItem();
            // $scope.grnDetails.forEach((item, index) => {
            //     $scope.onMasterItemSelected(index, item);
            // });
            $scope.onDiscountModeChange();
            $scope.OnSelectGst();
        };
        $scope.getItemByExcel = function (data) {
            let action = '';
            let inputData = '';
            let itemCodes = Array.isArray(data.Items) ? data.Items.map(item => item.ItemCode) : [];
            if ($scope.jssgrn == 1) {
                action = 'pharmacy/itemmaster/GetInventoryItemsforGRN';
                inputData = {
                    Params: [{
                            Key: 13,
                            Value: 2
                ...
                action = 'pharmacy/itemmaster/GetItemsForGRN';
                inputData = {
                    Params: [
                        {
                            Key: 3,
            ...
            var options = {
                ...
                data: inputData,
                type: 'post',
                onComplete: $scope.getItemByExcelcallback
            };
            utl.Http.doAction(options);
        }
        $scope.getStoreDetailsCallback = function (scope, data, options, hasError) {
            $scope.exStoreData = hasError ? {} : data.Data[0];
            $scope.item.StoreCode = $scope.exStoreData.StoreCode;
            $scope.item.StoreName = $scope.exStoreData.StoreName;
            $scope.item.StoreTypeId = $scope.exStoreData.StoreMaster.StoreTypeId;
            $scope.item.StoreSubTypeId = $scope.exStoreData.StoreMaster.StoreSubTypeId;
            $scope.item.SequenceOptionId = $scope.exStoreData.StoreMaster.SequenceOptionId;
            $scope.item.IsOpenGRN = $scope.exStoreData.StoreMaster.CanAllowOpenGRN;
            $scope.item.IsGstEditablePo = $scope.exStoreData.StoreMaster.IsGstEditablePo;
            $scope.item.IsPOMandatory = $scope.exStoreData.StoreMaster.IsPOMandatory;
        }
        ...
        $scope.getStoreDetails = function (storeId) {
            ...
            var options = {
                ...
                data: inputData,
                type: 'post',
                onComplete: $scope.getStoreDetailsCallback
            };
            utl.Http.doAction(options);
        }
    }
