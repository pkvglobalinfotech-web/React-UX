(function () {
    ...
    angular
        .module('app.pages')
        .controller('appointmentreschedulereportController', appointmentreschedulereportController);

    function appointmentreschedulereportController($scope, $stateParams, $state, $translate, $filter, utl) {
        var vm = this;
        ...
        $scope.excelDownloadCallbackExcel = function (scope, data, options, hasError) {
            const JsonFields = ["Patient Name", "MRN", "Age", "Gender", "Schedule Date", "Remark", "Doctor Name", "ReSchedule Date", "Appointment Status"]
            ...
            data.Data.forEach(function (rowArray) {
                ...
                if (rowArray.UpdatedAt) {
                    // reSchedule = rowArray.UpdatedAt;
                    reSchedule = utl.Formatter.getDateTimeString(rowArray.UpdatedAt);
                }
                if (rowArray.CreatedAt) {
                    // reSchedule = rowArray.CreatedAt;
                    reSchedule = utl.Formatter.getDateTimeString(rowArray.CreatedAt);
                }
                if (rowArray.AppointmentStatus.Description) {
                    appointment = rowArray.AppointmentStatus.Description;
                }
                csvContent += patName + ',' + mrn + ',' + age + ',' + gender + ',' + schedule + ',' + remark + ',' + docName + ',' + reSchedule + ',' + appointment + "\n";
            });
            var encodedUri = encodeURI(csvContent);
            var hiddenElement = document.createElement('a');
            hiddenElement.href = 'data:text/csv;charset=utf-8,' + encodedUri;
            hiddenElement.target = '_blank';
            hiddenElement.download = 'appointmentreschedule-report.csv';
            hiddenElement.click();
        ...
        $scope.print = function () {
            ...
            var options = {
                action: 'appointment/Appointment/PrintAppointmentReScheduleReport',
                data: inputData,
                type: 'post'
            };
        ...
        vm.gridConfig = {
            ...
            {
                field: "UpdatedAt",
                displayName: $translate.instant('ReSchedule Date'),
                cellTemplate: "<div class='ui-grid-cell-contents'><span >{{entity.UpdatedAt | date : 'dd-MMM-yyyy'}} </span>" + "<span >{{entity.CreatedAt| date: 'HH:mm'}}</span>" + "</div>"
            },
            {
    ...
    appointmentreschedulereportController.$inject = ['$scope', '$stateParams', '$state', '$translate', '$filter', 'utl'];
