(function () {
    ...
    angular
        .module('app.pages')
        .controller('appointmentfromcalFormController', appointmentfromcalFormController);

    function appointmentfromcalFormController($scope, $stateParams, $state, $translate, utl, $timeout, $filter, $uibModalInstance, modalConfig) {
        var vm = this;
        ...
        // }
        console.log($stateParams, '$scope.item55');
        $scope.AppointmentId = '';
        $scope.newVisit = 0;
        ...
        if (modalConfig && modalConfig.params) {
            ...
            $scope.currentcontext.isreschedule = modalConfig.params.isreschedule;
            $scope.event = modalConfig.params.event;
        ...
        if ($scope.event && $scope.event.AppointmentDate) {
            $scope.item.AppointmentDate = $scope.event.AppointmentDate;
        }
        ...
        $scope.onDoctorSelected = function (data) {
            ...
            $scope.getdepartment();
            $scope.getDocTariffDetails(data);
            console.log(data, '$scope.item44');
            $scope.item.deptid = data[0].DepartmentId;
        }
        $scope.getdeptCallback = function (scope, data, options, hasError) {
            $scope.item.map = data;
            var dept = [];
            $scope.item.DepartmentId = $scope.item.deptid;
            // for (var idx in data) {
            //     dept.push(data[idx])
        ...
        $scope.checkOut = function () {
            ...
            var options = {
                ...
                type: 'post',
                onComplete: $scope.confirmcheckout,
            };
            utl.Http.doAction(options);
        }
        ...
        $scope.getItemCallback = function (scope, data, options, hasError) {
            ...
            console.log(data, '$scope.item33');
            if ($scope.item.Patient) {
                $scope.item.MRNTypeId = $scope.item.Patient.MRNTypeId;
            }
            ...
            if ($scope.item.ReferralId) {
                $scope.item.ReferrerId = $scope.item.ReferralId;
                $scope.item.ReferTypeId = $scope.item.ReferralTypeId;
            }
        ...
        $scope.getDefaultReferralCallback = function (scope, res, options, hasError) {
            if (res && res.Data && res.Data.length > 0) {
                ...
                // $scope.item.ReferralName = refObj.Text;
                $scope.item.ReferralName = (refObj.ReferralName) ? refObj.ReferralName : '';
        ...
        $scope.validateForm = function () {
            ...
            // }
            if ($scope.item.MRNTypeId == 1 && $scope.item.AppointmentStatusId == 6) { //checkedin
                // isValid = false;
                // utl.Alert.showErrorMsg($translate.instant('appointment.appointment-form.temp-patient-valid-msg.lbl'));
                // var msgKey = "Do you Want to Create Permanent ID for this Patient";
                // var confirmOptions = {
                //     headingKey: 'common.confirm-modal-header.lbl',
                //     messageKey: msgKey,
                //     onSuccessMethod: $scope.updatePatientMRN(),
                // };
                // utl.Dialog.confirmMessage(confirmOptions);
                // return;
            } else {
        ...
        $scope.checkIn = function () {
            ...
            if ($scope.item.MRNTypeId == 1 && $scope.item.AppointmentStatusId == 6) { //checkedin
                // isValid = false;
                // utl.Alert.showErrorMsg($translate.instant('appointment.appointment-form.temp-patient-valid-msg.lbl'));
                var msgKey = "Do you Want to Create Permanent ID for this Patient and Checkin";
                var confirmOptions = {
                    headingKey: 'common.confirm-modal-header.lbl',
                    messageKey: msgKey,
                    itemId: $scope.item.PatientId,
                    onSuccessMethod: $scope.updatePatientMRN,
                };
                utl.Dialog.confirmMessage(confirmOptions);
                return;
        ...
        $scope.saveItem = function (validateDuplicateEncounter) {
            ...
            $scope.item.AddressLine1 = $scope.newPatient.AddressLine1;
        ...
        $scope.saveItem1 = function (validateDuplicateEncounter) {
            ...
            console.log($scope.currentcontext.isnewpatient);
            console.log($scope.item, '$scope.item2');
            $scope.item.AssignedUserId = $scope.item.DoctorId;
            $scope.item.AssignedUserName = $scope.item.DoctorName;
            $scope.item.ReferralId = $scope.item.ReferrerId;
            // $scope.selectedPatient.AddressLine1 = $scope.item.AddressLine1;
        ...
        $scope.confirmcheckout = function () {
            ...
            $scope.Checkout.AppointmentId = $scope.item.Id;
        ...
        function savePatient() {
            ...
            var inputData = {
                ...
                DistrictId: $scope.item.DistrictId,
        ...
        $scope.updatePatient = function () {
            var inputData = {
                ...
                Title: $scope.selectedPatient.Title.Description,
                ...
                CreatedUser: $scope.selectedPatient.CreatedUser,
            };
        ...
        function updatePatientMrnCallback(scope, data, options, hasError) {
            // utl.Alert.showSuccessMsg($translate.instant('common.successmsg.lbl'));
            // $scope.backToList();
            // $scope.item.PatientId = data;
            // $scope.currentcontext.isnewpatient = false;
        ...
        $scope.updatePatientMRN = function () {
            ...
            var options = {
                ...
                type: 'post',
                onComplete: updatePatientMrnCallback
            };
            utl.Http.doAction(options);
        };
        //Add guarantors
        ...
        $scope.getListCallback = function (scope, res, options, hasError) {
            ...
            console.log($scope.appointmentList, '$scope.appointmentList');
            ...
            if ($scope.appointmentSessionList.length > 0) {
                ...
                if (apptSession) {
                    ...
                    try {
                        ...
                        console.log(issameoraft, 'issameoraft');
                        console.log(issameorbef, 'issameorbef');
                        if (issameoraft && issameorbef) {
    ...
    appointmentfromcalFormController.$inject = ['$scope', '$stateParams', '$state', '$translate', 'utl', '$timeout', '$filter', '$uibModalInstance', 'modalConfig',];
