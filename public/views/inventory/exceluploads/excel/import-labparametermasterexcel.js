...
(function () {
    ...
    angular
        .module('app.pages')
        .controller('importLabParameterMasterExcelController', importLabParameterMasterExcelController);

    function importLabParameterMasterExcelController($scope, $stateParams, $state, $translate, utl, $uibModalInstance, modalConfig, $filter) {
        var vm = this;
        $scope.Items = [];
        $scope.Item = [];
        ...
        $scope.download = function () {
            ...
            const JsonFields = ["Code", "Name", "Mnemonics", "AnalyteType","UOM",
                "Methodology", "Sampletype", "AnalyteValueType", "IsActive"]
            ...
            hiddenElement.download = 'Lab Parameter Master Bulk Upload.csv';
            hiddenElement.click();
        ...
        $scope.save = function (inputData) {
            ...
            for (var idx in inputData) {
                ...
                if (data.AnalyteType) {
                    let sublen = $scope.lookup.ANALYTETYPE.length;
                    for (var i = 0; i < sublen; i++) {
                        if (convertToLowerCase($scope.lookup.ANALYTETYPE[i].Text) === convertToLowerCase(data.AnalyteType)) {
                            item.AnalyteTypeId = $scope.lookup.ANALYTETYPE[i].Id;
                            break;
                        }
                    }
                }
                ...
                if (data.AnalyteValueType) {
                    let sublen = $scope.lookup.ANALYTEVALUETYPE.length;
                    for (var i = 0; i < sublen; i++) {
                        if (convertToLowerCase($scope.lookup.ANALYTEVALUETYPE[i].Text) === convertToLowerCase(data.AnalyteValueType)) {
                            item.Valuetype_e = $scope.lookup.ANALYTEVALUETYPE[i].Id;
                            break;
                        }
                    }
                }
            ...
            $scope.loading = true;
            var options = {
                action: 'lis/analytemaster/AddAnalyteMasterExcel',
                data: {
                    Data: result
                },
                type: 'post',
    ...
    importLabParameterMasterExcelController.$inject = ['$scope', '$stateParams', '$state', '$translate', 'utl', '$uibModalInstance', 'modalConfig', '$timeout', '$filter'];
