(function () {
    ...
    angular
        .module('app.pages')
        .controller('pharmacybillmodifyformController', pharmacybillmodifyformController);

    function pharmacybillmodifyformController($rootScope, $scope, $stateParams, $state, $translate, utl, $filter, $timeout) {
        var vm = this;
        ...
        $scope.CalculateNetAmt = function () {
            ...
            if (parseFloat(($scope.item.TotDiscAmount).toFixed(2)) > parseFloat(($scope.item.GrossAmount).toFixed(2)) /* || $scope.currentcontext.TotBalanceAmt < 0*/) {
                ...
                $scope.currentcontext.BillDiscount = 0;
                $scope.currentcontext.TotNetAmount = parseFloat(parseFloat($scope.item.GrossAmount) + parseFloat($scope.currentfilter.RoundOffValue));
            }
        ...
        $scope.editPatientBillDetails = function () {
            utl.Modal.openFixedDialog('app.pharmacybillmodifydetails', {
                params: {
                    ...
                    TotInGstAmount: $scope.item.InGstAmount,
                    TotCGstAmount: $scope.item.CGstAmount,
                    TotSGstAmount: $scope.item.SGstAmount,
                },
            ...
            if ($scope.PatientBillInfo && $scope.PatientBillInfo.length > 0) {
                ...
                $scope.PatientBillInfo.forEach(patientbills => {
                    ...
                    if (patientbills.PharmacySaleTypeId == 4) {
                        ...
                        $scope.currentfilter.RoundOffValue = patientbills.RoundOffValue;
    ...
    pharmacybillmodifyformController.$inject = ['$rootScope', '$scope', '$stateParams', '$state', '$translate', 'utl', '$filter', '$timeout'];
