(function () {
    ...
    angular
        .module('app.pages')
        .controller('advancereceiptFormController', advancereceiptFormController);

    function advancereceiptFormController($scope, $filter, $stateParams, $state, $translate, utl) {
        var vm = this;
                ...
                */
                ...
                if ($scope.item.ReceiptStatusId) {
                    ...
                    } else if ($scope.item.ReceiptStatusId == 3) // Cancelled && not Bill Consumed
                    {
                        ...
                        $scope.canShowDeleteBtn = false;
                        $scope.canShowSaveandApproveBtn = false;
                    } else if ($scope.item.ReceiptStatusId == 4) // Refunded
                    {
                        $scope.isSaving = true;
                        $scope.canShowAddNewBtn = false;
                        $scope.canShowClearBtn = false;
        ...
        $scope.getBilldata = function (scope, data, options, hasError) {
            ...
            $scope.item = data;
            $scope.item.ReceiptTypeId = 7; //Pharmacy Advance
        ...
        $scope.onCancelConfirmed = function () {
            ...
            if ($scope.item.ReceiptDateTime) {
                var actionName = 'AppManager/FacilityPreference/ReceiptCancelFromBillSettings';
                var options = {
                    action: actionName,
                    data: {
                        Data: {
        ...
        $scope.getVisitIndentifier = function (scope, data, options, hasError) {
            if (data && data.Data.length > 0) {
                ...
                //$scope.item.GuarantorName = $scope.encounterInfo.PatientGuarantor.Guarantor.GuarantorName;
                //$scope.item.GuarantorScheme = $scope.encounterInfo.GuarantorScheme.PromotionSchemeName;
            }
        };
    ...
    advancereceiptFormController.$inject = ['$scope', '$filter', '$stateParams', '$state', '$translate', 'utl'];
