...
// the "Approx. Age" multi-input group below -- these aren't a single Input/Select, so
// they can't take the design system's own built-in label. Matches Input/Select's own
// label styling (typography.label token) so everything still lines up visually.
const groupLabelStyle: React.CSSProperties = {
  ...typography.label, color: colors.textMain, marginBottom: spacing.xs, fontFamily: typography.fontFamily, display: 'block',
};

// The Title <select> below is kept as a raw, token-restyled native element rather than
// the design-system Select component: Select (src/components/ui/Select.tsx) is a plain
// function component that does not forward a ref to its underlying DOM node, but this
// screen's real setFocusTitle()/callTitleFocus() behavior (see the effect below) depends
// on calling .focus() on that exact node. Rebuilding Select to add ref-forwarding is out
// of scope for a visual-only retrofit, so this one field is restyled-in-place only --
// same native <select ref=.../>, same focus effect, same onChange/dispatch, just tokens
...
// Patient Registration Form -- the edit/view screen reached from
// patientregistration-self's edit-row navigation (app.patientregistration-form).
// Unlike newregistration (a standalone kiosk-style page), this is a partial rendered
// inside the app shell, so there is no header/logout bar here -- the shell provides
// its own.
export const PatientRegistrationFormScreen: React.FC<PatientRegistrationFormScreenProps> = ({ reactProps, onAction }) => {
  ...
  // Mirrors setFocusTitle()/callTitleFocus(): for a brand-new patient (no id yet),
  // the Title control gets focus shortly after the form appears.
  useEffect(() => {
    if (!currentcontext.id || currentcontext.id <= 0) {
  ...
  // Note: the original template's file input has a dangling ng-change="photoFileChanged()"
  // -- that function is never defined on $scope in patientregistration-form.js, so it is
  // already a real no-op today. We faithfully do not invent one; only the real ng-model
  // effect (currentcontext.file) is preserved via the 'fileSelected' dispatch below.
  ...
  // original gmPlacesAutocomplete::placeChanged handler in patientregistration-form.js
  // exactly (including the same route/locality->city/area swap quirk).
  useEffect(() => {
  ...
  // The template's only wired save action is "Save & Activate" (saveAndApprove) --
  // there is no separate plain "Save as Draft" button here (unlike newregistration).
  const handleSaveAndApprove = () => {
    ...
    dispatch('saveAndApprove', payload);
  };
  ...
  // then a single default placeholder. Unlike newregistration, this template has no
  // gender-conditional default image -- that real difference is preserved as-is.
  ...
  return (
    <div style={{ maxWidth: 1100, margin: '0 auto', padding: `${spacing.lg} ${spacing.xl} 100px` }}>
      <PageHeader title="Patient Registration Form" />
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: spacing.lg }}>
        ...
        <Card title="Personal Details">
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: spacing.lg }}>
            <div>
              <label style={groupLabelStyle}>Title <span style={{ color: colors.danger, marginLeft: 2 }}>*</span></label>
              <select id="title" ref={titleSelectRef} style={titleSelectStyle} value={item.TitleId ?? ''}
                onChange={(e) => dispatch('titleChange', { value: e.target.value ? parseInt(e.target.value, 10) : null })}>
                <option value="">Select</option>
                {lookups.Title.map((t) => <option key={t.Id} value={t.Id}>{t.Text}</option>)}
              </select>
            </div>
            ...
            {canShowApproxAge && (
              <div>
                ...
                <div style={{ display: 'flex', gap: spacing.sm }}>
                  ...
                  {/* NOTE: this "Years" input and the standalone Age field below both map
                      to the same value (item.Age) and the same dispatch('ageChange', ...)
                      -- a pre-existing duplicate-field quirk from the original template,
                      preserved exactly as-is by this visual-only retrofit, not deduped. */}
      ...
      <div style={{
        ...
        borderTop: `1px solid ${colors.border}`, padding: `${spacing.md} ${spacing.xl}`, display: 'flex', justifyContent: 'space-between',
      }}>
        <Button variant="secondary" icon="fa-angle-left" onClick={() => dispatch('backToList')}>Back</Button>
        <div style={{ display: 'flex', gap: spacing.md }}>
          <Button variant="success" onClick={handleSaveAndApprove}>Save &amp; Activate</Button>
          {/* "Reject" is present but unwired in the live Angular template today (no
              ng-click) -- preserved as a real no-op rather than inventing behavior. */}
