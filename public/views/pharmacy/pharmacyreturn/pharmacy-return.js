(function () {
    ...
    angular
        .module('app.pages')
        .controller('pharmacyreturnController', pharmacyreturnController);

    function pharmacyreturnController($rootScope, $scope, $interval, $stateParams, $state, $translate, utl, $filter, $timeout) {
        var vm = this;
        ...
        $scope.saleandreturnprint = function () {
            ...
            var actionName = 'billing/patientbills/PrintAllPharmacybilldetails';
            var options = {
                action: actionName,
                data: inputData,
                type: 'post'
        ...
        $scope.print = function () {
            if ($scope.printpreferences != 1) {
                ...
                var actionName = 'billing/patientreturns/PrintPatientReturns';
                if (window.printcode.toLowerCase() == 'bewell') {
                    var actionName = 'billing/patientreturns/PrintPatientReturns1';
                }
                var options = {
                    action: actionName,
                    data: inputData,
        ...
        $scope.getReturnInfoCallback = function (scope, res, options, hasError) {
            ...
            if ($scope.PatientReturnInfo && $scope.PatientReturnInfo.length > 0) {
                $scope.PatientReturnInfo.forEach(patientreturns => {
                    ...
                    if (patientreturns.PatientRefunds[0]) {
                        $scope.refundId = patientreturns.PatientRefunds[0].Id;
                    }
                    console.log($scope.refundId, '$scope.refundId');
        ...
        $scope.printref = function () {
            ...
            var actionName = 'Billing/PatientRefund/PrintPatientRefund';
            if (window.printcode.toLowerCase() == 'bewell') {
                var actionName = 'Billing/PatientRefund/PrintPatientRefund';
            }
            var options = {
        ...
        function loadSelectedList(selectedList) {
            ...
            $scope.selectedPatient = selectedList.PatientInfo;
            $scope.item.PatientBills = selectedList.PatientBills;
            $scope.PatientReturnDetails = [];
            if (selectedList.ReturnData && selectedList.ReturnData.length > 0) {
                selectedList.ReturnData.forEach(SelectedReturn => {
                    ...
                    $scope.item.PharmacyReturnTypeId = 1;
                    $scope.item.PatientName = $scope.selectedPatient.PatientName;
                    $scope.item.MobileNo = $scope.selectedPatient.Mobile;
        ...
        $scope.saveItem = function (StatusId) {
            ...
            $scope.item.ReturnDateTime = utl.Formatter.getCurrentDate();
            if (!$scope.item.PatientId || $scope.item.PatientId == -1) {
    ...
    pharmacyreturnController.$inject = ['$rootScope', '$scope', '$interval', '$stateParams', '$state', '$translate', 'utl', '$filter', '$timeout'];
