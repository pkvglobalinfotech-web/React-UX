(function () {
    ...
    angular
        .module('app.pages')
        .controller('FollowupAppointmentsController', FollowupAppointmentsController);

    function FollowupAppointmentsController($scope, $stateParams, $state, $translate, $filter, utl, uibButtonConfig, $timeout) {
        var vm = this;
        ...
        $scope.getListCallback = function (scope, res, options, hasError) {
            ...
            for (var idx in res.Data) {
                ...
                if (item.Appointment.PatientTrackers) {
                    if (item.Appointment.PatientTrackers.length > 0)
                        item.Tracker = item.Appointment.PatientTrackers[0];
                }
                if (item.Patient) {
        ...
        $scope.getList = function () {
            ...
            if ($scope.currentfilter.consultationstatusid > 0) {
                inputData.Params.push({
                    Key: 4,
                    Value: $scope.currentfilter.consultationstatusid
                })
        ...
        $scope.followup = function (entity) {
            utl.Modal.open('app.patienttracker', {
                params: {
                    ...
                    tracker: (entity.Tracker) ? entity.Tracker : '',
                    //assignto: 1
                },
        ...
        $scope.handleEvents = function (actionType, entity) {
            ...
            //         params: {
            //             id: entity.AppointmentId, pid: entity.PatientId, eid: entity.Encounter.Id, doctid: entity.DoctorId,
        ...
        vm.gridConfig = {
            ...
            },
            {
                ...
                <span class="grid-action" ng-click="handleEvents(\'attend\',entity)">\
                <i class="fas fa-hospital-user" uib-tooltip="Follow Up"  aria-hidden="true" tooltip-placement="top"></i></span>\
                </div>',
                handleEvent: $scope.handleEvents,
                actions: []
            }
        ...
        //         /* getPendingOrders({ PatientId: entity.PatientId, EncounterId: entity.Encounter.Id }); */
        //         handleCheckout(entity);
        ...
        $scope.userCallback = function (scope, data, options, hasError) {
            $scope.lookup.Doctor = hasError ? {} : data['Doctor'];
        };
    ...
    FollowupAppointmentsController.$inject = ['$scope', '$stateParams', '$state', '$translate', '$filter', 'utl', 'uibButtonConfig', '$timeout'];
