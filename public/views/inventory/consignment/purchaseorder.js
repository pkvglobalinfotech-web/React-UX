(function () {
    ...
    angular
        .module('app.pages')
        .controller('ConsignmentPOFormController', ConsignmentPOFormController);

    function ConsignmentPOFormController($rootScope, $scope, $stateParams, $state, $translate, utl, $filter, $timeout) {
        var vm = this;
        ...
        $scope.getItemCallback = function (scope, data, options, hasError) {
            ...
            if (data.PoStatusId == 2) {
                ...
                $scope.item.RBDisabled = true;
                $scope.item.DisplayPoStatus = 'Save';
            }
        ...
        $scope.purchaseOrderNumCallback = function (scope, data, options, hasError) {
            ...
            var confirmOptions = {
                ...
                messageKey: 'Your Purchase Order has been Approved!.. Ref No is <strong>' + data.PoNumber + '</strong>',
                okkey: 'Close',
            ...
            utl.Dialog.OKConfirmMessage(confirmOptions);
        ...
        $scope.saveInvoiceItemCallback = function (scope, data, options, hasError) {
            utl.Alert.showSuccessMsg($translate.instant('Invoice Generated'));
            // savehitcompleted = 0;
            // if (data === true) {
        ...
        $scope.generateInvoice = function () {
            ...
            for (var idx in lines) {
                ...
                item.Id = 0;
                item.MasterItem = {
                    CategoryId: item.ItemMaster.CategoryId,
                    ProductTypeId: item.ItemMaster.ProductTypeId
                }
                item.ExpiryDate = item.BatchExpiryDate;
                $scope.grnDetails.push(item);
            }
            $scope.grnItem = $scope.item;
            $scope.grnItem.IsConsignmentPO = 1;
            $scope.grnItem.GrnStatusId = 2;
            $scope.grnItem.GrnTypeId = 2;
            $scope.grnItem.GrnNumber = null;
            if (!$scope.grnItem.RoundOff) {
                $scope.grnItem.RoundOff = 0.0;
            }
            $scope.grnItem.PurchaseOrderId = $scope.currentcontext.id;
            ...
            var inputData = {
                Header: $scope.grnItem,
                Details: $scope.grnDetails
            };
            ...
            var options = {
                ...
                type: 'post',
                onComplete: $scope.saveInvoiceItemCallback,
        ...
        $scope.onMasterItemSelected = function (idx, selectedItem) {
            ...
            selectedItem.Mrp = SelectedMasterItem.UomMrPrice;
    ...
    ConsignmentPOFormController.$inject = ['$rootScope', '$scope', '$stateParams', '$state', '$translate', 'utl', '$filter', '$timeout'];
