(function () {
    ...
    angular
        .module('app.pages')
        .controller('medicineissuesListController', medicineissuesListController);

    function medicineissuesListController($rootScope, $scope, $interval, $stateParams, $state, $translate, utl, $filter, modalConfig, $timeout) {
        var vm = this;
        ...
        $scope.saveItem = function (StatusId) {
            ...
            if ($scope.item.PatientBillStatusId == 1 && dReceiptAmt > 0) {
                ...
                $scope.currentfilter.GuarantorTypeId <= 1 && $scope.item.PrivateDueId <= 0 && (!$scope.item.IsMultiplePayment || $scope.item.IsMultiplePayment == false)) {
            ...
            if (!$scope.PatientPaymentDetails || $scope.PatientPaymentDetails.length === 0) {
                if ($scope.currentcontext.ReceiptAmt > 0) {
                    ...
                    if ($scope.currentcontext.id > 0 && $scope.currentcontext.PaidAmt >= 0) {
    ...
    medicineissuesListController.$inject = ['$rootScope', '$scope', '$interval', '$stateParams', '$state', '$translate', 'utl', '$filter', 'modalConfig', '$timeout'];
