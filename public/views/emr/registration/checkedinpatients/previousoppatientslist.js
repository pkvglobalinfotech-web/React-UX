(function() {
    ...
    angular
        .module('app.pages')
        .controller('PreviousOPPatientsController', PreviousOPPatientsController);

    function PreviousOPPatientsController($scope, $stateParams, $state, $translate, $filter, utl, uibButtonConfig, $timeout) {
        var vm = this;
        ...
        $scope.getList = function() {
            if (!$scope.currentfilter.patientname && !$scope.currentfilter.DoctorId && $scope.currentfilter.DoctorId == -1) {
                if ($scope.currentfilter.visitdate == null) {
                    utl.Alert.showErrorMsg($translate.instant('Please Select Date...'));
                    return false;
                }
            }
            // if (!$scope.currentfilter.DoctorId || $scope.currentfilter.DoctorId == -1) {
            //     if ($scope.currentfilter.visitdate == null) {
            //         utl.Alert.showErrorMsg($translate.instant('Please Select Date...'));
            //         return false;
            //     }
            ...
            if ($scope.currentfilter.DoctorId && $scope.currentfilter.DoctorId > 0) {
                inputData.Params.push({
                    Key: 13,
        ...
        vm.gridConfig = {
            ...
            columnDefs: [{
                ...
                },
                {
                    ...
                    <span class="grid-action" ng-click="handleEvents(\'attend\',entity)" ng-show="entity.EncounterDoctorStatus==1"  uib-tooltip="Attend" tooltip-placement="top">\
                    <i class="fas fa-hospital-user" uib-tooltip="Attend"  aria-hidden="true" tooltip-placement="top"></i></span>\
                    <span class="grid-action" ng-click="handleEvents(\'emr\',entity)" ng-hide="entity.EncounterDoctorStatus==1"  uib-tooltip="EMR" tooltip-placement="top">\
