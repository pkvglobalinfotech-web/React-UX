(function () {
    ...
    angular
        .module('app.pages')
        .controller('appointmentschedulereportController', appointmentschedulereportController);

    function appointmentschedulereportController($scope, $stateParams, $state, $translate, $filter, utl) {
        var vm = this;
        ...
        $scope.excelDownloadCallbackExcel = function (scope, data, options, hasError) {
            const JsonFields = ["Patient Name", "MRN", "Age", "Gender", "Schedule Date", "Remark", "Doctor Name", "Booked Date", "Appointment Status", "Created By"]
            ...
            data.Data.forEach(function (rowArray) {
                ...
                //     schedule = utl.Formatter.getDateTimeString(rowArray.AppointmentDate);
                // }
                if (rowArray.AppointmentDate) {
                    if (rowArray.Patient.Title.Description) {
                        schedule = rowArray.AppointmentDate;;
                ...
                if (rowArray.CreatedAt) {
                    // booked = rowArray.CreatedAt;
                    booked = utl.Formatter.getDateTimeString(rowArray.CreatedAt);
                }
                if (rowArray.AppointmentStatus) {
                ...
                csvContent += patName + ',' + mrn + ',' + age + ',' + gender + ',' + schedule + ',' + remark + ',' + docName + ',' + booked + ',' + appointment + ',' + createdby + "\n";
            });
            var encodedUri = encodeURI(csvContent);
            var hiddenElement = document.createElement('a');
            hiddenElement.href = 'data:text/csv;charset=utf-8,' + encodedUri;
            hiddenElement.target = '_blank';
            hiddenElement.download = 'appointmentschedule-report.csv';
            hiddenElement.click();
        ...
        $scope.excelDownload = function () {
            ...
            if (!$scope.currentfilter.AppointmentStatusId || $scope.currentfilter.AppointmentStatusId == -1) {
                inputData.Params.push({
                    Key: 7,
                    Value: [2, 3, 4]
                })
            }
            if ($scope.currentfilter.AppointmentStatusId > 0) {
                inputData.Params.push({
                    Key: 7,
        ...
        $scope.print = function () {
            ...
            var options = {
                action: 'appointment/Appointment/PrintAppointmentScheduleReport',
                data: inputData,
                type: 'post'
            };
        ...
        $scope.lookupCallback = function (scope, data, options, hasError) {
            forEach(data, function (value, key) {
                ...
                if (key == 'AppointmentStatus') {
                    for (var sx in $scope.lookup.AppointmentStatus) {
                        var apnmnt = $scope.lookup.AppointmentStatus[sx];
                        if (apnmnt.Id == 2 || apnmnt.Id == 3 || apnmnt.Id == 4) {
                            $scope.apnmntlookup.push(apnmnt);
                        }
                    }
                }
            });
    ...
    appointmentschedulereportController.$inject = ['$scope', '$stateParams', '$state', '$translate', '$filter', 'utl'];
