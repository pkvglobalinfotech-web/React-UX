(function () {
    ...
    angular
        .module('app.pages')
        .controller('LabResultReviewController', LabResultReviewController);

    function LabResultReviewController($scope, $filter, $stateParams, $state, $translate, utl) {
        var vm = this;
        $scope.gridData = [];
        $scope.Items = [];
        $scope.currentfilter = {
        ...
        $scope.getList = function (pageNo) {
            ...
            if ($scope.context == 'doctor')
                inputData.Params.push({ Key: 10, Value: $scope.currentfilter.DoctorId });
            var options = {
        ...
        $scope.handleEvents = function (actionType, row) {
            if (actionType == 'review') {
                $state.go('app.labresultreview-form', { id: row.entity.Id, pid: row.entity.PatientId, eid: row.entity.EncounterId });
            }
        };
        vm.gridConfig = {
            columnDefs: [
                { field: "Id", name: 'Lab Result Details', cellTemplate: 'prescriptionTemplate.html' }
            ],
    ...
    LabResultReviewController.$inject = ['$scope', '$filter', '$stateParams', '$state', '$translate', 'utl'];
