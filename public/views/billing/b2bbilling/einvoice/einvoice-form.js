...
      "Info": "Document Test"
 ...
 function randomBytes(count)  {
        ...
        $scope.invoiceCallback = function(scope, data, options, hasError) {
            // $scope.lookup = hasError ? {} : data;
           console.log(data);
        }

        $scope.einvoice_auth = function () {
            // Usage example:

            const a = randomBytes(32);
            console.log(a);
            const bytesString = btoa(String.fromCharCode(...a));

            // 3
            console.log('Bytes to string: ', bytesString)
            console.log(bytesString.length);
            var req = {
                'UserName': 'speaktosatishmh',
                'Password': 'Satish@9044',
                'AppKey': bytesString,
                'ForceRefreshAccessToken': true
            }
            var data = "U2FsdGVkX19mR9qxRNCkIEmq92ZUgyO9/ERCReNbb3iuIz48inNAhw/vEMSylRxQiz6mJOgRW6m0CXGL0rFODeX/2V0285jmo/bXhCSD4bO4mKQGs9xwKJUzoKkiu/7g20v1BviO7AfpcpGE+DvcicUWgu36i84qzVOsqvkR8z24VBvTYyKKu1tS4PRvWkrAwL3SkiS/TAzJmCkP4oyOgw==";
            const inputParams = btoa(req);
            var options = {
                action: 'auth/encryptJSON',
                data: req,
                type: 'post',
                onComplete: $scope.invoiceCallback
            };
            ...
            // var options = {
            //     action: 'https://testapi.mygstcafe.com/eicore/v1.03/Invoice/auth',
            //     data: requestBody,
            //     headers: {
            ...
            // utl.Http.doAction(options);
            // const options = {
            //     url: 'https://testapi.mygstcafe.com/eicore/v1.03/Invoice/auth',
            //     method: "POST",
            //     data: requestBody,
            //     headers: {
            //         'Content-Type': 'application/json',
