(function () {
    ...
    angular
        .module('app.pages')
        .controller('aepatientRegistrationController', aepatientRegistrationController);

    function aepatientRegistrationController($rootScope, $scope, $timeout, $stateParams, $state, $translate, utl, Upload) {
        var vm = this;
        ...
        $scope.isdaycarechanged = function (isdaycare) {


            if (isdaycare == true) {
                if ($scope.opvisit == true) {
                    utl.Alert.showErrorMsg($translate.instant('Please Close Existing Visit'));
                    $scope.ShowDayCarefields = false;
                    $scope.item.IsDayCare = false;
                    return false;
                }
                $scope.ShowDayCarefields = true;
            }
            if (isdaycare == false) {
                $scope.ShowDayCarefields = false;
            }
        ...
        $scope.getPastVisitInfoCallback = function (scope, res, options, hasError) {
            ...
            if ($scope.pastvisitinfo && $scope.pastvisitinfo.length > 0) {
                if ($scope.pastvisitinfo[0].EncounterTypeId == 1 && $scope.pastvisitinfo[0].IsLatest == true) {
                    $scope.opvisit = true;
                } else {
                    $scope.opvisit = false;
                }
        ...
        $scope.getPincodeDataCallback = function (scope, res, options, hasError) {
            if (res.Data) {
                if (res.Data.length > 0) {
                    ...
                    $scope.item.PinCodeId = res.Data[0].PinCodeId;
                }
            }
        };
        ...
        $scope.getPatientCallback = function (scope, res, options, hasError) {
            if (res.Data && res.Data.length > 0) {
                ...
                if ($scope.item.Encounters && $scope.item.Encounters.length > 0) {
                    ...
                    $scope.item.IsDayCare = encounteritem.IsDayCare;
                    if ($scope.item.IsDayCare) {
                        $scope.ShowDayCarefields = true;
                    }
                    $scope.item.WardId = encounteritem.WardId;
                    $scope.item.RoomId = encounteritem.RoomId;
                    $scope.item.BedId = encounteritem.BedId;
        ...
        $scope.PatInfoCallback = function (scope, data, options, hasError) {
            ...
            if ($scope.item.Encounters && $scope.item.Encounters.length > 0) {
                ...
                $scope.item.BedId = encounteritem.BedId;
                $scope.AppointmentId = encounteritem.AppointmentId;
                if (encounteritem.IsMLC) {
                    $scope.item.IsMLC = encounteritem.IsMLC;
                    if ($scope.item.IsMLC == true) {
                        $scope.item.IsMLC = "MLC";
                    }
                    if ($scope.item.IsMLC == false) {
                        $scope.item.IsMLC = " ";
                    }
                }
            ...
            if (vTitleId && $scope.lookup) {
                for (var idx in $scope.lookup.Title) {
                    if (vTitleId == $scope.lookup.Title[idx].Id)
                        if ($scope.lookup.Title[idx].Code && $scope.lookup.Title[idx].Code.toLowerCase() == "babyof")
                            return true;
                }
            }
            return false;
        ...
        $scope.saveAndApprove = function () {
            ...
            if ($scope.item.PatientId > 0) {
                if (!$scope.item.IsDayCare) {
                    msg = 'Do You Want to Update for ' + $scope.item.Title.Description + ' ' + $scope.item.FirstName;
                } else {
                    msg = 'Do You Want to Admit ' + $scope.item.Title.Description + ' ' + $scope.item.FirstName;
                }
            }
            if (!$scope.item.IsDayCare) {
                var confirmOptions = {
                ...
                if (!$scope.item.WardId || !$scope.item.RoomId || !$scope.item.BedId) {
                    utl.Alert.showErrorMsg("Please fill in all DayCare fields.");
                    return;
                }
                var confirmOptions = {
                    headingKey: 'common.confirm-modal-header.lbl',
                    ...
                    yesKey: 'common.yeskey.lbl',
                    noKey: 'common.nokey.lbl',
                    onSuccessMethod: $scope.SaveDaycare,
                };
                utl.Dialog.confirmMessage(confirmOptions);
            }
        };
        ...
        $scope.SaveDaycare = function () {
            ...
            $scope.item.ReferralTypeId = $scope.item.ReferTypeId;
            $scope.item.ReferralId = $scope.item.ReferrerId;
            $scope.item.Id = 0;
            if ($scope.item.IsDayCare == true) {
                $scope.item.ServiceRateCategoryId = $scope.item.ServiceRateCategoryId_;
            }

            var actionName = '';
            if (!$scope.item.PatientId || $scope.item.PatientId == 0) {
                actionName = 'registration/patient/AddDayCarePatient';
            } else {
                actionName = 'registration/patient/UpdateDayCarePatient';
            }
            var options = {
                action: actionName,
                data: {
                ...
                type: 'post',
                onComplete: $scope.savedaycareCallback
            };
            utl.Http.doAction(options);
        };
        // $scope.savedaycareCallback = function(scope, data, options, hasError) {
        //     savehitcompleted = 0;
        //     utl.Alert.showSuccessMsg($translate.instant('common.successmsg.lbl'));
        //     $scope.item.Id = data;
        //     $scope.getPatient();
        ...
        vm.doctorcontrolconfig = {
            ...
            searchparams: {},
            result: {},
            api: 'SystemSettings/User/GetEmergencyUsers',
        ...
        $scope.lookupCallback = function (scope, data, options, hasError) {
            ...
            for (var idx in facilitydata) {
                if (facilitydata[idx].Id > 0) {
                    ...
                    $scope.item.PinCodeId = facilitydata[idx].PinCodeId;
                    break;
                }
            }
        ...
        $scope.wardCallback = function (scope, data, options, hasError) {
            $scope.lookup['Ward'] = hasError ? {} : data.Ward;
            $scope.getRoomLookUp();
        };
        ...
        $scope.RoomCallback = function (scope, data, options, hasError) {
            $scope.lookup['Room'] = hasError ? {} : data.Room;
            $scope.getBedLookUp();
        };
        ...
        $scope.BedCallback = function (scope, data, options, hasError) {
            $scope.lookup['Bed'] = hasError ? {} : data.Bed;
        };
    ...
    aepatientRegistrationController.$inject = ['$rootScope', '$scope', '$timeout', '$stateParams', '$state', '$translate', 'utl', 'Upload'];
