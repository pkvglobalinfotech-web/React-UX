(function () {
    ...
    angular
        .module('app.pages')
        .controller('orderAcknowledgementListController', orderAcknowledgementListController);

    function orderAcknowledgementListController($rootScope, $scope, $filter, $stateParams, $state, $translate, utl, $timeout) {
        var vm = this;
            ...
            for (var idx in res.Data) {
                ...
                if (item.EncounterTypeId == 1) {
                    if (item.BillingId && item.BillingId > 0){
                        if (item.PatientOrderDetails.length > 0){
                            let shouldPush = false;
                            for (var idx in item.PatientOrderDetails) {
                                let details = item.PatientOrderDetails[idx].PatientBillDetail;
                                if(details){
        ...
        $scope.handleEvents = function (actionType, entity) {
            ...
            if (actionType == 'edit') {
                $state.go('app.orderacknowledgementform', {
                    id: entity.Id,
        ...
        if ($scope.showsubdept == 1) {
            var ColDefs = [sno, OrderDate, OrderNumber, mrn, patName, encType, subDept, wardDet, OrdPriority, isExternal, OrdStatus, actions];
        } else {

            var ColDefs = [sno, OrderDate, OrderNumber, mrn, patName, encType, wardDet, OrdPriority, isExternal, OrdStatus, actions];
        }
        ...
        function setDefaults() {
            ...
            var createdStatusId = utl.Lookup.getDefault($scope.lookup.OrderStatus, 'Created');
            var rejectedStautsId = utl.Lookup.getDefault($scope.lookup.OrderStatus, 'Rejected');
            var executedId = utl.Lookup.getDefault($scope.lookup.OrderStatus, 'Executed');
            var partiallycompletedStautsId = utl.Lookup.getDefault($scope.lookup.OrderStatus, 'Partially Done');
            $scope.currentfilter.OrderStatusId = createdStatusId + ',' + rejectedStautsId + ',' + executedId + ',' + partiallycompletedStautsId;
    ...
    orderAcknowledgementListController.$inject = ['$rootScope', '$scope', '$filter', '$stateParams', '$state', '$translate', 'utl', '$timeout'];
