(function () {
    ...
    angular
        .module('app.pages')
        .controller('itemMasterFormController', itemMasterFormController);

    function itemMasterFormController($scope, $filter, $stateParams, $state, $translate, utl, $timeout) {
        var vm = this;
        ...
        $scope.createdBy = '';
        $scope.updatedBy = '';
        $scope.IsBillableDissable = false;
        $scope.item = {
            OrganizationId: 1,
            IsAllFacility: false,
        ...
        $scope.itemcodenoteditable = 0;
        $scope.itemcodenoteditable =
            (utl.FacilitySetting.getFacilitySettingValue('billing', 'itemcodenoteditable')) ? utl.FacilitySetting.getFacilitySettingValue('billing', 'itemcodenoteditable') : 0;


        $scope.getItemCallback = function (scope, data, options, hasError) {
            $scope.item = data;
            $scope.createdByid = $scope.item.CreatedBy;
            $scope.updatedByid = $scope.item.UpdatedBy;
            if ($scope.itemcodenoteditable == true && $scope.currentcontext.id > 0) {

                if ($scope.item.ActiveStatusId == 2) {
                    $scope.item.isDisabled = true;
                }
            }
            $scope.item.ItemDescription = $scope.item.ItemName;
            if ($scope.item.FacilityId == utl.Session.getCurrentFacilityId() || -1) {
            ...
            if ($scope.item.SubCategoryId > 0) {
                ...
                $scope.filtergetSubType();
                $scope.item.ProductTypeId = data.ProductTypeId;
                $scope.item.SubTypeId = data.SubTypeId;
            } else {
            ...
            if ($scope.item.UomConversions) {
                ...
                if (purchaseuom) {
                    ...
                    $scope.item.PurConQty = 1;
                }

            }
            $scope.enablecssd();
            $scope.getUser($scope.createdByid);
            $scope.getUpdatedUser($scope.updatedByid);
        };
        $scope.updateCheckboxes = function () {
            var subCategoryId = $scope.item.SubCategoryId;

            // Set IsBillable and IsConsignment based on SubCategoryId
            switch (subCategoryId) {
                case 1: // IMPLANT
                    $scope.item.IsBillable = true;
                    $scope.item.IsConsignment = false;
                    break;
                case 2: // INVENTORY
                ...
                case 3: // PHARMACY
                    $scope.item.IsBillable = true;
                    $scope.item.IsConsignment = true;
                    break;
                default:
                    // Handle default case or unexpected values
                    $scope.item.IsBillable = false;
                    $scope.item.IsConsignment = false;
                    break;
            }
        ...
        $scope.filtergetSubType = function () {
            // $scope.item.SubTypeId = -1;
            var inputData = [{
                "Key": "ItemSubType",
                Request: {
        ...
        vm.itemcontrolconfig = {
            ...
            searchparams: {},
            result: {},
            api: 'pharmacy/itemmaster/GetItemMasterlist',
        ...
        function formatselecteditem() {
            ...
            if (selectedItem && !utl.Common.isEmptyJSONObject(selectedItem)) {
                ...
                $scope.item.ItemName = result;
                $scope.itemName = selectedItem.ItemName;
                $scope.item.ItemDescription = selectedItem.ItemName;
                // if ($scope.item.UomConversions) {
                //     var purchaseuom = $filter('filter')($scope.item.UomConversions, {
                //         UomTypeId: 2
                //     })[0];
                ...
                result = vm.itemcontrolconfig.rowdata.ItemName;
                $scope.item.ItemName = result;
                $scope.item.ItemDescription = result;
                $scope.itemName = vm.itemcontrolconfig.rowdata.ItemName;
            }
            return result;
            $scope.getList();
        }
        ...
        $scope.saveItem = function () {
            ...
            if ($scope.item.IsBillable === true) {
                if ($scope.item.MrPrice == 0) {
                    utl.Alert.showErrorMsg($translate.instant('MRP should not be 0'));
                    return false;
                }
                if (parseFloat($scope.item.MrPrice) < parseFloat($scope.item.ItemPrice)) {
                    utl.Alert.showErrorMsg($translate.instant('MRP should not be less than Item Price'));
                    return false;
                }
            }
            ...
            if ($scope.currentcontext.id && $scope.currentcontext.id > 0) {
                ...
                if ($scope.item.ItemMasterId && $scope.item.ItemMasterId > 0) {
                    // utl.Alert.showErrorMsg($scope.item.ItemName + ' already Exists..')
                    // return;
                    $scope.item.ItemName = $scope.item.ItemDescription;
                    actionName = 'pharmacy/itemmaster/UpdateItemMaster';
                } else {
                    $scope.item.ItemMasterId = 0;
                    $scope.item.ItemName = $scope.itemName;
                    $scope.item.ItemCode = $scope.itemCode;
                    $scope.item.ItemDescription = $scope.itemName;
                    // $scope.item.PurConQty = 1;
                    $scope.item.VEDId = 0;
                    $scope.item.Id = 0;
                }
        ...
        $scope.getDrugs = function (SelectedItem) {
            if ($scope.item) {
                // $scope.Drugs = $scope.item;
                // if ($scope.Drugs) {
        ...
        $scope.getFacilityCallback = function (scope, res, options, hasError) {
            ...
            $scope.item.IsShowItemType = $scope.facilityitem[0].IsShowItemType;
            $scope.item.IsItemmasterDrugshow = $scope.facilityitem[0].IsItemmasterDrugshow;
            $scope.item.IsItemCodeEdit = $scope.facilityitem[0].IsItemCodeEdit;
        };
        ...
        $scope.getMaxIdCallback = function (scope, data, options, hasError) {
            console.log(data);
            $scope.item.PrevItem = data;
            $state.params.PrevItem = data;
        };
        ...
        $scope.addcategory = function () {
            utl.Modal.open('app.itemcategory', {
                params: {
                    id: 0
                },
                confirmCallback: $scope.initLookup
        ...
        $scope.addsubtype = function () {
            utl.Modal.open('app.itemsubtype', {
                params: {
                    id: 0
                },
                confirmCallback: $scope.initLookup
        ...
        $scope.addsubcategory = function () {
            utl.Modal.open('app.itemsubcategory', {
                params: {
                    id: 0
                },
                confirmCallback: $scope.initLookup
        ...
        $scope.addproducttype = function () {
            utl.Modal.open('app.producttype', {
                params: {
                    id: 0
                },
                confirmCallback: $scope.initLookup
        ...
        $scope.IsBatchMandatory = function () {
            if ($scope.item.IsBatchMandatory == true) {
        ...
        $scope.lookupCallbackOnSelect = function (scope, data, options, hasError) {
            ...
            //     if ($scope.item.SubCategoryId == 1 || $scope.item.SubCategoryId == 2) {
            //         $scope.item.IsBillable = true;
            //     } else {
            //         $scope.item.IsBillable = false;
            //     }
        ...
        $scope.getSubType = function () {
            ...
            if ($scope.item.SubCategoryId == 1) {
                $scope.item.IsBillable = true;
                $scope.item.IsConsignment = false;
                // $scope.IsBillableDissable = false;
            } else if ($scope.item.SubCategoryId == 2) {
                $scope.item.IsBillable = false;
                $scope.item.IsConsignment = false;
                // $scope.IsBillableDissable = true;
            } else if ($scope.item.SubCategoryId == 3) {
                $scope.item.IsBillable = true;
                $scope.item.IsConsignment = true;
                // $scope.IsBillableDissable = true;
            }
        ...
        $scope.getUserCallback = function (scope, res, options, hasError) {
            ...
            if (userdata[0].Title.Description != null) {
                var title = userdata[0].Title.Description;
            } else {
                var updatedtitle = "";
            }
            if (userdata[0].FirstName != null) {
                var createdfirstname = userdata[0].FirstName;
            } else {
                var createdfirstname = "";
            }
            if (userdata[0].LastName != null) {
                var createdlastname = userdata[0].LastName;
            } else {
                var createdlastname = "";
            }
            $scope.createdBy = title + createdfirstname + " " + createdlastname;
        };
        ...
        $scope.getUpdatedUserCallback = function (scope, res, options, hasError) {
            var userUpdateddata = res.Data;
            if (userUpdateddata[0].Title.Description != null) {
                var updatedtitle = userUpdateddata[0].Title.Description;
            } else {
                var updatedtitle = "";
            }
            if (userUpdateddata[0].FirstName != null) {
                var updatedfirstname = userUpdateddata[0].FirstName;
            } else {
                var updatedfirstname = "";
            }
            if (userUpdateddata[0].LastName != null) {
                var updatedlastname = userUpdateddata[0].LastName;
            } else {
                var updatedlastname = "";
            }
            $scope.updatedBy = updatedtitle + updatedfirstname + " " + updatedlastname;
        };

        $scope.getUpdatedUser = function () {

            var inputData = {
                Params: [{
                    Key: 0,
                    Value: $scope.updatedByid
                }]
            };
            ...
            var options = {
                ...
                data: inputData,
                type: 'post',
                onComplete: $scope.getUpdatedUserCallback
            };
    ...
    itemMasterFormController.$inject = ['$scope', '$filter', '$stateParams', '$state', '$translate', 'utl', '$timeout'];
