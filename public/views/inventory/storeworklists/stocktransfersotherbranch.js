(function () {
    ...
    angular
        .module('app.pages')
        .controller('StockTransferlistsotherbranchController', StockTransferlistsotherbranchController);

    function StockTransferlistsotherbranchController($rootScope, $scope, $filter, $stateParams, $state, $translate, utl, $timeout) {
        var vm = this;
        ...
        $scope.getstore = function () {
            $scope.currentfilter.ToStoreMasterId = -1;
            var inputData = [{
                "Key": "ToStore",
                Request: {
                    Params: [
        ...
        $scope.getList = function (pageNo) {
            ...
            var inputData = {
                Params: [
                    ...
                    { Key: 18, Value: $scope.currentfilter.TransferStatusId },
                    //{ Key: 4, Value: [fromDate, toDate] },
                    { Key: 5, Value: $scope.currentfilter.FacilityId },
                    { Key: 17, Value: $scope.currentfilter.ToFacilityId },
        ...
        $scope.handleEvents = function (actionType, entity) {
            if (actionType == 'edit') {
                $state.go('app.stocktransferotherbranchform', { id: entity.Id });
            } else if (actionType == 'delete') {
                utl.Dialog.confirmDelete($scope.onDeleteConfirmed, entity.Id, entity.TransferIdentifier);
            } else if (actionType == 'view') {
                $state.go('app.stocktransferotherbranch-view', { id: entity.Id, StockTransferId: entity.Id, TransferStatusId: entity.TransferStatusId });
            }
        }
    ...
    StockTransferlistsotherbranchController.$inject = ['$rootScope', '$scope', '$filter', '$stateParams', '$state', '$translate', 'utl', '$timeout'];
