(function () {
    ...
    angular
        .module('app.pages')
        .controller('StockRequestFormController', StockRequestFormController);

    function StockRequestFormController($rootScope, $scope, $stateParams, $state, $translate, utl, $filter, $timeout) {
        var vm = this;
        ...
        $scope.canShowpartialCancelBtn = false;
        ...
        $scope.currentcontext.CanIndent_Authorize = utl.Privilege.hasAccess('CanIndent_Authorize');
        $scope.currentcontext.CanIndent_Approve = utl.Privilege.hasAccess('CanIndent_Approve');
        $scope.currentcontext.CanIndent_Save = utl.Privilege.hasAccess('CanIndent_Save');
        ...
        $scope.onpartialCancelConfirmed = function () {
            $scope.saveItem();
        };

        $scope.PartialCancelRequest = function () {
            var confirmOptions = {
                headingKey: 'common.confirm-modal-header.lbl',
                messageKey: 'Do you want to partial cancel?',
                yesKey: 'common.yeskey.lbl',
                noKey: 'common.nokey.lbl',
                onSuccessMethod: $scope.onpartialCancelConfirmed,
            };
        ...
        $scope.cancelPatientRequestDetail = function (idx, SelectedItem) {
            if (SelectedItem.ItemMasterId > 0 && SelectedItem.RequestStatusId == 2 || SelectedItem.RequestStatusId == 3) {
                $scope.canShowpartialCancelBtn = true;
                $scope.canShowCancelBtn = false;
            }
        ...
        $scope.getItemCallback = function (scope, data, options, hasError) {
            ...
            if (data.RequestStatusId == 1) {
                ...
                $scope.item.iscancelDisabled = false;
            ...
            if (data.RequestStatusId == 4) {
                $scope.item.isDisabled = true;
                $scope.item.iscancelDisabled = true;
        ...
        $scope.backToList = function () {
            $state.go('app.stockrequests', $scope.currentcontext.id);
        };
        ...
        $scope.cancel = function () {
            if (!$scope.item.Comments) {
                utl.Alert.showErrorMsg('Please give cancel comments');
                return;
            }
            var confirmOptions = {
                headingKey: 'common.confirm-modal-header.lbl',
        ...
        function getLinesForSave() {
            ...
            for (var idx in $scope.stockrequestDetails) {
                var item = $scope.stockrequestDetails[idx];
                item.RequestStatusId = $scope.item.RequestStatusId;
                if (item.IsAllSelected == true) {
                    item.RequestStatusId = 6;
                }
        ...
        $scope.onItemSelected = function (idx, selectedItem) {
            ...
            if (isDuplicate) {
                ...
                $scope.stockrequestDetails.splice(idx, 1)
                $scope.addNewLineItem();
                return;
            }
            ...
            if ($scope.item.AllowOpenRequest) {
                ...
                selectedItem.PurConQty = SelectedMasterItem.ItemMaster.PurConQty;
            ...
            if ($scope.item.StoreMasterId > 0) {
                var inputData = {
                    Params: [
                        ...
                        // { Key: 4, Value: 1 },
                        { Key: 1, Value: $scope.item.ToStoreMasterId },
                        { Key: 13, Value: 2 },
                        // { Key: 15, Value: 1 }
                    ],
                    PageContext: {
                ...
                utl.Alert.showErrorMsg($translate.instant('Please select my store'));
                return false;
            }
        ...
        $scope.lookupCallback = function (scope, data, options, hasError) {
            forEach(data, function (value, key) {
                ...
                if (key == 'ToStore' && $scope.item.ToStoreMasterId === -1) {
                    for (var userstoreid = 0; userstoreid < $scope.lookup['ToStore'].length; userstoreid++) {
                        if ($scope.lookup['ToStore'][userstoreid].IsDefaultWardindentStore == true) {
                            $scope.item.ToStoreMasterId = $scope.lookup['ToStore'][userstoreid].Id;
                            $scope.item.StockRequestTypeId = $scope.lookup['ToStore'][userstoreid].StoreTypeId;
                        }
                    }
                }
            });
    ...
    StockRequestFormController.$inject = ['$rootScope', '$scope', '$stateParams', '$state', '$translate', 'utl', '$filter', '$timeout'];
