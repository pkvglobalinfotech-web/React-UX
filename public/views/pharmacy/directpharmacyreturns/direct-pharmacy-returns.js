(function () {
    ...
    angular
        .module('app.pages')
        .controller('directpharmacyreturnsController', directpharmacyreturnsController);

    function directpharmacyreturnsController($rootScope, $scope, $interval, $stateParams, $state, $translate, utl, $filter, $timeout) {
        var vm = this;
        ...
        $scope.print = function () {
            if ($scope.printpreferences != 1) {
                ...
                var options = {
                    action: 'billing/patientreturns/PrintPatientReturns1',
                    data: inputData,
                    type: 'post'
                };
        ...
        $scope.findReturn = function () {
            $scope.SaveImdDMPrint = 0;
            utl.Modal.open('app.find-direct-pharmacy-returns', {
                params: {
                    id: $scope.currentfilter.PatientId
                },
        ...
        function patientBillPickerCallback(patientbilldata) {
            ...
            if (patientbilldata && patientbilldata.IsCashToCreditBill) {
                ...
                if(patientbilldata.PatientReturns.length > 0) {
                    $scope.currentcontext.id = patientbilldata.PatientReturns[0].Id;
        ...
        $scope.getBillInfoCallback = function (scope, res, options, hasError) {
                ...
                $scope.PatientBillInfo.forEach(patientbills => {
                    ...
                    if ($scope.item.PharmacySaleTypeId == 1) {
                        ...
                        if (patientbills.StoreMaster) {
                            ...
                            $scope.currentfilter.IsStoreSeparateSequence = patientbills.StoreMaster.IsSeqbasedStore;
                        }
                        ...
                        //Implemented In-Patient Return Scenario
                        $scope.currentcontext.isnewpatient = false;
                        ...
                        //Implemented A&E-Patient Return Scenario
                        $scope.currentcontext.isnewpatient = false;
                    ...
                    for (var idx in patientbills.PatientBillDetails) {
                        var billdetails = patientbills.PatientBillDetails[idx];
                        if (patientbills.BillDiscountModeId == 1) {
                            billdetails.DiscountData = billdetails.DiscountAmount;
                        }
                        if (patientbills.BillDiscountModeId == 2) {
                            billdetails.DiscountData = billdetails.DiscountPercentage;
                        }
                        $scope.PatientReturnDetails.push(billdetails);
                    }
            ...
            $('#pid').select();
            $('#pid').focus();
            $scope.patientaddress = res.Data[0].PatientAddress;
            $scope.doctorname = res.Data[0].PatientAddress;
        };
            ...
            if ($scope.PatientReturnInfo && $scope.PatientReturnInfo.length > 0) {
                ...
                $scope.PatientReturnInfo.forEach(patientreturns => {
                    ...
                    if ($scope.item.PharmacyReturnTypeId == 1) {
                        ...
                        $scope.currentfilter.GuarantorId = patientreturns.GuarantorId;
                        ...
                        if ($scope.item.PatientReturnStatusId === 3) {
                            if ($scope.item.IsRefundedFully === true) {
                                ...
                                $scope.item.RefundStatus = 'Partially Refunded';
                                $scope.currentcontext.TotNetAmount = patientreturns.ReturnAmount;
                        ...
                        $scope.newPatient.PatientName = patientreturns.PatientName;
                        //$scope.newPatient.DoctorName = patientbills.DoctorName;
                        //$scope.newPatient.Mobile = patientbills.Mobile;
                        //$scope.newPatient.TitleId = patientbills.TitleId;
                        //$scope.newPatient.GenderId = patientbills.GenderId;
                    ...
                    */
                    if (patientreturns.StoreMaster) {
                        $scope.currentfilter.StoreTypeId = patientreturns.StoreMaster.StoreTypeId;
                        $scope.currentfilter.StoreSubTypeId = patientreturns.StoreMaster.StoreSubTypeId;
                        $scope.currentfilter.SequenceOptionId = patientreturns.StoreMaster.SequenceOptionId;
                        $scope.currentfilter.IsStoreSeparateSequence = patientreturns.StoreMaster.IsSeqbasedStore;
                    }
        ...
        $scope.saveItem = function (StatusId) {
            ...
            $scope.item.DoctorName = $scope.patientaddress;
            $scope.item.PatientAddress = $scope.doctorname;
            ...
            if ($scope.item.PharmacyReturnTypeId == 4) {
            ...
            */
            ...
            if (!$scope.PatientRefunds || $scope.PatientRefunds.length === 0) {
                if ($scope.currentcontext.ReturnAmount > 0) {
        ...
        $scope.AddRefundDetails = function () {
            var PatientRefund = {
                ...
                RefundAmount: $scope.currentcontext.ReturnAmount,
        ...
        function getLinesForSave() {
            ...
            for (var piidx in $scope.PatientReturnDetails) {
                ...
                if (pharmacyitem.ItemMasterId > 0 && parseInt(pharmacyitem.ReturnQuantity) > 0 ) /* && pharmacyitem.Amount > 0 */{
                    ...
                    pharmacyitem.UnitCGstAmount = parseFloat(pharmacyitem.CGstAmount / pharmacyitem.ReturnQuantity).toFixed(2);
                    ...
                    pharmacyitem.UnitSGstAmount = parseFloat(pharmacyitem.SGstAmount / pharmacyitem.ReturnQuantity).toFixed(2);
    ...
    directpharmacyreturnsController.$inject = ['$rootScope', '$scope', '$interval', '$stateParams', '$state', '$translate', 'utl', '$filter', '$timeout'];
