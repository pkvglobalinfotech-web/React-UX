(function () {
    ...
    angular
        .module('app.pages')
        .controller('BedReservationController', BedReservationController);

    function BedReservationController($scope, $stateParams, $state, $translate, utl, $uibModalInstance, modalConfig) {
        var vm = this;
        ...
        $scope.getListCallback = function (scope, data, options, hasError) {
            ...
            for (var idx in data.Data) {
                ...
                if (item.ReleaseStatusId == 1) // Reserved
                    $scope.item = item;
            }
        };
 ...
 var msg = 'bedmaintenance.reservation.lbl';
            if (bedstatus == 1)
        ...
        $scope.saveItem = function (bedstatus) { // bedstatus => Status To bed Changed
            ...
            if (bedstatus == 1) {
                ...
                $scope.item.ReleaseStatusId = 1; // Reserve Bed
            $scope.item.ReserveMaintenanceTypeId = 1; // For Reservation Type
        ...
        vm.gridConfig = {
            columnDefs: [
                {
                    field: "BedReservationType.Description", displayName: $translate.instant('bedreservation.reservationtype.lbl')
                },
                {
                    field: "FromDate", displayName: $translate.instant('bedreservation.fromdate.lbl'),
                    cellTemplate: "<ngformatdate datetime-val='row.entity.FromDate '></ngformatdate>"
                },
                {
                    field: "ToDate", displayName: $translate.instant('bedreservation.todate.lbl'),
                    cellTemplate: "<ngformatdate datetime-val='row.entity.ToDate '></ngformatdate>"
                },
                {
                    field: "Details", displayName: $translate.instant('bedreservation.reservationdetails.lbl')
                },
                {
                    field: "ReservedBy.FirstName", displayName: $translate.instant('bedreservation.reservedby.lbl'),
                ...
                {
                    field: "ReleasedOn", displayName: $translate.instant('bedreservation.releasedon.lbl'),
                    cellTemplate: "<ngformatdate datetime-val='row.entity.ReleasedOn '></ngformatdate>"
                },
                {
                    field: "ReleasedBy.FirstName", displayName: $translate.instant('bedreservation.releasedby.lbl'),
                    cellTemplate: "<div class='ui-grid-cell-contents'>" +
                    '<a ng-click="grid.appScope.handleEvents(\'patientinfo\',row)" uib-tooltip="{{row.entity.Patient.FirstName}}" tooltip-placement="left" >' +
                ...
                { field: "ReleaseStatus.Description", displayName: $translate.instant('bedreservation.status.lbl') },
    ...
    BedReservationController.$inject = ['$scope', '$stateParams', '$state', '$translate', 'utl', '$uibModalInstance', 'modalConfig'];
