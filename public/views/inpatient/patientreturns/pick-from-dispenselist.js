(function () {
    ...
    angular
        .module('app.pages')
        .controller('pickfromdispenseListController', pickfromdispenseListController);

    function pickfromdispenseListController($scope, $stateParams, $state, $translate, utl, $filter, $uibModalInstance, modalConfig) {
        var vm = this;
        ...
        $scope.ChangeReturnQty = function (item) {
            ...
            // }
            // item.ReturnTransitQuantity = 0;
            // if (item.ReturnedQuantity > 0) {
            //     item.ReturnTransitQuantity = (parseInt(item.ReturnedQuantity) + (parseInt(item.ReturnQuantity)));
            // } else {
            //     item.ReturnTransitQuantity = (parseInt(item.ReturnQuantity));
            // }
            if (parseInt(item.ReturnQuantity) > (parseInt(item.Quantity) - (parseInt(item.ReturnedQuantity)))) {
                item.ReturnQuantity = 0;
                utl.Alert.showErrorMsg($translate.instant('pickdispenselist.actualquantity.lbl'));
            }
            ...
            if (res.Data.length > 0) {
                if (res.Data[0].PatientBill) {
                    if (res.Data[0].PatientBill.Patient) {
                        $scope.StoreMasterId = res.Data[0].PatientBill.StoreMasterId;
                    ...
                    for (var gidx in groupedItems) {
                        ...
                        for (var i = 0, len = groupedItem.length; i < len; i++) {
                            ...
                            if (groupedItem[i].IsPharmacySale == 1 && (groupedItem[i].PharmacySaleTypeId == 2 || groupedItem[i].PharmacySaleTypeId == 6)) {
                                ...
                                G_StoreName = groupedItem[i].StoreMaster.StoreName;
                            }
                        ...
                        var PatientBillDetail = {
                            ...
                            Status: 1,
                            StoreName: G_StoreName
                        }
                        // if (PatientBillDetail.Quantity > 0) {
                        //     $scope.PatientBillDetails.push(PatientBillDetail);
                        // }
                        if (PatientBillDetail.Quantity > 0 && PatientBillDetail.ReturnedQuantity != PatientBillDetail.Quantity) {
                            $scope.PatientBillDetails.push(PatientBillDetail);
                        }
                    }
                }
        ...
        //                 G_ItemCode = groupedItem[i].ItemCode;
        //                 G_IsSupplementary = groupedItem[i].IsSupplementary;
        //                 if (groupedItem[i].IsPharmacySale == 1 || groupedItem[i].IsPharmacySale == true && groupedItem[i].PharmacySaleTypeId == 2) {
        //                     itemqty = groupedItem[i].Quantity;
        ...
        $scope.stockreturnListCallback = function (scope, res, options, hasError) {

            $scope.PatientStockReturnDetails = res.Data || [];
            $scope.getPatientDispensedItems();
        };
        $scope.getPatientStockReturnDetails = function () {
            var inputData = {
                Params: [{
                    Key: 8,
                    Value: $scope.currentcontext.EncounterId
            ...
            var options = {
                ...
                data: inputData,
                type: 'post',
                onComplete: $scope.stockreturnListCallback
            };
        ...
        function loadData() {
            if ($scope.currentcontext.EncounterId > 0) {
                // $scope.getPatientStockReturnDetails();
                $scope.getPatientDispensedItems();
            }
        }
    ...
    pickfromdispenseListController.$inject = ['$scope', '$stateParams', '$state', '$translate', 'utl', '$filter', '$uibModalInstance', 'modalConfig'];
