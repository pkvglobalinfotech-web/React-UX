(function () {
    ...
    function purchaseOrderFormController($rootScope, $scope, $stateParams, $state, $translate, utl, $filter, $timeout, Upload) {
        var vm = this;
        ...
        $scope.toggle = false;
        // Excel Upload
        function poexceldata(data) {
            console.log('PO excel data', data);
            let poData = data.result;
            $scope.item.StoreMasterId = poData.StoreId;
            $scope.item.VendorFacilityMapId = poData.SupplierId;
            $scope.item.IsOpenPO = poData.IsOpenPO;
            // $scope.item.VendorMasterId = poData.SupplierId;
            // $scope.purchaseorderDetails = poData.Items || [];
        ...
        $scope.getStoreDetailsCallback = function (scope, data, options, hasError) {
            ...
            $scope.item.Email = $scope.exStoreData.StoreMaster.Email;
            $scope.item.Password = $scope.exStoreData.StoreMaster.Password;
            $scope.item.IsOpenPO = $scope.exStoreData.StoreMaster.CanAllowOpenPO;
        ...
        $scope.poUpload = function () {
            utl.Modal.open('app.importpoexcel', {
                params: {
                    id: 0
                },
                confirmCallback: poexceldata
            });
        };
        $scope.getItemByExcelcallback = function (scope, res, options, hasError) {
            ...
            for (var idx in $scope.purchaseorderDetails) {
                var poitem = $scope.purchaseorderDetails[idx];
                // poitem.UomMrPrice = poitem.MrPrice;
                // poitem.MrPrice = poitem.MrPrice;
                poitem.Id = 0;
                poitem.ConversionQuantity = poitem.UomConversions[0].ConversionQuantity;
                poitem.GstId = poitem.GstMaster.Id;
                if ($scope.ShowMrp) {
                    poitem.CanNotEditUomMrPrice = true;
                    poitem.CanEditUomMrPrice = false;
                } else {
            ...
            if (Array.isArray($scope.ExcelItems) && $scope.ExcelItems.length > 0) {
                $scope.purchaseorderDetails.forEach(poItem => {
                    let matchedExcelItem = $scope.ExcelItems.find(excelItem => excelItem.ItemCode === poItem.ItemCode);
        
                    if (matchedExcelItem) {
                        poItem.PoQuantity = matchedExcelItem.PoQuantity || 0;
                        poItem.FreeQty = matchedExcelItem.FreeQty || 0;
                        poItem.UomPrice = matchedExcelItem.PrPriceUom || 0;
                        poItem.UomMrPrice = matchedExcelItem.MRPUom || 0;
                        poItem.MrPrice = matchedExcelItem.MRPUom || 0;
                        poItem.DiscountMode1Id = matchedExcelItem.DiscountModeId1 || 0;
                        poItem.Discount1 = matchedExcelItem.Discount1 || 0;
                        poItem.DiscountMode2Id = matchedExcelItem.DiscountModeId2 || 0;
                        poItem.Discount2 = matchedExcelItem.Discount2 || 0;
                        poItem.Discount = matchedExcelItem.Discount1 + matchedExcelItem.Discount2 || 0;
                        // poItem.ConversionQuantity = matchedExcelItem.ConversionQuantity || 0;
                    }
                });
            }
            $scope.addNewLineItem();
        ...
        $scope.getItemByExcel = function (data) {
            ...
            if (data.IsOpenPO) {
                action = 'pharmacy/itemmaster/GetItemsForPurchaseOrder';
                inputData = {
                    Params: [{
                            Key: 3,
                            Value: 2
    ...
    purchaseOrderFormController.$inject = ['$rootScope', '$scope', '$stateParams', '$state', '$translate', 'utl', '$filter', '$timeout', 'Upload'];
