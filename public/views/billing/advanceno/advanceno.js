(function () {
    ...
    angular
        .module('app.pages')
        .controller('advancenoController', advancenoController);

    function advancenoController($scope, $filter, $stateParams, $state, $translate, utl, $uibModalInstance, modalConfig) {
        var vm = this;
            ...
            columnDefs: [
                {
                    ...
                    cellTemplate: '<div class="ui-grid-cell-contents" > {{row.entity.ReceiptDateTime ? (row.entity.ReceiptDateTime | date : "dd/MM/yyyy HH:mm:ss") : "N/A"}} </div>'
                },
                {
                    field: "Patient.MRN", displayName: $translate.instant('billing.receiptpicker.mrnumber.lbl'),
                },
                {
                    field: "EncountertypeId", displayName: $translate.instant('billing.receiptpicker.op/dg/ip.lbl'), width: '30%',
                    cellTemplate: '<div class="ui-grid-cell-contents">{{"OP"}}</div>'
                },

                {
                    field: "ReceiptNumber", displayName: $translate.instant('billing.receiptpicker.receipt.lbl'),
                },
                {
                    field: "Patient", displayName: $translate.instant('billing.receipt-list.patientinfor.lbl'),
    ...
    advancenoController.$inject = ['$scope', '$filter', '$stateParams', '$state', '$translate', 'utl', '$uibModalInstance', 'modalConfig'];
