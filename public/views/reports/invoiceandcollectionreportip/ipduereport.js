(function () {
    ...
    angular
        .module('app.pages')
        .controller('IPDueReportController', IPDueReportController);

    function IPDueReportController($scope, $stateParams, $state, $translate, $filter, utl) {
        var vm = this;
                ...
                if (rowArray.Updateduser) {
                    if (rowArray.Updateduser.Title) {
                        if (rowArray.Updateduser.Title.Description) {
                            billed = rowArray.Updateduser.Title.Description;
                        }
                    }
                    if (rowArray.Updateduser.FirstName) {
                        billed += ' ' + rowArray.Updateduser.FirstName;
                    }
                    if (rowArray.Updateduser.LastName) {
                        billed += ' ' + rowArray.Updateduser.LastName;
                    }
                }
                csvContent += billdate + ',' + billNum + ',' + patName + ',' + billAmt + ',' + billDis + ',' + paidAmt + ',' + dueAmt + ',' + dueApprove + ',' + remark + ',' + billed + "\n";
            });
            var encodedUri = encodeURI(csvContent);
            var hiddenElement = document.createElement('a');
            hiddenElement.href = 'data:text/csv;charset=utf-8,' + encodedUri;
            hiddenElement.target = '_blank';
            hiddenElement.download = 'ipduereportforprivate-reports.csv';
            hiddenElement.click();
        ...
        $scope.print = function () {
            ...
            var options = {
                action: 'billing/patientbills/PrintIPDueReport',
                data: inputData,
                type: 'post'
            };
    ...
    IPDueReportController.$inject = ['$scope', '$stateParams', '$state', '$translate', '$filter', 'utl'];
