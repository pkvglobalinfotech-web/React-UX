(function () {
    ...
    function cancelFormController($scope, $filter, $stateParams,
        ...
        $scope.currentcontext.CancelReason = '';

        if (modalConfig && modalConfig.params) {
            $scope.currentcontext.eid = parseInt(modalConfig.params.eid);
            $scope.currentcontext = modalConfig.params.bill;
        ...
        $scope.loadIPDetails = function () {
            var SelectedIPDetailList = getSelectionRows();
            $scope.confirmCallback({ index: $scope.currentcontext.index, data: SelectedIPDetailList });
        }
        ...
        $scope.applyCancel = function () {
            ...
            console.log(data);
            // $scope.currentcontext.bill.Comments = $scope.currentcontext.Comments;
            let details = [];
            details.push(
                {
                    ...
                    CancelledBy: utl.Session.getCurrentUserId()
                });
            let Data = {
            ...
            var options = {
                action: 'Billing/PatientBillDetails/ManagePatientBillComments',
                data: {
                    Data
                },
                type: 'post',
                onComplete: $scope.confirmCallback({ index: $scope.currentcontext.index, data: Data.details[0] })
            };
    ...
    cancelFormController.$inject = ['$scope', '$filter', '$stateParams', '$state', '$translate', 'utl', '$uibModalInstance', 'modalConfig'];
