(function () {
    ...
    angular
        .module('app.pages')
        .controller('dispappointmentFormController', dispappointmentFormController);

    function dispappointmentFormController($scope, $stateParams, $state, $translate, utl, $uibModalInstance, modalConfig, $timeout, $filter) {
        var vm = this;
        ...
        if (modalConfig && modalConfig.params) {
            ...
            $scope.EncounterStatus = modalConfig.params.EncounterStatus;
        ...
        function setDefaults() {
            ...
            if (modalConfig && modalConfig.params && modalConfig.params.apptstatusid) {
        ...
        $scope.history = function () {
            if ($scope.item.Id) {
                utl.Modal.open('app.appointmenthistory', {
                    params: { appointmentId: $scope.item.Id },
                    confirmCallback: $scope.getList
                });
            } else {
        ...
        function apptClick(event) {
            ...
            $scope.item.AppointmentDate = moment(start).format('YYYY-MM-DD');
        ...
        $scope.Apply = function () {
            if ($scope.currentcontext.id == 0 && utl.Formatter.isPastDateTime($scope.item.AppointmentDate, $scope.item.StartTime)) {
                ...
                var schsttime = $scope.item.AppointmentDate + ' ' + $scope.item.StartTime;
                var schentime = $scope.item.AppointmentDate + ' ' + $scope.item.EndTime;
                $scope.confirmCallback({
                    ScheduleApptId: 0,
                    IsCheckedInAppt: true,
                    ScheduleApptDispTime: $scope.item.StartTime + ' - ' + $scope.item.EndTime,
                    ScheduleApptStartTime: moment(schsttime).toDate(),
                    ScheduleApptEndTime: moment(schentime).toDate(),
                    ScheduleApptDoctorId: $scope.item.DoctorId,
                    ScheduleApptDepartmentId: $scope.item.DepartmentId
                });
            }
        }
        ...
        $scope.prepareAppointments = function (items) {
            ...
            for (var idx in items) {
                ...
                appt.description = item.Patient.FirstName + " " + remarks;
        ...
        $scope.prepareAppointmentSessions = function (apptSession) {
            ...
            if (apptSession) {
                ...
                var slots = [];
                var isHoliday = false;
                var holidayFrom = new moment(utl.Formatter.getDateString(apptSession.HolidayFrom));
                var holidayTo = new moment(utl.Formatter.getDateString(apptSession.HolidayTo));
                var apptDate = new moment(utl.Formatter.getDateString($scope.item.AppointmentDate));
    ...
    dispappointmentFormController.$inject = ['$scope', '$stateParams', '$state', '$translate', 'utl', '$uibModalInstance', 'modalConfig', '$timeout', '$filter'];
