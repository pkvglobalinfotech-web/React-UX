(function () {
    ...
    opbilling.controller('directBillingListController', directBillingListController);

    function directBillingListController($rootScope, $scope, $stateParams, $state, $translate, utl, $filter, $timeout) {
        var vm = this;
        vm.Context = 'DG';
        ...
        $scope.calculateDOB = function (age, substractPart) {
            var options = {
                d: $scope.currentfilter.ApproxAgeDays,
                m: $scope.currentfilter.ApproxAgeMonths,
                y: $scope.currentfilter.Age
            };
            $scope.currentfilter.DOB = utl.Formatter.getDOBFromAgeConfig(options);
            $scope.currentfilter.IsBirthDateApproximate = true;
        };
        ...
        $scope.EnableDisableDropdown($scope.isPatinfoSaving);
        ...
        $scope.getPatientInfo = function (scope, data, options, hasError) {
            ...
            $scope.currentfilter.FirstName = $scope.selectedPatient.FirstName;
            $scope.currentfilter.Age = $scope.selectedPatient.Age;
            $scope.currentfilter.Mobile = $scope.selectedPatient.Mobile;
            $scope.currentfilter.GenderId = $scope.selectedPatient.GenderId;
            ...
            if (vm.Context == 'DG') {
                ...
                // $scope.facilitydefaultdoctor = utl.FacilitySetting.getFacilitySettingValue('general', 'doctor');
                // $scope.facilitydefaultdept = utl.FacilitySetting.getFacilitySettingValue('general', 'department');
                // if ($scope.facilitydefaultdoctor > 0) {
                //     $scope.item.DoctorId = $scope.facilitydefaultdoctor;
                // }
                // if ($scope.facilitydefaultdept > 0) {
                //     $scope.item.DepartmentId = $scope.facilitydefaultdept;
                // }
            }
                ...
                if (ServiceItemobj.IsPackage && ServiceItemobj.IsSaveServiceDetails) {
                    for (var packid in ServiceItemobj.ServiceItemPackageMaps) {
                        ...
                        if (packitem.Formula && packitem.DoctorId) {
                            ...
                            var computedValue = 0;
                            var lastIndex = $scope.PatientBillDetails.length - 1;
                            $scope.PatientBillDetails[lastIndex].SharePercentage = 0;
                            $scope.PatientBillDetails[lastIndex].PackageFormula = 0;
                            $scope.PatientBillDetails[lastIndex].ServiceId = packitem.ServiceId;
                            $scope.PatientBillDetails[lastIndex].ServiceCategoryId = item.ServiceCategoryId;
                            $scope.PatientBillDetails[lastIndex].ServiceName = packitem.ServiceName;
                            ...
                            for (var packsubidx in $scope.PatientBillDetails) {
                                if ($scope.PatientBillDetails[packsubidx].Status == 1) {
                                    ...
                                    if (packsubline.PackageMasterServiceId == item.ServiceId &&
                                        ...
                                        packsubline.DoctorShareActual = parseFloat(computedValue).toFixed(2);
                                        var netformulaobj = {
                                            NetAmount: item.NetAmount - discamt,
                                            SharePercentage: packsubline.SharePercentage,
                                        };
                                        var computedValue = (math.eval(packsubline.PackageFormula, netformulaobj));
                                        packsubline.DoctorShare = parseFloat(computedValue).toFixed(2);
        ...
        $scope.itemwiseDrChange = function (evt, item) {
            item.DiscountAmount = 0;
            $scope.currentcontext.BillDiscount = 0;
            if (item.DoctorShare > item.NetAmount) {
                item.DoctorShare = 0;
            }
        }
        ...
        $scope.CalcualteAmt = function (item) {
            ...
            if (item.NetAmount >= 0) {
                ...
                utl.Alert.showErrorMsg($translate.instant('billing.directbilling.discountlimit.lbl'));
            }
        ...
        $scope.HeaderDiscountValueChange = function () {
            if ($scope.currentfilter.DiscountModeId && $scope.currentfilter.DiscountModeId != -1) {
                ...
                utl.Alert.showErrorMsg($translate.instant('billing.directbilling.discounttypemessage.lbl'));
            }
        };
        ...
        $scope.CalculateNetAmt = function () {
            ...
            for (var i = 0, len = $scope.PatientBillDetails.length; i < len; i++) {
                if ($scope.PatientBillDetails[i].Status == 1) {
                    ...
                    if ($scope.CanShowLineItemDiscount == true) {
                        if ($scope.currentfilter.DiscountModeId > 0 && $scope.currentfilter.DiscountModeId == 2) {
                            $scope.PatientBillDetails[i].DiscountAmount = $scope.PatientBillDetails[i].DiscountPercentage;
                        }
                    }
            ...
            if ($scope.item.PatientBillStatusId == 1 && $scope.currentcontext.id > 0) {
            ...
            // }
            // if ($scope.item.TotDiscAmount > $scope.item.GrossAmount || $scope.currentcontext.TotBalanceAmt < 0) {
            //     $scope.currentcontext.dBillDiscount = 0;
            //     $scope.currentcontext.ReceiptAmt = null;
            //     $scope.currentcontext.TotBalanceAmt = 0;
            //     utl.Alert.showSuccessMsg($translate.instant('billing.ipbillingtab.billdiscount.lbl'));
        ...
        $scope.OnLoadPackageItem = function (SelectedItem) {
            if (SelectedItem.IsPackage) {
                utl.Alert.showSuccessMsg($translate.instant('billing.directbilling.needtoload.lbl'));
            } else {
                SelectedItem.IsPackage = false;

                utl.Alert.showSuccessMsg($translate.instant('billing.directbilling.noneedtoload.lbl'));
            }
        };
        ...
        function patientBillPickerCallback(patientbilldata) {
            $scope.targetaction = null;
            $scope.isPatinfoSaving = true;
            $scope.currentcontext.id = patientbilldata.BillId;
            $scope.currentfilter.PatientId = patientbilldata.PatientId;
        ...
        $scope.add_new = function () {
            utl.Modal.open('app.directbilling', {
                params: {
                    id: 0
                },
                confirmCallback: $scope.getList
        ...
        $scope.dmPrint = function () {
            ...
            if ($scope.dmprintpreferences != 1) {
                utl.Alert.showSuccessMsg($translate.instant('billing.directbilling.currentfacility.lbl'));
        ...
        $scope.backToList = function () {
            $state.go('app.directbilling', $scope.currentcontext.id);
        };
        ...
        $scope.getBillInfoCallback = function (scope, res, options, hasError) {
            ...
            if ($scope.PatientBillInfo && $scope.PatientBillInfo.length > 0) {
                ...
                for (var idx in $scope.PatientBillInfo) {
                    var patientbills = $scope.PatientBillInfo[idx];
        ...
        $scope.loadPatientGuarantorsCallback = function (scope, data, options, hasError) {
            ...
            if (!$scope.currentfilter.GuarantorId || $scope.currentfilter.GuarantorId == -1) {
                ...
                $scope.item.GuarantorDueId = $scope.lookup.PatientGuarantor[1].Id;
                $scope.currentfilter.GuarantorName = $scope.lookup.PatientGuarantor[1].GuarantorName;;
                $scope.currentfilter.GuarantorTypeId = $scope.lookup.PatientGuarantor[1].GuarantorTypeId;
                $scope.currentfilter.ServiceRateCategoryId = $scope.lookup.PatientGuarantor[1].Guarantor.ServiceRateCategoryId;
            }
            if ($scope.targetaction)
                $scope.completeBill($scope.targetaction);
        ...
        $scope.savePatientInfoCallback = function (scope, data, options, hasError) {
            if (typeof (data) == "number") {
                $scope.currentfilter.GuarantorId = -1; // don't remove this line
                $scope.item.PatientId = data;
                $scope.currentfilter.PatientId = data;
                $scope.patientChange();
            }
        };
        ...
        $scope.completeBill = function (action) {
            ...
            if (!$scope.item.PatientId || $scope.item.PatientId < 0) {
                $scope.targetaction = action;
                $scope.currentfilter.OverrideDuplicate = true;
                $scope.currentfilter.MRNTypeId = 1;
                $scope.currentfilter.FacilityId = utl.Session.getCurrentFacilityId();
                $scope.currentfilter.PatientStatus = 'Active';
        ...
        $scope.saveItem = function (StatusId) {
            ...
            if (!$scope.currentfilter.FirstName && !$scope.currentfilter.GenderId &&
                (!$scope.currentfilter.ReferralId || $scope.currentfilter.ReferralId == -1) && !$scope.currentfilter.Age &&
                !$scope.currentfilter.Mobile) {
                utl.Alert.showErrorMsg('Please Enter All Required Patient Data');
                return false;
            }
                    ...
                    if ($scope.currentfilter.DiscountModeId > 0 && $scope.currentfilter.DiscountModeId == 2) {
                        ...
                        for (var per = 0, perlen = $scope.PatientBillDetails.length; per < perlen; per++) {
                            ...
                            if (billingitem.DiscountAmount == 0) {
                                ...
                                $scope.PatientBillDetails[per].NetAmount = $scope.PatientBillDetails[per].Amount - $scope.PatientBillDetails[per].ProportionateDiscount;
                            }
                        }
                    } else if ($scope.currentfilter.DiscountModeId == 1) {
                        ...
                        for (var inr = 0, inrlen = $scope.PatientBillDetails.length; inr < inrlen; inr++) {
                            ...
                            $scope.PatientBillDetails[inr].NetAmount = $scope.PatientBillDetails[inr].Amount - $scope.PatientBillDetails[inr].ProportionateDiscount;
                        }
                    }
                } else {
                    for (var idx in $scope.PatientBillDetails) {
    ...
    directBillingListController.$inject = ['$rootScope', '$scope', '$stateParams', '$state', '$translate', 'utl', '$filter', '$timeout'];
