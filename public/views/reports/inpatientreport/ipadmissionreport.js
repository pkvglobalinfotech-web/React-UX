(function () {
    ...
    angular
        .module('app.pages')
        .controller('IPAdmissionReportController', IPAdmissionReportController);

    function IPAdmissionReportController($scope, $stateParams, $state, $translate, $filter, utl) {
        var vm = this;
        ...
        $scope.excelDownloadCallbackExcel = function (scope, data, options, hasError) {
            const JsonFields = ["DOA", "IP Number", "Patient Name", "MRN", "Room Details", "Doctor Name", "Payer Name", "TPA", "Department", "Created By"]
            ...
            data.Data.forEach(function (rowArray) {
                ...
                if (rowArray.Created) {
                    if (rowArray.Created.Title) {
                        if (rowArray.Created.Title.Description) {
                            created += ' ' + rowArray.Created.Title.Description;
                        }
                    }
                    if (rowArray.Created.FirstName) {
                        created += ' ' + rowArray.Created.FirstName;
                    }
                    if (rowArray.Created.LastName) {
                        created += ' ' + rowArray.Created.LastName;
                    }
                }
                ...
                csvContent += admdate + ',' + visitno + ',' + patname + ',' + mrn + ',' + ward + ',' + docname + ',' + insurance + ',' + tpa + ',' + dept + ',' + created + "\n";
            });
            var encodedUri = encodeURI(csvContent);
            var hiddenElement = document.createElement('a');
            hiddenElement.href = 'data:text/csv;charset=utf-8,' + encodedUri;
            hiddenElement.target = '_blank';
            hiddenElement.download = 'ipadmissionreport.csv';
            hiddenElement.click();
        ...
        $scope.print = function () {
            ...
            var options = {
                action: 'Visit/Visit/PrintIPAdmissionReport',
                data: inputData,
                type: 'post'
            };
        ...
        vm.gridConfig = {
            ...
            {
                field: "Age",
                displayName: $translate.instant('Age/Gender'),
                cellTemplate: "<div class='ui-grid-cell-contents'>" +
                    //     '<a ng-click="handleEvents(\'patientinfo\',entity)" uib-tooltip="{{entity.Patient.Title.Description}} ' + '{{entity.Patient.FirstName }} ' +
                    //     '{{entity.Patient.LastName}} | ' + '{{entity.Patient.MRN}} | ' + '{{entity.Patient.Age}} | ' + '{{entity.Patient.Gender.Description}}" tooltip-placement="bottom">'
                    '<a class="grid-action">' +
            ...
            {
                field: "TPA.Description",
                displayName: $translate.instant('TPA')
            },
            {
            ...
            },
            {
                ...
                cellTemplate: "<div class='ui-grid-cell-contents'>\
                                       <span ng-if='entity.Created.Title && entity.Created.Title.Description'>{{entity.Created.Title.Description}}&nbsp;</span>\
                                       <span>{{entity.Created.FirstName}}</span>&nbsp;<span>{{entity.Created.LastName}}</span>\
                                        </div>"
            },
            ],
            pagerObj: {
    ...
    IPAdmissionReportController.$inject = ['$scope', '$stateParams', '$state', '$translate', '$filter', 'utl'];
