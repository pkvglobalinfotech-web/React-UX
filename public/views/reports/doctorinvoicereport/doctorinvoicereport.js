(function () {
    ...
    angular
        .module('app.pages')
        .controller('DoctorInvoiceReportController', DoctorInvoiceReportController);

    function DoctorInvoiceReportController($rootScope, $scope, $stateParams, $state, $translate, utl, $filter, $timeout) {
        var vm = this;
        ...
        $scope.excelDownloadCallbackExcel = function (scope, data, options, hasError) {
            ...
            data.Data.forEach(function (rowArray) {
                ...
                if (rowArray.DueAmount) {
                    balance = rowArray.DueAmount;
                }

                csvContent += Date + ',' + invoicenum + ',' + docName + ',' + invoiceAmt + ',' + tdsAmt + ',' + paidAmt + ',' + balance + "\n";
            });
            var encodedUri = encodeURI(csvContent);
            var hiddenElement = document.createElement('a');
            hiddenElement.href = 'data:text/csv;charset=utf-8,' + encodedUri;
            hiddenElement.target = '_blank';
            hiddenElement.download = 'doctorinvoice-report.csv';
            hiddenElement.click();
        ...
        $scope.print = function () {
            ...
            var options = {
                action: 'doctorinvoice/DoctorInvoice/PrintDoctorInvoiceReport',
                data: inputData,
                type: 'post'
            };
    ...
    DoctorInvoiceReportController.$inject = ['$rootScope', '$scope', '$stateParams', '$state', '$translate', 'utl', '$filter', '$timeout'];
