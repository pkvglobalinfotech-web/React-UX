(function() {
    ...
    angular
        .module('app.pages')
        .controller('vendorMasterGSTController', vendorMasterGSTController);

    function vendorMasterGSTController($scope, $stateParams, $state, $translate, utl) {
        var vm = this;
        $scope.item = {
        ...
        $scope.getItem = function(pageNo) {
            var inputData = {
                Params: [
                    { Key: 2, Value: $scope.currentcontext.vendormasterid }
                ],
                PageContext: {
                    PageSize: 25,
                    PageNumber: 1
        ...
        $scope.back = function() {
            $state.go('app.vendormastertab.vendormaster');
        }
    ...
    vendorMasterGSTController.$inject = ['$scope', '$stateParams', '$state', '$translate', 'utl'];
