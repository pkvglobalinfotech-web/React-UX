(function () {
    ...
    angular
        .module('app.pages')
        .controller('ipbillingprofiledetailsController', ipbillingprofiledetailsController);

    function ipbillingprofiledetailsController($scope, $stateParams, $state, $translate, utl, $uibModalInstance, modalConfig, $filter, $timeout) {
        var vm = this;
        ...
        $scope.allowipdrshare = utl.FacilitySetting.getFacilitySettingValue('billing', 'allowipdrshare');

        $scope.currentcontext.CanShowDelete = 0
        $scope.currentcontext.CanShowDelete = $scope.HasAccess('IPBILLING_DETAILS', 'CanDeletedNewBillButton');

        $scope.currentcontext.CanEditTariff = 0
        $scope.currentcontext.CanEditTariff = $scope.HasAccess('IPBILLING_DETAILS', 'CanEditTariff');
        ...
        $scope.CheckserviceDate = function (item) {
            ...
            // var orderDate = new Date(item.BillDateTime);
            // var enddate = orderDate.getDate();
        ...
        $scope.rejectKeys = function (e, item) {
        ...
        $scope.calcAmt = function (index, item) {
            if (item.ServiceId > 0) {
                ...
                item.DoctorShare = Number(item.Quantity) * parseFloat(item.DoctorShare);
                ...
                if (item.UnitGSTAmount > 0) {
                    ...
                    item.NetAmount = ((parseFloat(item.Rate) * parseInt(item.Quantity)) + parseFloat(item.GSTAmount)) - parseFloat(item.DiscountAmount);
                }
                if (item.DoctorShareValue > 0) {
                    if (item.DrShareTypeId == 2) {
                        item.DoctorShare = item.NetAmount * (item.DoctorShareValue / 100);
                    } else if (item.DrShareTypeId == 1) {
                ...
                for (var idx in $scope.PatientBillDetails) {
                    if ($scope.PatientBillDetails[idx].Status == 1) {
                        ...
                        if ($scope.PatientBillDetails[idx].PatNetAmount) {
                            $scope.item.NetPatientAmount = $scope.item.NetPatientAmount + $scope.PatientBillDetails[idx].PatNetAmount;
                        }
                        if ($scope.PatientBillDetails[idx].InsNetAmount) {
                            $scope.item.NetInsuranceAmount = $scope.item.NetInsuranceAmount + $scope.PatientBillDetails[idx].InsNetAmount;
                        }
                        ...
                        $scope.PatientBillDetails[idx].NetAmount = $scope.PatientBillDetails[idx].NetAmount;
                    }
                }
                ...
                for (var idx in $scope.PatientBillDetails) {
                    if ($scope.PatientBillDetails[idx].Status == 1) {
                        ...
                        $scope.item.GSTAmount += parseInt($scope.PatientBillDetails[idx].GSTAmount);
                    }
                }
            }
        };
        ...
        $scope.getEncounterIPPackagesCallback = function (scope, res, options, hasError) {
            $scope.EncounterIPPackage = res.Data[0];
            $scope.EncounterIPPackageDetails = $scope.EncounterIPPackage.EncounterIPPackageDetails;
            $scope.EncounterIPPackageServiceInclusion = $scope.EncounterIPPackageDetails.EncounterIPPackageServiceInclusion;
            $scope.EncounterIPPackageServiceExclusion = $scope.EncounterIPPackageDetails.EncounterIPPackageServiceExclusion;
        };
        $scope.getEncounterIPPackages = function () {
            var inputData = {
                Params: [{
                    Key: 6,
            ...
            var options = {
                ...
                data: inputData,
                type: 'post',
                onComplete: $scope.getEncounterIPPackagesCallback
            };
            utl.Http.doAction(options);
        };
                        ...
                        if (shareitem.SharingTypeId == 1) { // Category wise share
                            ...
                            if (item.ServiceId == shareitem.ServiceId) {
                                ...
                                try {
                                    ...
                                    // item.DoctorShare = Number(item.Quantity) * SharePerAmount;
                                    item.DoctorShare = SharePerAmount;
                                } catch (ex) { }
                                try {
                                    if ($scope.DrIncludeTax && item.DrTaxPercentage > 0) {
        ...
        $scope.loadPatientGuarantorsCallback = function (scope, data, options, hasError) {
            ...
            forEach($scope.lookup.PatientGuarantor, function (v) {
                if (v.GuarantorId === $scope.Encounter.GuarantorId)
        ...
        $scope.saveAndApprove = function () {
            ...
            if ($scope.selectedPatient.LastName != null) {
                ...
                if ($scope.selectedPatient.Title.Description) {
                    $scope.item.PatientName = $scope.selectedPatient.Title.Description +
                        ' ' + $scope.selectedPatient.FirstName;
                } else {
                    $scope.item.PatientName = $scope.selectedPatient.FirstName;
        ...
        $scope.checkforautoLock = function () {
            if ((parseFloat($scope.TotalNet) + parseFloat($scope.TotalDue)) > parseFloat($scope.currentcontext.MaxLockCredit)) {
        ...
        $scope.saveItem = function () {
            ...
            if ($scope.currentcontext.isAutoBillLock == true) {
                if ($scope.Encounter && $scope.Encounter.GuarantorId == 1) {
                    if ($scope.Encounter.BillUnlockRequestStatusId != 2) {
                        var amount = parseFloat($scope.TotalNet) + parseFloat($scope.TotalDue);
            ...
            var actionName = 'billing/patientbills/AddIPPatientBills';
            if ($scope.currentcontext.bid && $scope.currentcontext.bid > 0) {
                actionName = 'billing/patientbills/UpdatePatientBills';
            }
            $scope.item.checkBill = true;
            var inputData = {
                Header: $scope.item,
                Details: lines,
                paymentDetail: paymentlines,
                encounter: $scope.Encounter,
                encounterippackage: $scope.EncounterIPPackage,
                encounterippackagedetail: $scope.EncounterIPPackageDetails,
            };
            savehitcompleted = 1;
        ...
        //         LockStatusId: 1,
        //         LockTypeId: 2,//PAtient not Paid
        //         Comments: 'Exceeded Max Credit',
            ...
            for (var idx in $scope.PatientBillDetails) {
                ...
                if ((item.Quantity > 0 && item.Status == 1) || item.PackageMasterServiceId > 0) {
                    if (item.ServiceId > 0) {
                        ...
                        item.flagset = 0;
                        if ($scope.Encounter.IsPackageAssigned) {
                            // for (var idx in $scope.EncounterIPPackageDetails) {
                            // var items = $scope.EncounterIPPackageDetails[idx];
                            if ($scope.EncounterIPPackage.IsUnlimitedServices == 1) {
                                item.IsInclusionItem = 1;
                                item.flagset = 1;
                                // $scope.EncounterIPPackageServiceExclusion = items.EncounterIPPackageServiceExclusions;
                                if ($scope.EncounterIPPackage.IsUnlimitedServices == 0) {
                                    if ((items.ServiceCategoryId == item.ServiceCategoryId) && items.IPPackageDetailId > 0) {
                                        item.IsInclusionItem = 1;
                                        item.flagset = 1;
                                        $scope.EncounterIPPackageServiceExclusion = items.EncounterIPPackageServiceExclusions;
                                    }
                                }
                                // for (var iex in $scope.EncounterIPPackageServiceExclusion){
                                //     var exclusion = $scope.EncounterIPPackageServiceExclusion[iex];
                                //     if(exclusion.ServiceCategoryId == item.ServiceCategoryId || $scope.EncounterIPPackage.IsUnlimitedServices == 0 ){
                                //         item.IsExclusionItem = 1;
                                //     }
                                // }
                            }
                            // else{
                        ...
                        item.allowBilltoSave = $scope.allowBilltoSave;
                        result.push(item);
                    } else
                        isValid = false;
                }
    ...
    ipbillingprofiledetailsController.$inject = ['$scope', '$stateParams', '$state', '$translate', 'utl', '$uibModalInstance', 'modalConfig', '$filter', '$timeout'];
