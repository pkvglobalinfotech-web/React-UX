(function () {
    ...
    angular
        .module('app.pages')
        .controller('editunlockbillingrequestController', editunlockbillingrequestController);

    function editunlockbillingrequestController($scope, $filter, $stateParams, $state, $translate, utl, $uibModalInstance, modalConfig) {
        ...
        $scope.item = {};
        $scope.items = {};
        if (modalConfig && modalConfig.params && modalConfig.params.data) {
            $scope.item = modalConfig.params.data;
        }
        ...
        $scope.UnlockApprovedCallback = function (scope, data, options, hasError) {
            // utl.Alert.showSuccessMsg($translate.instant('common.successmsg.lbl'));
            // $scope.UpdatePatientBillsForCancel();
            $scope.confirmCallback();
        };
        $scope.UnlockApproved = function () {

            $scope.items.BillingRequestStatusId = 2;
            $scope.items.Id = $scope.item.BillingRequest.Id;
            $scope.items.EncounterId = $scope.item.Id;
            $scope.items.Unlock = 1;
            $scope.items.BillUnlockApprovebyId = utl.Session.getCurrentUserId();
            // var lines = getLinesForSaveCancel();
            var actionName = 'Billing/BillingRequest/AddBillingRequest';
            if ($scope.item.BillingRequest.Id && $scope.item.BillingRequest.Id > 0) {
            ...
            var options = {
                ...
                type: 'post',
                onComplete: $scope.UnlockApprovedCallback
            };
            utl.Http.doAction(options);
        };
        $scope.UnlockRejectedCallback = function (scope, data, options, hasError) {
            //  utl.Alert.showSuccessMsg($translate.instant('common.successmsg.lbl'));
            // $scope.UpdatePatientBillsForCancel();
            $scope.confirmCallback();
        };
        $scope.UnlockRejected = function () {
            ...
            var options = {
                ...
                type: 'post',
                onComplete: $scope.UnlockRejectedCallback
            };
            utl.Http.doAction(options);
        };
    ...
    editunlockbillingrequestController.$inject = ['$scope', '$filter', '$stateParams', '$state', '$translate', 'utl', '$uibModalInstance', 'modalConfig'];
