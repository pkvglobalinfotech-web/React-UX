(function () {
    ...
    angular
        .module('app.pages')
        .controller('ChildToothChartController', ChildToothChartController);

    function ChildToothChartController($scope, $interval, $stateParams, $state, $translate, utl) {
        var vm = this;
        ...
        $scope.PreviousDetails = function(encinfo) {
            if(encinfo && encinfo.Patient.Id && encinfo.Id) {
                utl.Modal.open('patientemr.previouschildtooth', {
    ...
    ChildToothChartController.$inject = ['$scope', '$interval', '$stateParams', '$state', '$translate', 'utl'];
