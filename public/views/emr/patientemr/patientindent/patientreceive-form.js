(function () {
    ...
    angular
        .module('app.pages')
        .controller('patientReceiveFormController', patientReceiveFormController);

    function patientReceiveFormController($scope, $stateParams, $state, $translate, utl, $filter, uibButtonConfig, lodash, $uibModalInstance, modalConfig) {
        var vm = this;
        ...
        $scope.item.EncounterId = $scope.currentcontext.eid;
        $scope.currentcontext.PatientRequestStatusId = parseInt($stateParams.PatientRequestStatusId);
        $scope.item.PatientId = $scope.currentcontext.pid;
        ...
        $scope.onSaveandApproveConfirmed = function () {
            ...
            //     $scope.item.PatientRequestDateTime = utl.Formatter.getCurrentDate();
            // }
            if ($scope.currentcontext.PatientRequestStatusId == 4) {
                $scope.item.PatientRequestStatusId = 8;
            }else{
            $scope.item.PatientRequestStatusId = 7;
            }
        ...
        $scope.getRequestInfoCallback = function (scope, res, options, hasError) {
            ...
            if ($scope.PatientRequestInfo && $scope.PatientRequestInfo.length > 0) {
                $scope.PatientRequestInfo.forEach(patientrequest => {
                    ...
                    $scope.item.CancelledDate = patientrequest.CancelledDate;
        ...
        $scope.getDetailCallback = function (scope, data, options, hasError) {
            ...
            if (data.Data.length > 0) {
                for (var pdx in data.Data) {
                    var stockReq = data.Data[pdx];
                    stockReq.PendingQuantity = stockReq.RequestedQuantity - stockReq.DispensedQuantity;
                    $scope.PatientStockRequestDetails.push(stockReq);
                }
            }
        ...
        function presearchpharmacyitem() {
            ...
            var inputData = {
                Params: [{ Key: 11, Value: $scope.item.ToStoreId }, { Key: 13, Value: 2 }],
        ...
        $scope.print = function () {
            ...
            var options = {
                action: 'IPManagement/PatientStockRequests/PrintPatientStockReceive',
                data: inputData,
                type: 'post'
            };
    ...
    patientReceiveFormController.$inject = ['$scope', '$stateParams', '$state', '$translate', 'utl', '$filter', 'uibButtonConfig', 'lodash', '$uibModalInstance', 'modalConfig'];
