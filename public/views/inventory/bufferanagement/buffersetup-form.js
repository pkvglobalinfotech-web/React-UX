(function () {
    ...
    angular
        .module('app.pages')
        .controller('buffersetupformController', buffersetupformController);

    function buffersetupformController($scope, $stateParams, $state, $translate, utl, $uibModalInstance, modalConfig, $filter) {
        var vm = this;
        ...
        /*
        $scope.MinMaxValidation = function(item) {
            if (item.MinQty > item.MaxQty) {
                utl.Alert.showErrorMsg('Max Qty Should be Greater than Min Qty..!');
                item.MaxQty = 0;
            }
        };
        */
        ...
        $scope.getListCallback = function (scope, res, options, hasError) {
            ...
            $scope.item.MaxForeCast = data.MaxForeCast;
            $scope.item.FactorOnUnReliable = data.FactorOnUnReliable;
            $scope.item.TargetInventory = data.TargetInventory;
            $scope.item.BReT = data.BReT;
            $scope.item.OLT = data.OLT;
            $scope.item.PLT = data.PLT;
            $scope.item.TLT = data.TLT;
        };
        ...
        $scope.saveItem = function () {
            ...
            if ($scope.item.MinQty > $scope.item.MaxQty) {
                utl.Alert.showErrorMsg($translate.instant('inventory.bufferanagement.maxqty.lbl'));
                return false;
            }
    ...
    buffersetupformController.$inject = ['$scope', '$stateParams', '$state', '$translate', 'utl', '$uibModalInstance', 'modalConfig', '$filter'];
