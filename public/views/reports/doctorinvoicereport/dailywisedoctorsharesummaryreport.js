(function () {
    ...
    angular
        .module('app.pages')
        .controller('dailywisedoctorsharesummaryreportController', dailywisedoctorsharesummaryreportController);

    function dailywisedoctorsharesummaryreportController($scope, $stateParams, $state, $translate, $filter, utl) {
        var vm = this;
        ...
        $scope.OPDDrProceShareDetailsCallback = function (scope, res, options, hasError) {
            ...
            for (var gdx in grpData) {
                ...
                var itemgrouped = [];
                for (var idx in drShare) {
                    var item = drShare[idx];
                    var year = new Date(item.PatientBill.BillDateTime).getFullYear();
                    var month = new Date(item.PatientBill.BillDateTime).getMonth();
                    var date = new Date(item.PatientBill.BillDateTime).getDate();
                    item.ConsultDate = (month + 1) + '/' + date + '/' + year;
                    itemgrouped.push(item);
                }
                var Servicegroup = _.groupBy(itemgrouped, 'ConsultDate');
        ...
        $scope.IPDDrProceShareDetailsCallback = function (scope, res, options, hasError) {
            ...
            for (var gdx in grpData) {
                ...
                var itemgrouped = [];
                for (var idx in IPdrShare) {
                    var item = IPdrShare[idx];
                    var year = new Date(item.PatientBill.BillDateTime).getFullYear();
                    var month = new Date(item.PatientBill.BillDateTime).getMonth();
                    var date = new Date(item.PatientBill.BillDateTime).getDate();
                    item.ConsultDate = (month + 1) + '/' + date + '/' + year;
                ...
                var IPServicegroup = _.groupBy(itemgrouped, 'ConsultDate');
        ...
        $scope.SelectedCtgry = function (selectedItem) {
            $scope.ServiceCategory = selectedItem.Text;
        };
        ...
        $scope.print = function () {
            if ($scope.currentfilter.DoctorId > 0) {
                ...
                var options = {
                    action: 'billing/patientdoctorsharedetails/PrintDailyWiseDoctorShareSummaryReport',
                    data: inputData,
                    type: 'post'
                };
    ...
    dailywisedoctorsharesummaryreportController.$inject = ['$scope', '$stateParams', '$state', '$translate', '$filter', 'utl'];
