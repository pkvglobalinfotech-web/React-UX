(function() {
    ...
    angular
        .module('app.pages')
        .controller('SendmessagesController', SendmessagesController);

    function SendmessagesController($rootScope, $timeout, $scope, $stateParams, $state, $translate, utl, Upload, $http) {
        var vm = this;
        $scope.messages = [];
        ...
        $scope.getSendmsgsCallback = function(scope, res, options, hasError) {
            ...
            for (var idx in res.Data) {
                ...
                if (msg.UserTypeId == 8) {
                    if (msg.Patient) {
                        if (msg.Patient.Title) {
                            msg.ToUserName = msg.Patient.Title.Description;
                        }
                        if (msg.Patient.FirstName) {
                            if (msg.Patient.Title) {
                                msg.ToUserName += ' ' + msg.Patient.FirstName;
                            } else {
                                msg.ToUserName = msg.Patient.FirstName;
                            }
                        }
                        if (msg.Patient.LastName) {
                            msg.ToUserName += ' ' + msg.Patient.LastName;
                        }
                    }
                } else {
                    if (msg.ToUser) {
        ...
        $scope.getListCallback = function(scope, res, options, hasError) {
            ...
            for (var idx in res.Data) {
                ...
                if (msg.UserTypeId == ($scope.currentfilter.UserTypeId)) {
                    if (msg.ToUserId == utl.Session.getCurrentUserId()) {
        ...
        $scope.saveItem = function() {
            ...
            var actionName = 'SystemSettings/Message/AddMessage';
            if ($scope.currentcontext.id && $scope.currentcontext.id > 0) {
                actionName = 'SystemSettings/Message/UpdateMessage';
            }
        ...
        $scope.setMaster = function(ut) {
            $scope.selected = ut;
        }

        $scope.isSelected = function(ut) {
            return $scope.selected === ut;
        }

        $scope.lookupCallback = function(scope, data, options, hasError) {
            // $scope.lookup = hasError ? {} : data;
            $scope.lookup.SepUserType = [];
            forEach(data, function(value, key) {
                $scope.lookup[key] = value;
                if (key == 'UserType') {
                    for (var udx in $scope.lookup.UserType) {
                        var usertype = $scope.lookup.UserType[udx];
                        if ($scope.currentfilter.UserTypeId == 11) {
                            if (usertype.Id != 11) {
                                $scope.lookup.SepUserType.push(usertype);
                            }
                        } else {
                            $scope.lookup.SepUserType.push(usertype);
    ...
    SendmessagesController.$inject = ['$rootScope', '$timeout', '$scope', '$stateParams', '$state', '$translate', 'utl', 'Upload', '$http'];
