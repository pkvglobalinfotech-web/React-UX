(function () {
    ...
    angular
        .module('app.pages')
        .controller('videoconsultationpatientlistController', videoconsultationpatientlistController);

    function videoconsultationpatientlistController($scope, $stateParams, $state, $translate, $filter, utl) {
        var vm = this;
        ...
        $scope.excelDownloadCallbackExcel = function (scope, data, options, hasError) {
            const JsonFields = ["Patient Name", "MRN", "Age", "Gender", "Schedule Date", "Remark", "Doctor Name", "Booked Date", "AppointmentStatus", "Reason", "PaymentMode", "Consult Type", "Amount"]
            ...
            data.Data.forEach(function (rowArray) {
                ...
                if (rowArray.Patient.Age) {
                    patName += ' ' + rowArray.Patient.Age;
                }
                if (rowArray.Patient.Gender.Description) {
                    patName += ' ' + rowArray.Patient.Gender.Description;
                }
                if (rowArray.Patient.MRN) {
                ...
                if (rowArray.Remark) {
                    if (rowArray.Remark.Remarks) {
                        remark = rowArray.Remark.Remarks;
                    }
                }
                ...
                if (rowArray.CreatedAt) {
                    bookedDate = rowArray.CreatedAt;
                }
                if (rowArray.AppointmentStatus.Description) {
                    appointment = rowArray.AppointmentStatus.Description;
                }
                if (rowArray.Remarks) {
                    reason = rowArray.Remarks;
                }
                if (rowArray.PaymentMode) {
                    if (rowArray.PaymentMode.Description) {
                        paymode = rowArray.PaymentMode.Description;
                    }
                }

                if (rowArray.VirtualOrder) {

                    if (rowArray.VirtualOrder.OrderConsultType) {
                        consult = rowArray.VirtualOrder.OrderConsultType;
                    }
                }
                if (rowArray.VirtualOrder) {
                    if (rowArray.VirtualOrder.OrderTotal) {
                        amt = rowArray.VirtualOrder.OrderTotal;
                    }
                }
                csvContent += patName + ',' + mrn + ',' + age + ',' + gender + ',' + schedule + ',' + remark + ',' + docName + ',' + bookedDate + ',' + appointment + ',' + reason + ',' + paymode + ',' + consult + ',' + amt + "\n";
            });
            var encodedUri = encodeURI(csvContent);
            var hiddenElement = document.createElement('a');
            hiddenElement.href = 'data:text/csv;charset=utf-8,' + encodedUri;
            hiddenElement.target = '_blank';
            hiddenElement.download = 'videoconsultationpatientlist-report.csv';
            hiddenElement.click();
        ...
        $scope.print = function () {
            ...
            var options = {
                action: 'appointment/Appointment/PrintVideoConsultationPatientList',
                data: inputData,
                type: 'post'
            };
        ...
        vm.gridConfig = {
            ...
            columnDefs: [{
                ...
                {
                    field: "CreatedAt",
                    displayName: $translate.instant('Booked Date'),
                ...
                {
                    field: "PaymentMode.Description",
                    displayName: $translate.instant('PaymentMode')
                },
                {
                    field: "VirtualOrder.OrderConsultType.Description",
                    displayName: $translate.instant('Consult Type')
                },
                {
                    field: "VirtualOrder.OrderTotal",
                    displayName: $translate.instant('Amount'),
                    cellTemplate: "<div class='ui-grid-cell-contents'><span class='pl-3'>{{entity.VirtualOrder.OrderTotal}} </span> </div>"
                },
    ...
    videoconsultationpatientlistController.$inject = ['$scope', '$stateParams', '$state', '$translate', '$filter', 'utl'];
