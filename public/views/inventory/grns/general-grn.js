(function () {
    ...
    angular
        .module('app.pages')
        .controller('GeneralGRNFormController', GeneralGRNFormController);

    function GeneralGRNFormController($rootScope, $scope, $stateParams, $state, $translate, utl, $filter, $timeout) {
        var vm = this;
        ...
        $scope.gstinclusive = 0;
        $scope.gstinclusive =
            (utl.FacilitySetting.getFacilitySettingValue('billing', 'gstinclusiveforgeneralgrn')) ? utl.FacilitySetting.getFacilitySettingValue('billing', 'gstinclusiveforgeneralgrn') : 0;
        // $scope.gstinclusive = 1;
        ...
        $scope.ComputeMultipleDiscount = function (item) {
            ...
            if ($scope.gstinclusive == 0) {
                ...
                // Base prices
                const baseUomPrice = item.UomPriceAfterDiscount / (1 + item.GstPercentage / 100);
                const baseUnitPrice = item.PurchasePriceAfterDiscount / (1 + item.GstPercentage / 100);

                // GST Amounts
                item.GstAmount = parseFloat(((item.UomPriceAfterDiscount - baseUomPrice) * item.GrnQuantity).toFixed(4));
                item.UnitGstAmount = parseFloat((item.PurchasePriceAfterDiscount - baseUnitPrice).toFixed(4));

                item.CGstAmount = parseFloat(((baseUomPrice * item.CGstPercentage / 100) * item.GrnQuantity).toFixed(4));
                item.UnitCGstAmount = parseFloat((baseUnitPrice * item.CGstPercentage / 100).toFixed(4));

                item.SGstAmount = parseFloat(((baseUomPrice * item.SGstPercentage / 100) * item.GrnQuantity).toFixed(4));
                item.UnitSGstAmount = parseFloat((baseUnitPrice * item.SGstPercentage / 100).toFixed(4));

                // Cost prices are same as discounted price (GST already included)
                item.UomCostPrice = parseFloat(item.UomPriceAfterDiscount.toFixed(4));
                item.UnitCostPrice = parseFloat(item.PurchasePriceAfterDiscount.toFixed(4));

                // 4. Final Amounts
                item.GrossAmount = parseFloat((parseFloat(item.UomPrice) * parseFloat(item.GrnQuantity)).toFixed(4));
                item.TaxableAmount = parseFloat((item.PurchasePriceAfterDiscount * parseFloat(item.GrnQuantityAfterConversion)).toFixed(4));
                item.NetAmount = parseFloat((item.UnitCostPrice * parseFloat(item.GrnQuantityAfterConversion)).toFixed(4));
            }
    ...
    GeneralGRNFormController.$inject = ['$rootScope', '$scope', '$stateParams', '$state', '$translate', 'utl', '$filter', '$timeout'];
