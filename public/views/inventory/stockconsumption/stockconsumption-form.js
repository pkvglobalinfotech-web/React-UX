(function () {
    ...
    angular
        .module('app.pages')
        .controller('StockConsumptionFormController', StockConsumptionFormController);

    function StockConsumptionFormController($rootScope, $scope, $stateParams, $state, $translate, utl, $filter, $timeout) {
        var vm = this;
        ...
        $scope.SelectedFromStore = function (selectedstore) {
            ...
            if ($scope.stockconsumptionDetails.length > 0) {
        ...
        $scope.setIndexforTableIndex = function () {
            ...
            for (var idx in $scope.stockconsumptionDetails) {
                if ($scope.stockconsumptionDetails[idx].Status == 1) {
                    $scope.stockconsumptionDetails[idx].SNo = SNo;
                    $scope.stockconsumptionDetails[idx].itemidxdesc = 'desc' + (SNo - 1);
                    $scope.stockconsumptionDetails[idx].itemidxqty = 'qty' + (SNo - 1);
                    SNo++;
                }
            }
        };
        ...
        $scope.add_new = function () {
            utl.Modal.open('app.stockconsumption', {
                params: {
                    id: 0
                },
        ...
        $scope.backToList = function () {
            $state.go('app.stockconsumptions', $scope.currentcontext.id);
        };
        ...
        function checkMandatoryFields() {
            for (var iddx in $scope.stockconsumptionDetails) {
                ...
                if (iddxitem.ItemMasterId > 0 && iddxitem.QtyConsumed <= 0 && iddx.Status == 1) {
        ...
        $scope.onBatchSelected = function (consumingItem, selectedMasterItem, idx) {
            ...
            if (modified == -1) {
                ...
                consumingItem.BarcodeId = selectedMasterItem.BarcodeId;
        ...
        $scope.onItemSelected = function (idx, selectedItem) {
            ...
            if (SelectedMasterItem.ItemMaster.ToStoreStock &&
                SelectedMasterItem.ItemMaster.ToStoreStock.StockSerialItems.length > 0) {
                selectedItem.availableqty = SelectedMasterItem.ItemMaster.ToStoreStock.Quantity;
                stockserialitems = SelectedMasterItem.ItemMaster.ToStoreStock.StockSerialItems;
                selectedItem.StockItemRev = SelectedMasterItem.ItemMaster.ToStoreStock.Rev;
                var usedBatches = $scope.itemUsedBatches[selectedItem.ItemMasterId] || [];
                for (var batid = 0; batid < stockserialitems.length; batid++) {
        ...
        function formatselectedstockconsumptionitem() {
            ...
            if (selectedItem && !utl.Common.isEmptyJSONObject(selectedItem)) {
                ...
                result = [vm.stockconsumptionitemcontrolconfig.rowdata.ItemCode, vm.stockconsumptionitemcontrolconfig.rowdata.ItemName, vm.stockconsumptionitemcontrolconfig.rowdata.ProductTypeName].join(' ');
            }
            return result;
        }
    ...
    StockConsumptionFormController.$inject = ['$rootScope', '$scope', '$stateParams', '$state', '$translate', 'utl', '$filter', '$timeout'];
