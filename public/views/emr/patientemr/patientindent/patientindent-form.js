(function () {
    ...
    angular
        .module('app.pages')
        .controller('patientIndentFormController', patientIndentFormController);

    function patientIndentFormController($scope, $stateParams, $state, $translate, utl, $filter, uibButtonConfig, lodash, $uibModalInstance, modalConfig) {
        var vm = this;
        ...
        $scope.options = [{
            key: 'detail',
            name: $translate.instant('Details')
        },
        {
            key: 'ticksheet',
            name: $translate.instant('patientemr.prescription-form.ticksheet.lbl')
        },
        ...
        $scope.addTickSheet = function () {
            ...
            utl.Modal.openFixedDialog('patientemr.prescriptiondetail', {
                params: {
                    id: 0,
        ...
        $scope.addNew = function () {
            $state.go('patientemr.patientindentform', {
                id: 0
            });
        }
        ...
        $scope.history = function (HistoryId) {
            utl.Modal.openFixedDialog('app.history', {});
        };

        $scope.History = function (item, idx) {
            if (item.ItemMasterId > 0) {
                utl.Modal.openFixedDialog('app.patientrequesthistory', {
                    params: {
        ...
        $scope.deletePatientRequestDetail = function (idx, SelectedItem) {
            if (SelectedItem.ItemMasterId > 0) {
                var name = "this item" || '';
        ...
        //     if ($scope.context == 'emr')
        //         $state.go('patientemr.patientindent', { id: 0 });
        // };
        $scope.backToList = function () {
            // $state.go('patientemr.patientindent', { id: 0 });
        ...
        $scope.CancelRequest = function () {
            if($scope.item.RequesterComments != '' || $scope.item.RequesterComments != null){
                var confirmOptions = {
                    headingKey: 'common.confirm-modal-header.lbl',
                ...
                utl.Alert.showErrorMsg('Comment box is empty');
            }
        };
        ...
        $scope.saveItem = function () {
            ...
            if ($scope.item.AdmissionStatusId == 5 || $scope.item.AdmissionStatusId == 6 || $scope.item.AdmissionStatusId == 4) {
                utl.Alert.showErrorMsg('Please Check.. Patient Already Discharged');
                return false;
            } else {
                if ($scope.item.IsBillLock == true) {
                    utl.Alert.showErrorMsg('Please Check.. Patient Bills Locked');
                    return false;
                }
            }
        ...
        $scope.getPatientRequestInfoById = function () {
            var SearchRequestId = $scope.currentcontext.pid;
        ...
        $scope.getOtregisterCallback = function (scope, data, options, hasError) {
            var surgeryentry = data;
            $scope.item.OTRoomId = surgeryentry.SurgeryRoomId;
            $scope.item.OTRegisterId = surgeryentry.Id;
            $scope.item.OTIdentifier = surgeryentry.SurgeryIdentifier;
            $scope.item.IsSurgery = true;
        };
        ...
        $scope.getOtregisterById = function () {
            var options = {
                ...
                data: {
                    Id: $scope.currentcontext.surgeryid
                },
                type: 'post',
        ...
        $scope.getDetailCallback = function (scope, data, options, hasError) {
            $scope.PatientStockRequestDetails = data.Data || [];
            if ($scope.PatientRequestInfo && $scope.PatientRequestInfo.length > 0) {
                $scope.PatientRequestInfo.forEach(patientrequest => {
                    $scope.item.Id = patientrequest.Id;
                    $scope.item.PatientStockRequestId = patientrequest.Id;
                    ...
                    $scope.item.DepartmentId = patientrequest.DepartmentId;
                    $scope.item.GuarantorId = patientrequest.GuarantorId;
                    // $scope.item.GuarantorId = patientrequest.GuarantorTypeId;
                    $scope.item.LocationId = patientrequest.LocationId;
                    $scope.item.WardId = patientrequest.WardId;
                    $scope.item.RoomId = patientrequest.RoomId;
                    $scope.item.BedId = patientrequest.BedId;
        ...
        $scope.alternateDetails = function (idx, item) {
            utl.Modal.openFixedDialog('app.patientrequestalternates', {
                params: {
        ...
        $scope.getperviousindentCallback = function (scope, res, options, hasError) {
            $scope.PatientIndentDetails = res.Data || [];
            var indents = [];
            var indentdetails = [];
            for (var idx in res.Data) {
                var indents = res.Data[idx];
                if (indents.PatientId == $scope.currentcontext.pid) {
                    for (var iddx in indents.PatientStockRequestDetails) {
                        var indentdetails = indents.PatientStockRequestDetails[iddx];
                        for (var jdx in $scope.PatientStockRequestDetails) {
                            if ($scope.PatientStockRequestDetails[jdx].ItemMasterId > 0)
                                var ItemMaster = $scope.PatientStockRequestDetails[jdx].ItemMasterId;
                        }
                        if (indentdetails.ItemMasterId == ItemMaster) {
                            utl.Alert.showErrorMsg($translate.instant('Item Already Indented'));
                        }
                    }
                }
            }
        ...
        $scope.getperviousindent = function (SelectedMasterItem) {
            var FromReq = $filter('date')(utl.Formatter.getCurrentDate(), 'yyyy-MM-dd 00:00:00') || null;
            var ToReq = $filter('date')(utl.Formatter.getCurrentDate(), 'yyyy-MM-dd 23:59:59') || null;
            var inputData = {
                Params: [{
                    Key: 14,
                    Value: FromReq
                ...
                {
                    Key: 1,
                    Value: $scope.item.PatientRequestId
                },
                {
                    Key: 2,
                    Value: $scope.item.PatientRequestNumber
                },
                ],
            };
            var options = {
                action: 'IPManagement/PatientStockRequests/GetPatientStockRequestsList',
                data: inputData,
                type: 'post',
                onComplete: $scope.getperviousindentCallback
            };
        ...
        $scope.onItemSelected = function (idx, selectedItem) {
            ...
            if (SelectedMasterItem.ItemMaster) {
                ...
                if (SelectedMasterItem.ItemMaster.IsNarcotic == true) {
                    selectedItem.IsNarcotic = 'N';
                }
                if (SelectedMasterItem.ItemMaster.IsEmergency == true) {
                    selectedItem.IsEmergency = 'EMER';
                }
                if (SelectedMasterItem.ItemMaster.IsLASADrug == true) {
                    selectedItem.IsLASADrug = 'LASA';
                }
                if (SelectedMasterItem.ItemMaster.IsHighAlert == true) {
                    selectedItem.IsHighAlert = 'HIGH';
                }
            }
            ...
            if (SelectedMasterItem.ItemMaster.ToStoreStock) {
                selectedItem.QuantityOnHand = SelectedMasterItem.ItemMaster.ToStoreStock.Quantity;
            } else {
            ...
            $scope.getperviousindent(SelectedMasterItem);
        };
        ...
        $scope.getwardCallback = function (scope, res, options, hasError) {
            $scope.items = res.Data;
            $scope.item.ToStoreId = $scope.items[0].StoreMasterId;
        };
        $scope.getward = function () {
            ...
            var options = {
                ...
                data: inputData,
                type: 'post',
                onComplete: $scope.getwardCallback
            };
        ...
        $scope.getFacilityCallback = function (scope, res, options, hasError) {
            ...
            $scope.item.IsCashPatientEmrIndent = $scope.facilityitem[0].IsCashPatientEmrIndent;
        };
        ...
        $scope.wardLookUp = function () {
            var inputData = [{
                ...
                Request: {
                    Params: [{
                        Key: 2,
                        Value: $scope.item.FacilityId || 0
                    },
                    {
                        Key: 5,
                        Value: $scope.item.LocationId || 0
                    }
                    ]
                }
            },];
        ...
        $scope.stockstatus = function () {
            ...
            utl.Modal.open('patientemr.genericwicestockstatus', {
                ...
                confirmCallback: $scope.getBedLookUp
            });
        }

        $scope.pmrselection = function () {
            utl.Modal.open('patientemr.pmrselection', {
                params: {
                    id: 0,
                    storemasterid: $scope.item.ToStoreId
                },
        ...
        function loadSelectedList(selectedList) {
            $scope.PatientStockRequestDetails = [];
            if (selectedList.IndentData && selectedList.IndentData.length > 0) {
                selectedList.IndentData.forEach(SelectedIndent => {
                    var SelectedMasterItem = {};
                    SelectedMasterItem = SelectedIndent.ItemMaster;
                    var IndentDetail = {
                        Id: 0,
                        ItemMasterId: SelectedIndent.ItemMasterId,
                        ItemCode: SelectedIndent.ItemCode,
                        ItemName: SelectedIndent.ItemName,
                        RequestedQuantity: parseInt(SelectedIndent.Quantity),
                        DrugId: SelectedMasterItem.DrugId,
                        DrugCode: SelectedMasterItem.DrugCode,
                        DrugName: SelectedMasterItem.DrugName,
                        GenericId: SelectedMasterItem.GenericId,
                        GenericCode: SelectedMasterItem.GenericCode,
                        GenericName: SelectedMasterItem.GenericName,
                        BaseUomId: SelectedMasterItem.BaseUomId,
                        PurchaseUomId: SelectedMasterItem.PurchaseUomId,
                        SaleUomId: SelectedMasterItem.SaleUomId,
                        Ucp: parseFloat((SelectedMasterItem.ItemPrice).toFixed(2)),
                        Mrp: parseFloat((SelectedMasterItem.MrPrice).toFixed(2)),
                        Comments: '',
                        Status: 1,
                        GrossAmount: (SelectedMasterItem.MrPrice) * SelectedIndent.Quantity,
                        NetAmount: (SelectedMasterItem.MrPrice) * SelectedIndent.Quantity,
                    }
                    $scope.PatientStockRequestDetails.push(IndentDetail);
                });
            }
            calculatetotalAmount();
        }
    ...
    patientIndentFormController.$inject = ['$scope', '$stateParams', '$state', '$translate', 'utl', '$filter', 'uibButtonConfig', 'lodash', '$uibModalInstance', 'modalConfig'];
