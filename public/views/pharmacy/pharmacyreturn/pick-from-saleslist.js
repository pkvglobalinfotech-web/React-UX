(function() {
    ...
    angular
        .module('app.pages')
        .controller('pickfromsalesListController', pickfromsalesListController);

    function pickfromsalesListController($scope, $stateParams, $state, $translate, utl, $filter, $uibModalInstance, modalConfig) {
        var vm = this;
        ...
        function getReturnSelectionRows() {
            ...
            for (var idx in $scope.PatientBillDetails) {
                if ($scope.PatientBillDetails[idx].select) {
                    $scope.item.PatientBills.push($scope.PatientBillDetails[idx].PatientBillId)
        ...
        $scope.onLoadConfirmed = function() {
            ...
            $scope.confirmCallback({
                ...
                PatientBills: $scope.item.PatientBills,
                EncId: $scope.currentcontext.EncounterId
            });
        };
        ...
        $scope.getDispensedItemListCallback = function(scope, res, options, hasError) {
            ...
            if (res.Data.length > 0) {
                if (res.Data[0].PatientBill) {
                    if (res.Data[0].PatientBill.Patient) {
                        ...
                        $scope.selectedPatient.Id = res.Data[0].PatientBill.PatientId;
                        if ($scope.selectedPatient.Title)
                            $scope.selectedPatient.PatientName = $scope.selectedPatient.Title.Description;
                        if ($scope.selectedPatient.FirstName)
                            $scope.selectedPatient.PatientName += ' ' + $scope.selectedPatient.FirstName;
                        if ($scope.selectedPatient.LastName)
                            $scope.selectedPatient.PatientName += ' ' + $scope.selectedPatient.LastName;
            ...
            $scope.PatientInfo = $scope.selectedPatient;
            ...
            for (var gdidx in groupedDispensedItems) {
                ...
                for (var gidx in groupedItems) {
                    ...
                    for (var i = 0, len = groupedItem.length; i < len; i++) {
                        ...
                        if (groupedItem[i].IsPharmacySale == 1 && groupedItem[i].PharmacySaleTypeId == 1) {
                            itemqty = groupedItem[i].Quantity;
                        }
                        if (groupedItem[i].ReturnedQuantity > 0) {
                            itemreturnedqty = groupedItem[i].ReturnedQuantity;
                        }
                        //  else if (groupedItem[i].IsPharmacyReturn == 1 && groupedItem[i].PharmacyReturnTypeId == 1) {
        ...
        $scope.getPatientDispensedItems = function() {
            if ($scope.currentcontext.PatientId > 0) {
                var FrmDate = $filter('date')($scope.currentcontext.FromDate, 'yyyy-MM-dd 00:00:00');
                ...
                utl.Alert.showErrorMsg($translate.instant('Please Enter Patient Information'));
                return false;
            }
        };
        ...
        $scope.getPatientInfo = function(scope, data, options, hasError) {
            ...
            $scope.currentcontext.PatientId = $scope.selectedPatient.Id;
            ...
            if ($scope.selectedPatient.Encounters &&
                ...
                if (ipencounter) {
                    $scope.currentcontext.EncounterId = ipencounter.Id;
                }
                if (opencounter && opencounter.Id > 0) {
                    $scope.currentcontext.EncounterId = opencounter.Id;
                }
            }
        };
    ...
    pickfromsalesListController.$inject = ['$scope', '$stateParams', '$state', '$translate', 'utl', '$filter', '$uibModalInstance', 'modalConfig'];
