(function () {
    ...
    angular
        .module('app.pages')
        .controller('InvoicereceiptataccountListController', InvoicereceiptataccountListController);

    function InvoicereceiptataccountListController($rootScope, $scope, $filter, $stateParams, $state, $translate, utl, $timeout) {
        var vm = this;
        ...
        $scope.getList = function () {
            ...
            var inputData = {
                ...
                {
                    Key: 20,
                    Value: $scope.currentfilter.SubmissionStatusId
                },
                {
                    Key: 22,
                    Value: From
        ...
        $scope.handleEvents = function (actionType, entity) {
            if (actionType == 'edit') {
                $state.go('app.invoicereceiptataccountform', {
                    id: entity.Id,
                ...
                Saveandsubmit(entity.Id);
            }
            else if (actionType == 'reject') {
                Saveandreject(entity.Id);
            }
        };
        function Saveandsubmit(entity) {
            $scope.GrnId = entity;
            var confirmOptions = {
                headingKey: 'common.confirm-modal-header.lbl',
                messageKey: 'Do you want to Receive invoice?',
                yesKey: 'common.yeskey.lbl',
                noKey: 'common.nokey.lbl',
                onSuccessMethod: onSaveandsubmitConfirmed,
            };
            utl.Dialog.confirmMessage(confirmOptions);
        };

        function onSaveandsubmitConfirmed() {
            var grnid = $scope.GrnId;
            $scope.item.Id = grnid;
            $scope.item.SubmissionStatusId = 3;
            $scope.item.InvReceivedBy = utl.Session.getCurrentUserId()
            $scope.item.InvReceivedDate = utl.Formatter.getCurrentDate();
            $scope.saveItem();
        };
        function Saveandreject(entity) {
            $scope.GrnId = entity;
            var confirmOptions = {
                headingKey: 'common.confirm-modal-header.lbl',
                messageKey: 'Do you want to reject invoice?',
                yesKey: 'common.yeskey.lbl',
                noKey: 'common.nokey.lbl',
                onSuccessMethod: onSaveandrejectConfirmed,
            };
            utl.Dialog.confirmMessage(confirmOptions);
        };

        function onSaveandrejectConfirmed() {
            var grnid = $scope.GrnId;
            $scope.item.Id = grnid;
            $scope.item.SubmissionStatusId = 5;
            $scope.item.InvRejectedBy = utl.Session.getCurrentUserId()
            $scope.item.InvRejectedDate = utl.Formatter.getCurrentDate();
            $scope.saveItem();
        };
        ...
        $scope.saveItem = function (entity) {
            var actionName = 'pharmacy/grn/UpdateGrn';
            var inputData = {
                Header: $scope.item,
            };
            var options = {
        ...
        vm.gridConfig = {
            ...
            //     displayName: $translate.instant('GRN Status')
            // },
            {
                field: "SubmittedUser",
                displayName: $translate.instant('Sumitted By'),
                cellTemplate: "<div class='ui-grid-cell-contents'><span >{{entity.SubmittedUser.Title.Description}}&nbsp;</span>" + "<span >{{entity.SubmittedUser.FirstName}}&nbsp;</span>" + "<span >{{entity.SubmittedUser.LastName}}</span>" + "</div>"
            },
            {

                field: "SubmittedDate",
                displayName: $translate.instant('Submitted Date'),
                cellTemplate: "<div class='ui-grid-cell-contents'><span >{{entity.SubmittedDate | date : 'dd-MMM-yyyy'}}&nbsp;</span>" + "<span >{{entity.SubmittedDate| date: 'HH:mm'}}</span>" + "</div>"
            },
            {
                field: "ReceivedUser",
                displayName: $translate.instant('Received By'),
                cellTemplate: "<div class='ui-grid-cell-contents'><span >{{entity.ReceivedUser.Title.Description}}&nbsp;</span>" + "<span >{{entity.ReceivedUser.FirstName}}&nbsp;</span>" + "<span >{{entity.ReceivedUser.LastName}}</span>" + "</div>"
            },
            {

                field: "InvReceivedDate",
                displayName: $translate.instant('Received Date'),
                cellTemplate: "<div class='ui-grid-cell-contents'><span >{{entity.InvReceivedDate | date : 'dd-MMM-yyyy'}}&nbsp;</span>" + "<span >{{entity.InvReceivedDate| date: 'HH:mm'}}</span>" + "</div>"
            },
            {
                field: "ClaimSubmissionStatus.Description",
                displayName: $translate.instant('Submission Status')
            },
            {
                field: "Id",
                displayName: $translate.instant('common.actions_col.lbl'),
                cellTemplate: '<div class="ui-grid-cell-contents">\
                                                    <span class="grid-action"  uib-tooltip="View" ng-click="handleEvents(\'view\',entity)" ng-show="entity.GrnStatusId==2||entity.GrnStatusId==3||entity.GrnStatusId==4||entity.GrnStatusId==5"><img class="drhms-edit-button" src="assets/svg/edit.svg" aria-hidden="true"></span>\
                                                     ...
                                                     <span class="grid-action" uib-tooltip="Receive" tooltip-placement="bottom" ng-click="handleEvents(\'receipt\',entity)" ng-hide="entity.SubmissionStatusId==3 || entity.SubmissionStatusId==5"><span class=\"amend"\>Receive</span></span>\
                                                     <span class="grid-action" uib-tooltip="Reject" tooltip-placement="bottom" ng-click="handleEvents(\'reject\',entity)" ng-hide="entity.SubmissionStatusId==3 || entity.SubmissionStatusId==5"><span class=\"amend"\>Reject</span></span>\
    ...
    InvoicereceiptataccountListController.$inject = ['$rootScope', '$scope', '$filter', '$stateParams', '$state', '$translate', 'utl', '$timeout'];
