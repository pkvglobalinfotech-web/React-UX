(function () {
    ...
    angular
        .module('app.pages')
        .controller('purchasereturnFormController', purchasereturnFormController);

    function purchasereturnFormController($rootScope, $scope, $stateParams, $state, $translate, utl, $filter, $timeout) {
        var vm = this;
        ...
        $scope.item.IsPurchaseReturnEditable = false;
        ...
        $scope.deletePurchaseReturnDetail = function (idx, item) {
            if (item.ItemMasterId > 0) {
                if (item.ItemMasterId != -1) {
                    ...
                    var existing = $scope.itemUsedBatches[item.ItemMasterId].indexOf(item.SelectedPrimaryId);
        ...
        $scope.getPurchaseReturnInfoCallback = function (scope, res, options, hasError) {
            ...
            if ($scope.PurchaseReturnInfo && $scope.PurchaseReturnInfo.length > 0) {
                $scope.PurchaseReturnInfo.forEach(purchasereturn => {
                    ...
                    $scope.item.ReturnTypeId = purchasereturn.ReturnTypeId;
                    ...
                    $scope.item.Comments = purchasereturn.Comments;
                    ...
                    if (purchasereturn.ApprovedUser) {
                    ...
                    if ($scope.item.PrnTypeId == 1) {
                        if (purchasereturn.VendorMaster) {
                            if (purchasereturn.VendorMaster.VendorCode)
                    ...
                    if ($scope.item.PrnTypeId == 2) {
                        ...
                        if (purchasereturn.VendorMaster) {
                            if (purchasereturn.VendorMaster.VendorName)
                    ...
                    for (var pridx in $scope.PurchaseReturnDetails) {
                        ...
                        if (pritem.ItemMasterId > 0) {
                            if ($scope.item.PrnTypeId >= 2) {
                                ...
                                pritem.SelectedPrimaryId = pritem.StockSerialItemId;
                                if ($scope.item.PrnTypeId == 2) {
                                    ...
                                    BatchDetail.SelectedPrimaryId = pritem.StockSerialItemId;
        ...
        $scope.onBatchSelected = function (prnItem, selectedMasterItem, idx) {
            ...
            var existing = $scope.itemUsedBatches[prnItem.ItemMasterId].indexOf(prnItem.StockSerialItemId);
            var modified = $scope.itemUsedBatches[prnItem.ItemMasterId].indexOf(prnItem.SelectedPrimaryId);
            if (modified == -1) {
                ...
                // prnItem.SelectedPrimaryId = selectedMasterItem.StockSerialItemId;
                prnItem.SelectedPrimaryId = selectedMasterItem.Id;
                ...
                prnItem.UomPrice = selectedMasterItem.UomPrice;
                ...
                $scope.itemUsedBatches[prnItem.ItemMasterId].push(prnItem.StockSerialItemId);
            } else if (modified > -1) {
                ...
                prnItem.SelectedPrimaryId = prnItem.StockSerialItemId;
            }
        ...
        $scope.onItemSelected = function (idx, selectedItem) {
            ...
            if (selectedItem.SelectedItem.ItemMaster.StockItem) {
                if (selectedItem.SelectedItem.ItemMaster.StockItem.StockSerialItems.length > 0) {
                    ...
                    if (SelectedMasterItem.ItemMaster.StockItem &&
                        ...
                        for (var batid = 0; batid < stockserialitems.length; batid++) {
                            ...
                            if (serialitem.Quantity > 0 && usedBatches.indexOf(serialitem.Id) == -1) {
                        ...
                        if (selectedItem.BatchDetails && selectedItem.BatchDetails.length > 0) {
                            ...
                            selectedItem.SelectedPrimaryId = selectedItem.StockSerialItemId;
                ...
                selectedItem.SelectedPrimaryId = '';
        ...
        $scope.computeAmount = function (item) {
            ...
            //     item.ReturnQty = 0;
            // } else if (item.ReturnQty === null) { } else {
            //     item.GrossAmount = (item.PurchasePrice * item.ReturnQty);
            //     item.GstAmount = (item.GstPercentage * item.PurchasePrice) / 100 * (item.ReturnQty);
            //     if (item.GstAmount) {
            //         item.NetAmount = (item.PurchasePrice * item.ReturnQty) + (item.GstAmount);
            //     } else {
            //         item.NetAmount = (item.PurchasePrice * item.ReturnQty);
            //     }
            //     if (item.DiscountModeId == 1) {
            //         item.NetAmount = (item.PurchasePrice + item.GstAmount) - item.DiscountAmount;
            //     }
            //     if (item.DiscountModeId == 2) {
            //         item.Discount = (item.PurchasePrice * item.DiscountAmount) / 100 * (item.ReturnQty);
            //         item.NetAmount = (item.GrossAmount + item.GstAmount) - item.Discount;
            //     }
            console.log(item);
            ...
            if (parseFloat(item.Discount) > 0) {
                if (item.DiscountModeId == 1) {
                    item.UomDiscountAmount = parseFloat(item.Discount);
                    item.DiscountAmount = parseFloat(item.Discount) / item.ConversionQuantity || 1;
                } else if (item.DiscountModeId == 2) {
                    item.UomDiscountAmount = (parseFloat(item.UnitCostPrice || 0) / 100) * parseFloat(item.Discount);
                    item.DiscountAmount = (parseFloat(item.PurchasePrice) / 100) * parseFloat(item.Discount);
                    // item.DiscountAmount = (parseFloat(item.DiscountAmount)) * parseFloat(item.ReturnQty);
                } else {
                    // item.UomDiscountAmount = (parseFloat(item.UomPrice) / 100) * parseFloat(item.Discount);
                    item.DiscountAmount = (parseFloat(item.PurchasePrice) / 100) * parseFloat(item.Discount);
                }
            ...
            item.UomPriceAfterDiscount = parseFloat(item.UnitCostPrice || 0) - item.UomDiscountAmount;

            //uom --- with quantity
            // item.UomPriceAfterDiscount = parseFloat(item.UomPrice || 0) - item.UomDiscountAmount;
            item.PurchasePriceAfterDiscount = parseFloat(item.PurchasePrice) - item.DiscountAmount;
            // if (parseFloat(item.DiscountAmount) > 0) {
            //     if (item.DiscountModeId == 1) {
            //         item.UomDiscountAmount = parseFloat(item.DiscountAmount);
            //     } else if (item.DiscountModeId == 2) {
            //         item.UomDiscountAmount = (parseFloat(item.PurchasePrice) / 100) * parseFloat(item.DiscountAmount);
            //     } else {
            //         item.UomDiscountAmount = 0;
            //     }
            //     item.UomPriceAfterDiscount = parseFloat(item.PurchasePrice) - item.UomDiscountAmount;
            // }
            item.GrossAmount = (item.PurchasePrice * item.ReturnQty);
            // if (parseFloat(item.Discount) > 0 && item.UomPriceAfterDiscount > 0) {
            //     item.GstAmount = (item.GstPercentage * item.UomPriceAfterDiscount) / 100 * (item.ReturnQty);
            //     if (item.GstAmount) {
            //         item.NetAmount = (item.UomPriceAfterDiscount * item.ReturnQty) + (item.GstAmount);
            //     } else {
            //         item.NetAmount = (item.UomPriceAfterDiscount * item.ReturnQty);
            //     }
            // } else {
            //     item.GstAmount = (item.GstPercentage * item.PurchasePrice) / 100 * (item.ReturnQty);
            //     if (item.GstAmount) {
            ...
            if (parseFloat(item.DiscountAmount) > 0) {
                item.GstAmount = (item.GstPercentage * item.PurchasePriceAfterDiscount) / 100 * (item.ReturnQty);
                if (item.GstAmount) {
                    item.NetAmount = (item.PurchasePriceAfterDiscount * item.ReturnQty) + (item.GstAmount);
                } else {
                    item.NetAmount = (item.PurchasePriceAfterDiscount * item.ReturnQty);
                }
        ...
        function calculatetotalAmount() {
            ...
            for (var idx in $scope.PurchaseReturnDetails) {
                ...
                if (activeData.ItemMasterId > 0 && activeData.Status == 1) {
                    ...
                    // }
                    $scope.TotalDiscountAmount = parseFloat(($scope.TotalDiscountAmount + activeData.DiscountAmount * activeData.ReturnQty).toFixed(2));
        ...
        $scope.getfacilityCallback = function (scope, data, options, hasError) {
            $scope.data = data;
            $scope.item.IsPurchaseReturnEditable = $scope.data.IsPurchaseReturnEditable;
        };
    ...
    purchasereturnFormController.$inject = ['$rootScope', '$scope', '$stateParams', '$state', '$translate', 'utl', '$filter', '$timeout'];
