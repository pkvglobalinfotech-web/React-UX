(function () {
    ...
    angular
        .module('app.pages')
        .controller('POGRNAmendmentController', POGRNAmendmentController);

    function POGRNAmendmentController($rootScope, $scope, $stateParams, $state, $translate, utl, $filter, $timeout) {
        var vm = this;
        ...
        $scope.getGrnDetailsCallback = function (scope, res, options, hasError) {
            ...
            for (var idx in $scope.grnDetails) {
                ...
                if (grnitem.ItemMasterId > 0) {
                    ...
                    if (grnitem.Id > 0 && grnitem.ItemMasterId > 0) {
                        grnitem.IsLineDisabled = true;
                    }
                    else {
                        grnitem.IsLineDisabled = false;
                    }
        ...
        $scope.getPurchaseOrderCallback = function (scope, data, options, hasError) {
            ...
            $scope.currentfilter.PoNumber = result.PoNumber;
            ...
            if ($scope.item.GrnStatusId <= 2) {
                if ($scope.grnDetails.length > 1 && $scope.item.PurchaseOrderId === 0) {
                    for (var iddx in $scope.grnDetails) {
                        ...
                        //     if (iddxitem.ItemMasterId > 0 && parseFloat(iddxitem.UomMrPrice) < parseFloat(iddxitem.UomCostPrice) && iddxitem.Status === 1) {
    ...
    POGRNAmendmentController.$inject = ['$rootScope', '$scope', '$stateParams', '$state', '$translate', 'utl', '$filter', '$timeout'];
