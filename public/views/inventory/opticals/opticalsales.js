(function () {
    ...
    angular
        .module('app.pages')
        .controller('opticalsalesController', opticalsalesController);

    function opticalsalesController($rootScope,$scope, $interval, $stateParams, $state, $translate, utl, $filter, modalConfig, $timeout) {
        var vm = this;
        var savehitcompleted = 0;
        $scope.autosearchpopup = 0;

        $scope.separatePaymentCounter = 0; // Separate Payment Counter
        ...
        // $scope.currentcontext.CanPSConsumerBill = utl.Privilege.hasPrivilege('CanPSConsumerBill')
        ...
        function initPharmacy() {
            ...
            $scope.item = {
                PatientId: -1,
                // StoreMasterId:currentfilter.StoreMasterId,
            ...
            // $scope.currentcontext.CanPSFindBills = utl.Privilege.hasPrivilege('CanPSFindBills');
            // $scope.currentcontext.CanPSOutStanding = utl.Privilege.hasPrivilege('CanPSOutStanding');
            // $scope.currentcontext.CanPSPendingBills = utl.Privilege.hasPrivilege('CanPSPendingBills');
            // $scope.currentcontext.CanPSPendingPrescription = utl.Privilege.hasPrivilege('CanPSPendingPrescription');
            // $scope.currentcontext.CanPSFinanceInfo = utl.Privilege.hasPrivilege('CanPSFinanceInfo');
            // $scope.currentcontext.CanPSDMPrint = utl.Privilege.hasPrivilege('CanPSDMPrint');
            // $scope.currentcontext.CanPSViewReceipt = utl.Privilege.hasPrivilege('CanPSViewReceipt');
            // $scope.currentcontext.CanPSConsumerBill = utl.Privilege.hasPrivilege('CanPSConsumerBill');
        ...
        $scope.getPatientInfo = function (scope, data, options, hasError) {
            ...
            if ($scope.selectedPatient.Encounters && $scope.selectedPatient.Encounters.length > 0) {
                ...
                $scope.currentfilter.GuarantorId = encGuarantor.GuarantorId;
            }
        ...
        function replaceAlternate(alternatedata) {
            ...
            for (var count = 0; count < $scope.PatientBillDetails.length; count++) {
                var existeditem = $scope.PatientBillDetails[count];
                var existeditemindex = $scope.PatientBillDetails.indexOf(existeditem);
                if (existeditem.ItemMasterId == alternatedata.itemid && existeditemindex == alternatedata.lineindex) {
                    existeditem.Amount = 0;
                    existeditem.BatchDetails = alternatedata.ItemData.StockItem.StockSerialItems;
                    existeditem.BatchId = '';
                    existeditem.BatchQuantity = 0;
                    existeditem.CGstAmount = 0;
                    existeditem.CGstId = 0;
                    existeditem.CGstPercentage = 0;
                    existeditem.ExpiryDate = '';
                    existeditem.ExpiryProceed = false;
                    existeditem.GSTAmount = 0;
                    existeditem.GSTId = 0;
                    existeditem.GSTPercentage = 0;
                    existeditem.InGstAmount = 0;
                    existeditem.InGstId = 0;
                    existeditem.InGstPercentage = 0;
                    existeditem.ItemCode = alternatedata.ItemData.ItemCode;
                    existeditem.Id = 0;
                    existeditem.ItemMasterId = alternatedata.ItemData.Id;
                    existeditem.ItemName = alternatedata.ItemData.ItemName;
                    existeditem.ManufacturerId = alternatedata.ItemData.ManufacturerId;
                    existeditem.ManufacturerName = alternatedata.ItemData.ManufacturerName;
                    existeditem.MrPrice = 0;
                    existeditem.Mrp = 0;
                    existeditem.NetAmount = 0;
                    existeditem.NetAmountBeforeGST = 0;
                    existeditem.Quantity = 0;
                    existeditem.Rate = 0;
                    existeditem.SGstAmount = 0;
                    existeditem.SGstId = 0;
                    existeditem.SGstPercentage = 0;
                    existeditem.ScheduleTypeDescription = alternatedata.ItemData.ScheduleType.Description;
                    existeditem.ScheduleTypeId = alternatedata.ItemData.ScheduleTypeId;
                    existeditem.SelectedBatchId = '';
                    existeditem.ServiceCode = '(' + alternatedata.ItemData.ItemCode + ')';
                    existeditem.ServiceId = alternatedata.ItemData.Id;
                    existeditem.ServiceName = alternatedata.ItemData.ItemName;
                    existeditem.StockItemId = 0;
                    existeditem.StockSerialItemId = 0;
                    existeditem.TotalQuantity = alternatedata.ItemData.StockItem.Quantity;
                    existeditem.Ucp = 0;
                    existeditem.UnitCGstAmount = 0;
                    existeditem.UnitCostPrice = 0;
                    existeditem.UnitGSTAmount = 0;
                    existeditem.UnitInGstAmount = 0;
                    existeditem.UnitPrice = 0;
                    existeditem.UnitSGstAmount = 0;

                    //vm.pharmacyitemcontrolconfig.rowdata.ItemCode = alternatedata.ItemData.ItemCode;
                    //vm.pharmacyitemcontrolconfig.rowdata.ItemName = alternatedata.ItemData.ItemName;
                    //result = [vm.pharmacyitemcontrolconfig.rowdata.ItemName, vm.pharmacyitemcontrolconfig.rowdata.ItemCode].join(' ');

                    //vm.pharmacyitemcontrolconfig.selected.ItemMasterId = alternatedata.ItemData.Id;
                    //vm.pharmacyitemcontrolconfig.selected.ItemCode = alternatedata.ItemData.ItemCode;
                    //vm.pharmacyitemcontrolconfig.selected.ItemName = alternatedata.ItemData.ItemName;
                    //vm.pharmacyitemcontrolconfig.selected.ItemMaster = alternatedata.ItemData;

                    //formatselectedpharmacyitem();
                }
            }
            */
        }
        ...
        $scope.saleTypeChanged = function (item) {
            if (item.Text.toLowerCase() == "direct-sale") {
                ...
                // $scope.currentcontext.CanCancelBill = utl.Privilege.hasPrivilege('CanCancelBill');
        ...
        $scope.addNewLineItem = function () {
            var lastIndex = $scope.PatientBillDetails.length - 1;
            if (lastIndex >= 0) {
                if ($scope.PatientBillDetails[lastIndex].OpticalItemMasterId == -1)
                    return false;
            }
        ...
        $scope.ServiceItemChanged = function (idx, selectedItem) {
            ...
            if (selectedItem.IsThisPrescription) {
                ...
                */
                ...
                if (SelectedMasterItem.OpticalStockItem) {
                    selectedItem.MinQty = SelectedMasterItem.StockItem.MinQuantity;
                    selectedItem.TotalQuantity = SelectedMasterItem.StockItem.Quantity;
                    selectedItem.MaxQty = SelectedMasterItem.StockItem.MaxQuantity;
                ...
                if (selectedItem.SelectedItem.StockInHand <= 0) {
                    ...
                    selectedItem.StockItemId = SelectedMasterItem.Id;
                    selectedItem.OpticalItemMasterId = SelectedMasterItem.OpticalItemMasterId;
                    selectedItem.ItemCode = SelectedMasterItem.ItemCode;
                    selectedItem.ItemName = SelectedMasterItem.ItemName;
                    selectedItem.StoreMasterId = SelectedMasterItem.StoreMasterId;
                    ...
                    /*
                    selectedItem.ManufacturerId = SelectedMasterItem.OpticalItemMaster.ManufacturerId;
                    selectedItem.ManufacturerName = SelectedMasterItem.OpticalItemMaster.ManufacturerName;
                    */
                    ...
                    */
                    selectedItem.MinQty = SelectedMasterItem.MinQuantity;
                    selectedItem.TotalQuantity = SelectedMasterItem.Quantity;
                    selectedItem.MaxQty = SelectedMasterItem.MaxQuantity;
                    selectedItem.Quantity = 0;
                    selectedItem.Price = parseFloat(((SelectedMasterItem.OpticalItemMaster.SalesPrice * 100) / (100 + SelectedMasterItem.OpticalItemMaster.GSTPercentage)).toFixed(2));
                    selectedItem.MrPrice = SelectedMasterItem.OpticalItemMaster.SalesPrice;
                    selectedItem.Rate = SelectedMasterItem.OpticalItemMaster.SalesPrice;
                    ...
                    selectedItem.UnitGSTAmount = parseFloat((parseFloat(selectedItem.Price) / 100) * selectedItem.GSTPercentage).toFixed(2);
                    ...
                    selectedItem.StockItemRev = SelectedMasterItem.Rev;
                    if (selectedItem.TotalQuantity <= selectedItem.MinQty) {
                        selectedItem.IsFallUnderMinQty = true;
                    }
                    $scope.addNewLineItem();
        ...
        */
        ...
        $scope.CheckBatchQty = function (item) {
            if (item.BatchQuantity > 0) {
                if (parseInt(item.Quantity) > item.BatchQuantity) {
        ...
        $scope.CheckQty = function (item) {
            if (item.TotalQuantity > 0) {
                if (parseInt(item.Quantity) > item.TotalQuantity) {
                    utl.Alert.showSuccessMsg($translate.instant('billing.pharmacy.quantityalert.lbl'));
                    item.Quantity = 0;
                    item.Amount = 0;
                    item.GSTAmount = 0;
        ...
        $scope.backToList = function () {
            $state.go('app.opticalsales', $scope.currentcontext.id);
        };
        ...
        $scope.saveItem = function (StatusId) {
            ...
            if ($scope.item.PatientBillStatusId == 1 && dReceiptAmt > 0) {
                ...
                $scope.CalculateNetAmt();
                return false;
            } else if (($scope.item.PatientBillStatusId == 1 || $scope.item.PatientBillStatusId == 3) && dReceiptAmt <= 0 &&
                ...
                if ($scope.currentcontext.DiscountModeValue > 0) {
                    ...
                    if ($scope.currentfilter.DiscountModeId > 0 && $scope.currentfilter.DiscountModeId == 2) {
                        for (var per = 0, perlen = $scope.PatientBillDetails.length; per < perlen; per++) {
                            ...
                            $scope.PatientBillDetails[per].DiscountPercentage = $scope.currentcontext.DiscountModeValue;
                            $scope.PatientBillDetails[per].UnitProportionateDiscount = parseFloat(($scope.currentcontext.DiscountModeValue / 100 * $scope.PatientBillDetails[per].Rate).toFixed(2));
                            $scope.PatientBillDetails[per].ProportionateDiscount = parseFloat(($scope.currentcontext.DiscountModeValue / 100 * $scope.PatientBillDetails[per].Amount).toFixed(2));

                            var RateAfterDiscount = parseFloat($scope.PatientBillDetails[per].Rate) - parseFloat($scope.PatientBillDetails[per].UnitProportionateDiscount);
                            var UnitPriceAfterDiscount = parseFloat(((parseFloat(RateAfterDiscount) * 100) / (100 + parseFloat($scope.PatientBillDetails[per].GSTPercentage))).toFixed(2));
                            ...
                            $scope.PatientBillDetails[per].NetAmount = parseFloat((parseFloat(RateAfterDiscount) * parseInt($scope.PatientBillDetails[per].Quantity)).toFixed(2));
                        ...
                        for (var inr = 0, inrlen = $scope.PatientBillDetails.length; inr < inrlen; inr++) {
                            ...
                            var RateAfterDiscount = parseFloat($scope.PatientBillDetails[inr].Rate) - parseFloat($scope.PatientBillDetails[inr].UnitProportionateDiscount);
                            var UnitPriceAfterDiscount = parseFloat(((parseFloat(RateAfterDiscount) * 100) / (100 + parseFloat($scope.PatientBillDetails[inr].GSTPercentage))).toFixed(2));
                            ...
                            $scope.PatientBillDetails[inr].NetAmount = parseFloat((parseFloat(RateAfterDiscount) * parseInt($scope.PatientBillDetails[inr].Quantity)).toFixed(2));
                    ...
                    for (var disidx in $scope.PatientBillDetails) {
                        ...
                        if ($scope.currentfilter.DiscountModeId && $scope.currentfilter.DiscountModeId > 0 && $scope.currentfilter.DiscountModeId == 2) {
                            ...
                            disitem.UnitDiscountAmount = disitem.DiscountAmount / disitem.Quantity;
                        }
                    }
                }
            ...
            $scope.item.Comments = $scope.item.Comments.length > 0 ? $scope.item.Comments : '';
            $scope.item.ToBeRefunded = 0;
            ...
            if (checkMandatoryFields()) {
                ...
                */
                ...
                var actionName = 'billing/patientbills/AddPatientOpticalBills';
                if ($scope.currentcontext.id && $scope.currentcontext.id > 0) {
                    actionName = 'billing/patientbills/UpdatePatientOpticalBills';
                }
        ...
        function getLinesForSave() {
            ...
            for (var piidx in $scope.PatientBillDetails) {
                ...
                if (pharmacyitem.OpticalItemMasterId > 0 && parseInt(pharmacyitem.Quantity) > 0) {
                    //pharmacyitem.BillDateTime = utl.Formatter.getCurrentDate();
                    pharmacyitem.ServiceId = pharmacyitem.OpticalItemMasterId || 0;
                    ...
                    pharmacyitem.OpticalStockItemId = pharmacyitem.StockItemId;
        ...
        vm.pharmacyitemcontrolconfig = {
            ...
            */
            ...
            searchparams: {},
            result: {},
            api: 'pharmacy/opticalstockitem/GetOpticalStoreItems',
        ...
        function presearchpharmacyitem() {
            ...
            if (vm.pharmacyitemcontrolconfig.searchbyid === true) {
                ...
                inputData.Params.push({ Key: 8, Value: query });
            }
        ...
        function postsearchpharmacyitem() {
            ...
            for (var idx in vm.pharmacyitemcontrolconfig.result) {
                ...
                item.MrPrice = item.OpticalItemMaster.SalesPrice;
            }
        }
        ...
        function preparePrintData(data) {
            ...
            if (data.PatientBills.IsOpticalBill == true &&
    ...
    opticalsalesController.$inject = ['$rootScope','$scope', '$interval', '$stateParams', '$state', '$translate', 'utl', '$filter', 'modalConfig', '$timeout'];
