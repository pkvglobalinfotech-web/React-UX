(function() {
    ...
    opbilling.controller('updatetreatmentplanbillingController', updatetreatmentplanbillingController);

    function updatetreatmentplanbillingController($rootScope, $scope, $stateParams, $state, $translate, utl, $filter, $timeout, $uibModalInstance, modalConfig) {
        var vm = this;
        ...
        if (modalConfig && modalConfig.params) {
            ...
            $scope.currentcontext.did = parseInt(modalConfig.params.did);
            $scope.currentcontext.pdid = parseInt(modalConfig.params.pdid);
            $scope.currentcontext.tp = modalConfig.params.tp;
        ...
        vm.Context = $scope.currentcontext.tp;
        ...
        $scope.currentfilter.PatientId = $scope.currentcontext.pid;
        ...
        $scope.item.DoctorId = $scope.currentcontext.did;
        ...
        $scope.CalculateNetAmt = function() {
            ...
            $scope.currentcontext.TotNetAmount = parseFloat($scope.item.GrossAmount) + parseFloat($scope.item.RoundOffValue) + parseFloat($scope.item.TaxAmt || 0) - parseFloat($scope.item.TotDiscAmount) - parseFloat($scope.currentcontext.RefundAmount || 0);
            ...
            if ($scope.PatientBillInfo && $scope.PatientBillInfo.length > 0) {
                ...
                $scope.PatientBillInfo.forEach(patientbills => {
                    ...
                    for (var odx in patientbills.PatientBillDetails) {
                        ...
                        // if ($scope.currentcontextedit.IsPartialCancel) {
                        //     if (billItem.CancelReqRaisedStatusId == 1) {
        ...
        $scope.updatePlanDataCallback = function(scope, data, options, hasError) {
            if ($scope.currentcontext.from == 'details') {
                $scope.updatePlan();
            } else {
        ...
        $scope.updatePlanCallback = function(scope, data, options, hasError) {
            utl.Alert.showSuccessMsg($translate.instant('common.successmsg.lbl'));
            $scope.print();
            $scope.confirmCallback();
        };
        ...
        $scope.updatePlan = function() {
            $scope.items.Id = $scope.currentcontext.pdid;
            var actionName = 'emr/TreatmentPlanDetail/UpdateTreatmentPlanBills';
            var options = {
                action: actionName,
                data: {
                    Data: $scope.items
                ...
                type: 'post',
                onComplete: $scope.updatePlanCallback
            };
            utl.Http.doAction(options);
        }
        ...
        $scope.updatePlanData = function() {
            if ($scope.currentcontext.from == 'details') {
                $scope.items.Id = $scope.currentcontext.pdid;
                $scope.items.PatientBillId = $scope.currentcontext.id;
                $scope.items.PatientBillStatusId = 3;
                $scope.items.IsPaid = true;
                var actionName = 'emr/TreatmentPlanDetail/UpdateTreatmentPlanBillDetail';
                var options = {
                    action: actionName,
                    data: {
                        Data: $scope.items
                    ...
                    type: 'post',
                    onComplete: $scope.updatePlanDataCallback
                };
                utl.Http.doAction(options);
            }
            if ($scope.currentcontext.from == 'header') {
                $scope.items.Id = $scope.currentcontext.pdid;
                $scope.items.PatientBillId = $scope.currentcontext.id;
                $scope.items.BillingStatusId = 3;
                $scope.items.IsPaid = true;
                var actionName = 'emr/TreatmentPlan/UpdateTreatmentPlanBillInfo';
                var options = {
                    action: actionName,
                    data: {
                        Data: $scope.items
        ...
        $scope.gettplandetailsCallback = function(scope, data, options, hasError) {
            $scope.PatientBillDetails = [];
            if ($scope.currentcontext.from == 'details') {
                for (var dx in data.Data) {
                    var billdetail = data.Data[dx];
                    var PatientBillDetails = {
                        RdoDiscountMode: true, // disable discount mode
                        RdoDiscount: true, // disable discount
                        Id: 0,
                        ServiceId: billdetail.ServiceItemId,
                        ServiceCode: billdetail.ServiceCode,
                        ServiceName: billdetail.ServiceName,
                        RequestDate: billdetail.PlanScheduleDate,
                        DepartmentId: billdetail.DepartmentId,
                        SubDepartmentId: -1,
                        BillDateTime: utl.Formatter.getCurrentDate(),
                        IsPackage: false,
                        Quantity: 1,
                        Rate: billdetail.ServicePrice,
                        Amount: billdetail.ServicePrice,
                        ProportionateDiscount: 0,
                        DiscountAmount: 0,
                        DiscountModeId: -1,
                        CanDiscountProportionate: 0,
                        GSTAmount: 0,
                        TaxCode: 'ES',
                        NetAmount: billdetail.ServicePrice,
                        DiscountTypeId: -1,
                        IsOrderable: 0,
            ...
            if ($scope.currentcontext.from == 'header') {
                if (data.Data.length > 0) {
                    var GroupedBatchData = _.groupBy(data.Data, 'ServiceItemId');
                    for (var idx in GroupedBatchData) {
                        var serviceInfo = GroupedBatchData[idx];
                        var detlength = serviceInfo.length;
                        var billdetail = serviceInfo[0];
                        var PatientBillDetails = {
                            ...
                            Quantity: detlength,
                            Rate: billdetail.ServicePrice,
                            Amount: billdetail.ServicePrice * detlength,
                            ProportionateDiscount: 0,
                            DiscountAmount: 0,
                            DiscountModeId: -1,
                            CanDiscountProportionate: 0,
                            GSTAmount: 0,
                            TaxCode: 'ES',
                            NetAmount: billdetail.ServicePrice * detlength,
                            DiscountTypeId: -1,
                            IsOrderable: 0,
                            ServiceCategoryId: billdetail.ServiceCategoryId,
                            Status: 1,
        ...
        $scope.gettplandetails = function() {
            if ($scope.currentcontext.pdid && !$scope.currentcontext.id && $scope.currentcontext.from == 'details') {
                var inputData = {
                    Params: [{
                        Key: 0,
                        Value: $scope.currentcontext.pdid
                    }],
                };
                var options = {
                    ...
                    data: inputData,
                    type: 'post',
                    onComplete: $scope.gettplandetailsCallback
                };
                utl.Http.doAction(options);
            }
            if ($scope.currentcontext.pdid && !$scope.currentcontext.id && $scope.currentcontext.from == 'header') {
                var inputData = {
                    Params: [{
                        Key: 1,
                        Value: $scope.currentcontext.pdid
    ...
    updatetreatmentplanbillingController.$inject = ['$rootScope', '$scope', '$stateParams', '$state', '$translate', 'utl', '$filter', '$timeout', '$uibModalInstance', 'modalConfig'];
