(function () {
    ...
    angular
        .module('app.pages')
        .controller('patientIndentListController', patientIndentListController);

    function patientIndentListController($scope, $filter, $stateParams, $state, $translate, utl) {
        var vm = this;
        ...
        $scope.getList = function (pageNo) {
            ...
            if ($scope.context == 'emr' || $scope.context == 'surgery') {
                ...
                inputData.Params.push({ Key: 5, Value: $scope.currentcontext.pid },
                    // { Key: 4, Value: $scope.currentfilter.PatientRequestStatusId },
                    { Key: 14, Value: utl.Formatter.getFilterDate(From) }, { Key: 15, Value: utl.Formatter.getFilterDate(To) });
            }
            var options = {
        ...
        $scope.addNew = function () {
            // $state.go('patientemr.patientindentform', { id: 0 });
            ...
            if ($scope.context == 'surgery') {
                $state.go('surgeryentry.patientindentform', {
                    id: 0,
                    surgeryid: $scope.currentcontext.surgeryid,
                    context: 'surgery'
                });
            }
        };
        ...
        $scope.handleEvents = function (actionType, entity) {
            if (actionType == 'view') {
                ...
                $state.go('patientemr.patientindentform', { id: entity.Id, eid: entity.EncounterId, pid: entity.PatientId });
                // if ($scope.context == 'emr')
                //     $state.go('patientemr.medicinerequestform', { id: entity.Id, pid: entity.PatientId });
            }
            if (actionType == 'edit') {
                // if ($scope.context == 'main')
                $state.go('patientemr.patientindentform', { id: entity.Id, eid: entity.Id, pid: entity.PatientId });
                // if ($scope.context == 'emr')
                //     $state.go('patientemr.medicinerequestform', { id: entity.Id, pid: entity.PatientId });
        ...
        vm.gridConfig = {
            ...
            },
            {
                ...
                cellTemplate: "<div class='ui-grid-cell-contents'>" +
                    "{{entity.DispensedUser.Title.Description}}&nbsp;</span>" +
                    "<span >{{entity.DispensedUser.FirstName}}&nbsp;</span>" +
                    "<span >{{entity.DispensedUser.LastName}}</span>" +
                    "</a></div>"
            },
            { field: "PatientRequestStatus.Description", displayName: $translate.instant('patientrequests.status.lbl') },
            {
                ...
                displayName: $translate.instant('common.actions_col.lbl'),
                cellTemplate: '<div class="ui-grid-cell-contents">\
                                                <span class="grid-action" ng-click="handleEvents(\'view\',entity)" ng-show="entity.PatientRequestStatusId == 2 || entity.PatientRequestStatusId == 3 || entity.PatientRequestStatusId == 4 || entity.PatientRequestStatusId == 5 || entity.PatientRequestStatusId == 6 || entity.PatientRequestStatusId == 7"><img class="drhms-edit-button" src="assets/svg/edit.svg" aria-hidden="true"> </span>\
    ...
    patientIndentListController.$inject = ['$scope', '$filter', '$stateParams', '$state', '$translate', 'utl'];
