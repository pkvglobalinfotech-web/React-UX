(function () {
    ...
    angular
        .module('app.pages')
        .controller('purchasetallyapprovedController', purchasetallyapprovedController);

    function purchasetallyapprovedController($rootScope, $scope, $filter, $stateParams, $state, $translate, utl, $timeout) {
        var vm = this;
        ...
        vm.gridConfig = {
            ...
            {
                field: "GrnNumber",
                displayName: $translate.instant('GRN/Inv#'),
                cellTemplate: "<div class='ui-grid-cell-contents'><span >{{entity.GrnNumber}}&nbsp;{{entity.InvoiceNumber}}</span>" + "</div>"
            },
            {
            ...
            },
            {
                ...
                displayName: $translate.instant('Purchase Status')
            },
            {
                field: "TallyApprovedStatus.Description",
                displayName: $translate.instant('Tally Status')
            },
                // {
                //     field: "Id",
        ...
        function setDefaults1() {
            var ApprovedId = utl.Lookup.getDefault($scope.lookup.TallyApprovedStatus, 'Approved');
            var AuthorizedId = utl.Lookup.getDefault($scope.lookup.TallyApprovedStatus, 'Authorized');
            //var CompletedId = utl.Lookup.getDefault($scope.lookup.TallyApprovedStatus, 'Completed');
            $scope.currentfilter.TallyApprovedStatusId = ApprovedId;
        }
        ...
        $scope.approveOrder = function () {
            ...
            var options = {
                action: 'pharmacy/grn/ManagePurchaseTallyApprove',
                data: {
                    Data: lines
                },
                type: 'post',
        ...
        $scope.getSelectionRows = function () {
            ...
            for (var idx in $scope.items) {
                var item = $scope.items[idx];
                if (item.IsSelected == true && item.GrnStatusId >= 2) {
                    item.TallyApprovedStatusId = 2;
                    item.IsTallyApproved = true;
    ...
    purchasetallyapprovedController.$inject = ['$rootScope', '$scope', '$filter', '$stateParams', '$state', '$translate', 'utl', '$timeout'];
